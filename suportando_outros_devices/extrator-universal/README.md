...
- UI gráfica adaptativa em SDL2 ou no framebuffer direto, com o mesmo painel,
  paleta, texto bilíngue, barras real/geral e estados de sucesso/erro;
- backend visual negociado pela SDL: `mali`/fbdev, KMSDRM, Wayland ou outro backend visível
  anunciado pelo firmware; após uma falha real, providers não fixados podem ser
  recuperados pelos nomes portáveis `libEGL.so`/`libGLESv2.so`; se EGL/GLES
  ainda não abrir, o mesmo desenho gráfico usa uma superfície de software no
  framebuffer, sem substituir a interface por uma caixa ASCII;
- segunda execução rápida pelo marcador validado, sem exigir que o pacote original ainda
  esteja disponível;
- resultado terminal JSON atômico com fase final e código estável, sem nome ou
  origem do pacote externo, além de log compacto e log detalhado separados.
...
O gate de release executa 72 fixtures sintéticas, testa o isolamento de runtime
e a pinagem integral, recusa arquivos vazios/symlink/hardlink e audita todo ELF
encontrado. Nenhum APK, OBB, ZIP ou dado proprietário pode existir na árvore de
release. O NXExtract standalone conserva a chave abaixo; no framework público
ela é sempre obrigatória e não existe opt-out por port. O runner exige uma prova
privada de que um frame gráfico SDL ou fbdev foi apresentado antes de permitir
a leitura/extração dos dados:
...
Ao terminar, `nxextract-result.json` contém o resultado estruturado conforme
[`docs/terminal-result-schema-v1.json`](docs/terminal-result-schema-v1.json).
`nxextract.log` conserva marcos, misses resumidos e a causa terminal;
`nxextract-detail.log` conserva a lista por arquivo e a saída completa dos
hooks. Para um diagnóstico explícito, `--verbose-log` ou
`NXEXTRACT_VERBOSE_LOG=1` também espelha o detalhe no log compacto.
...
continua livre para autodetectar. Se a janela falhar e o firmware não tiver
fixado providers explícitos, a UI tenta uma vez `libEGL.so`/`libGLESv2.so`;
uma escolha explícita nunca é substituída. Se uma janela real continuar
impossível, a UI reutiliza exatamente o mesmo renderer gráfico em `/dev/fb0`.
A prova gráfica tem limite fail-closed de 40 segundos; sua ausência falha antes
de examinar ou modificar os dados do dono. O antigo TTY ASCII existe apenas
como diagnóstico explícito e nunca autoriza uma execução pública.

O handshake da UI não fica em disco nenhum. FAT, exFAT e alguns FUSE
reportam `0777` mesmo quando `chmod` é pedido, e uma sessão de login com
`Linger=no` pode reciclar o `XDG_RUNTIME_DIR` no meio de uma extração longa.
Por isso o motor abre um canal privado de sessão — dois pipes, validados por
dono, tipo, modo e identidade dev/inode antes do spawn — e entrega à UI somente
descritores herdados (`fd:N`). Prova de renderer e pedido de parada viajam por
esses descritores; remover qualquer diretório durante a extração não invalida
uma transação de dados já validada. O `ui.log` diagnóstico continua persistente
no workspace, sem poder de autorizar a extração, e nunca imponha permissões
POSIX privadas ao estado persistente dentro do game filesystem.
...
Para validar uma atualização de binário sem autorizar reinstalação, use o modo
fechado de reaproveitamento:

```bash
NXEXTRACT_REUSE_ONLY=1 ./run-extractor.sh
```

Ele aceita somente um payload já instalado e autenticado. Se houver corrupção,
marcador incompatível ou uma transação pendente, termina com erro antes de
recuperar a transação, abrir a UI, procurar a fonte ou extrair dados. A única
escrita permitida no sucesso é a migração/re-selagem atômica do marcador quando
os bytes existentes forem comprovados. Se um marcador 1.2.20 também mudou nos
metadados, obtenha primeiro o selo forte pelo comando read-only `content-seal`
e forneça-o por `NXEXTRACT_EXPECT_CONTENT_SEAL`; sem essa prova o fluxo aborta.
...
Quando `container` copia o APK para o runtime, `input.packages` é obrigatório e
a compatibilidade não pode ficar presa a um único SHA-256, CRC32 ou tamanho
exato externo: assinatura, alinhamento e compressão podem mudar sem alterar a
carga necessária. Limite
`min_size`/`max_size`, exija magic ZIP e fixe por SHA-256 as bibliotecas ou
outros membros críticos extraídos. Se a engine exigir builds de container
totalmente distintos, liste duas ou mais identidades explícitas e documente
cada APK aceito.
...
O build de release usa o Zig 0.16.0 fixado e gera artefatos separados em
`ui/release/{aarch64,armv7,x86_64,i386}/nxextract-ui`. O manifesto canônico
`ui/release/manifest-v1.json` fixa arquitetura, tamanho, modo e SHA-256. Todo
ELF distribuído precisa exigir **GLIBC 2.30 ou anterior**:

```bash
./ui/tools/release-manifest.py --verify
```

Os quatro artefatos exigem no máximo GLIBC 2.17. A UI carrega SDL2 em runtime;
por isso não fica presa à SDL de um único firmware. Se nenhum renderer gráfico
comprovado abrir, o runner público falha antes de examinar ou modificar os dados
do dono.
