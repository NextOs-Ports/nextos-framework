...
void *SDL_CreateWindow(const char *title, int x, int y, int width, int height,
  ...
  int portable = egl && gles && strcmp(egl, "libEGL.so") == 0 &&
                 strcmp(gles, "libGLESv2.so") == 0;
  int explicit_provider =
      getenv("NXEXTRACT_FAKE_ACCEPT_EXPLICIT") && egl && gles &&
      strcmp(egl, "explicit-egl") == 0 &&
      strcmp(gles, "explicit-gles") == 0;
  (void)title;
  (void)x;
  (void)y;
  (void)width;
  (void)height;
  (void)flags;
  return portable || explicit_provider ? (void *)1 : NULL;
}
void SDL_DestroyWindow(void *window) { (void)window; }
void *SDL_CreateRenderer(void *window, int index, uint32_t flags) {
  (void)index;
  (void)flags;
  return window ? (void *)2 : NULL;
}
void *SDL_CreateRGBSurfaceFrom(void *pixels, int width, int height, int depth,
                               int pitch, uint32_t red, uint32_t green,
...
void SDL_FreeSurface(void *surface) { (void)surface; }
void *SDL_CreateSoftwareRenderer(void *surface) {
  return surface ? (void *)2 : NULL;
}
