...
PREFIX_SAFE_DIR="$TEST_ROOT/game data-safe"
OUTSIDE_ALIAS_TO_GAME="$TEST_ROOT/outside-alias-to-game"
mkdir -p \
  "$GAME_DIR/libs" \
  "$GAME_DIR/private" \
  "$FIRMWARE_DIR" \
...
ln -s "$GAME_DIR/libs" "$OUTSIDE_ALIAS_TO_GAME"
...
original_ld="$GAME_DIR/libs:$INHERITED_DIR:$GAME_DIR/private:$GAME_ALIAS/libs:$GAME_DIR/private-link:$OUTSIDE_ALIAS_TO_GAME:$PREFIX_SAFE_DIR"
...
case ":$direct_ld:" in
  *":$OUTSIDE_ALIAS_TO_GAME:"*) fail "an outside alias back into game-private data leaked" ;;
esac
case ":$direct_ld:" in
  *":$FIRMWARE_DIR:"*) ;;
...
case ":$direct_ld:" in
  *":$PREFIX_SAFE_DIR:"*) ;;
  *) fail "a safe path sharing only the game-directory prefix was removed" ;;
esac
case "$direct_ld" in
...
case "$(sed -n 's/^args=//p' "$CAPTURE")" in
  *nxextract.py*install*--require-ui*) ;;
  *) fail "run-extractor did not require visible UI readiness" ;;
esac
...
NXEXTRACT_FIRMWARE_LIBRARY_PATH="$FIRMWARE_DIR" \
NXEXTRACT_MACHINE=aarch64 \
NXEXTRACT_REUSE_ONLY=1 \
NXEXTRACT_EXPECT_CONTENT_SEAL=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA \
TEST_CAPTURE="$CAPTURE" \
  "$PROJECT_ROOT/run-extractor.sh"
case "$(sed -n 's/^args=//p' "$CAPTURE")" in
  *nxextract.py*install*--require-ui*--reuse-only*--expected-content-seal*aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa*) ;;
  *) fail "run-extractor did not preserve the fail-closed reuse-only policy" ;;
esac

# ZIP extraction on FAT/exFAT and some PortMaster install paths can drop mode
# bits. The launcher must invoke its helper through bash and keep the same
# process-scoped isolation even when that helper is a regular 0644 file.
MODELESS_ROOT="$TEST_ROOT/modeless-package"
mkdir -p "$MODELESS_ROOT"
cp "$PROJECT_ROOT/run-extractor.sh" "$MODELESS_ROOT/run-extractor.sh"
cp "$PROJECT_ROOT/nxextract-runtime-env.sh" "$MODELESS_ROOT/nxextract-runtime-env.sh"
chmod 0755 "$MODELESS_ROOT/run-extractor.sh"
chmod 0644 "$MODELESS_ROOT/nxextract-runtime-env.sh"
[ ! -x "$MODELESS_ROOT/nxextract-runtime-env.sh" ] ||
  fail "the modeless helper fixture is unexpectedly executable"
...
SDL_VIDEODRIVER=kmsdrm \
TEST_CAPTURE="$CAPTURE" \
  "$MODELESS_ROOT/run-extractor.sh" --input mode-bits-lost.apk

[ "$(sed -n 's/^sdl=//p' "$CAPTURE")" = kmsdrm ] ||
  fail "the non-executable helper changed the inherited SDL backend"
[ "$(sed -n 's/^active=//p' "$CAPTURE")" = 1 ] ||
  fail "the non-executable helper bypassed the isolated child scope"
modeless_ld=$(sed -n 's/^ld=//p' "$CAPTURE")
case ":$modeless_ld:" in
  *":$GAME_DIR:"*|*":$GAME_DIR/"*)
    fail "the non-executable helper leaked a game-private path"
    ;;
esac
case "$modeless_ld" in
  *"$FIRMWARE_DIR"*"$INHERITED_DIR"*) ;;
  *) fail "the non-executable helper changed firmware-first library ordering" ;;
esac
case "$(sed -n 's/^args=//p' "$CAPTURE")" in
  *nxextract.py*install*--input*mode-bits-lost.apk*) ;;
  *) fail "the non-executable helper changed the CLI arguments" ;;
esac
case "$(sed -n 's/^args=//p' "$CAPTURE")" in
  *nxextract.py*install*--require-ui*) ;;
  *) fail "the modeless runner lost mandatory UI readiness" ;;
esac
