...
TOOLCHAIN_ROOT="${NXSPLASH_TOOLCHAIN_ROOT:-${XDG_CACHE_HOME:-$HOME/.cache}/nxsplash/toolchains}"
ZIG_VERSION=0.16.0
...
ZIG="${NXSPLASH_ZIG:-$ZIG_ROOT/zig}"

mkdir -p "$TOOLCHAIN_ROOT" "$TOOLCHAIN_ROOT/cache"
if [ ! -x "$ZIG" ]; then
  command -v curl >/dev/null 2>&1 || {
    echo "nxsplash build: curl is required to fetch the pinned Zig toolchain" >&2
    exit 1
  }
  ...
  [ "$actual_sha" = "$ZIG_SHA256" ] || {
    echo "nxsplash build: Zig archive hash mismatch" >&2
    exit 1
  }
  extract_root=$(mktemp -d "$TOOLCHAIN_ROOT/.zig-extract.XXXXXX")
  tar -xf "$archive" -C "$extract_root"
  [ ! -e "$ZIG_ROOT" ] || {
    echo "nxsplash build: non-executable Zig directory already exists" >&2
    exit 1
  }
...
[ "$("$ZIG" version)" = "$ZIG_VERSION" ] || {
  echo "nxsplash build: Zig version mismatch" >&2
  exit 1
}
...
build_one() {
  ...
  output="$COMPONENT_DIR/release/$architecture/nxsplash-nextos"
  ...
  "$ZIG" cc \
    ...
    -pie \
    -s \
    -ffile-prefix-map="$COMPONENT_DIR"=. \
    -fno-ident \
    -Wall \
    -Wextra \
    -Werror \
