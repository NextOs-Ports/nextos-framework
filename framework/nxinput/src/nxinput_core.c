...
int nxinput_core_cursor_dpad_engages(uint32_t capabilities, uint32_t options,
                                     int context_is_menu) {
  return (options & NXINPUT_CURSOR_OPTION_DPAD_IF_NO_STICK) != 0u &&
         context_is_menu != 0 &&
         (capabilities & NXINPUT_PAD_CAP_DPAD) != 0u &&
         (capabilities & NXINPUT_PAD_CAP_LEFT_STICK) == 0u &&
         (capabilities & NXINPUT_PAD_CAP_RIGHT_STICK) == 0u;
}

void nxinput_core_cursor_axes_from_dpad(uint32_t buttons, float *x,
                                        float *y) {
  int horizontal;
  int vertical;
  float scale;
  ...
  if (!x || !y)
    return;
  horizontal =
      ((buttons & NXINPUT_BUTTON_BIT(NXINPUT_BUTTON_DPAD_RIGHT)) != 0u) -
      ((buttons & NXINPUT_BUTTON_BIT(NXINPUT_BUTTON_DPAD_LEFT)) != 0u);
  vertical =
      ((buttons & NXINPUT_BUTTON_BIT(NXINPUT_BUTTON_DPAD_DOWN)) != 0u) -
      ((buttons & NXINPUT_BUTTON_BIT(NXINPUT_BUTTON_DPAD_UP)) != 0u);
  scale = horizontal != 0 && vertical != 0 ? 0.70710678118f : 1.0f;
  *x = (float)horizontal * scale;
  *y = (float)vertical * scale;
}
