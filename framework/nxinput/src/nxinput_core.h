 ...
 *
 * Engagement predicate: the additive opt-in acts ONLY when the option bit is
 * set, the context is a proven MENU, and the MEASURED capabilities show a
 * D-pad and ZERO sticks. Never keyed to a model, CFW or name; never in
 * gameplay. */
int nxinput_core_cursor_dpad_engages(uint32_t capabilities, uint32_t options,
                                     int context_is_menu);
/* Derive the cursor axis vector from the D-pad button bits (diagonals
 * normalized), exactly the vector a stick would have fed. */
void nxinput_core_cursor_axes_from_dpad(uint32_t buttons, float *x, float *y);
