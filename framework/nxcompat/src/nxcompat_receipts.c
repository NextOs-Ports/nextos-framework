...
static int nxcompat_input_mapping_generation(
    const nxcompat_registry *registry, uint64_t preferred,
    uint64_t *generation) {
  uint64_t previous;
  if (!registry || !generation)
    return 0;
  previous = registry
                 ->evidence[NXCOMPAT_CAPABILITY_INPUT_CONTROLLER_MAPPING]
                 .generation;
...
nxcompat_result_code nxcompat_registry_publish_input_ex(
  ...
  if (mapping_resolved) {
    if (!nxcompat_input_mapping_generation(
            &staged, receipt->topology_generation, &mapping_generation)) {
      nxcompat_receipt_store_reason(
          reason, NXCOMPAT_REASON_OBSERVATION_OUT_OF_RANGE);
      return NXCOMPAT_FAILED;
    }
    ...
    nxcompat_registry_stage_evidence(
        ...
        NXCOMPAT_REASON_CAPABILITY_PUBLISHED, mapping_generation);
  } else if (staged.probe_controller_mapping) {
    nxcompat_capability_evidence *current =
        &staged.evidence[NXCOMPAT_CAPABILITY_INPUT_CONTROLLER_MAPPING];
    if (current->state != NXCOMPAT_EVIDENCE_OBSERVED ||
        current->source != NXCOMPAT_SOURCE_PROBE) {
      uint64_t preferred = staged.probe_controller_mapping_generation;
      if (receipt->topology_generation > preferred)
        preferred = receipt->topology_generation;
      if (!nxcompat_input_mapping_generation(&staged, preferred,
                                             &mapping_generation)) {
        nxcompat_receipt_store_reason(
            reason, NXCOMPAT_REASON_OBSERVATION_OUT_OF_RANGE);
        return NXCOMPAT_FAILED;
      ...
      nxcompat_registry_stage_evidence(
          ...
          NXCOMPAT_REASON_PROBE_COMPLETE, mapping_generation);
    }
    ...
    nxcompat_registry_stage_evidence(
        &staged, NXCOMPAT_CAPABILITY_INPUT_CONTROLLER_MAPPING,
        NXCOMPAT_EVIDENCE_LOST, receipt->source,
        NXCOMPAT_REASON_CAPABILITY_LOST, mapping_generation);
  }
  nxcompat_registry_stage_boolean(
