...
const char *nxcompat_reason_name(nxcompat_reason_code value) {
  ...
  case NXCOMPAT_REASON_INPUT_CONTROLLER_MAPPING_RESOLVED:
    return "input-controller-mapping-resolved";
...
static void nxcompat_probe_library_arches(const nxcompat_context *context,
  ...
  /* The strongest evidence of all: this probe runs INSIDE the port's own
   * dynamically linked process.  If this code is executing, the dynamic
   * linker already resolved a working runtime for OUR architecture — however
   * the launcher arranged it (standard multiarch dirs, an explicit alternate
   * ld.so with --library-path on 64-bit-only firmware, a mounted 32-bit
   * tree...).  Path lists can never enumerate every firmware layout; a live
   * process is proof by construction.  Field case: 64-bit-only firmware with
   * the ARMHF runtime under a vendor mount refused host.armhf-libs while the
   * ARMv7 loader was literally running. */
...
static int nxcompat_relative_controller_file_valid(const char *path) {
  const char *component;
  const char *terminator;
  if (!path || path[0] == '\0' || path[0] == '/')
    return 0;
  terminator = (const char *)memchr(path, '\0', NXCOMPAT_PATH_MAX);
  if (!terminator)
    return 0;
  component = path;
  while (component < terminator) {
    const char *slash = memchr(component, '/', (size_t)(terminator - component));
    const char *end = slash ? slash : terminator;
    size_t length = (size_t)(end - component);
    if ((length == 1u && component[0] == '.') ||
        (length == 2u && component[0] == '.' && component[1] == '.'))
      return 0;
    component = slash ? slash + 1 : terminator;
  }
  return 1;
}
...
static void nxcompat_probe_portmaster(const nxcompat_context *context,
  ...
  const char *controller_file_environment;
  int inherited_controller_file = 0;
  int inherited_controller_candidate_length;
  char inherited_controller_candidate[NXCOMPAT_PATH_MAX];
  ...
  controller_file_environment = getenv("SDL_GAMECONTROLLERCONFIG_FILE");
  if (controller_file_environment && controller_file_environment[0] != '\0') {
    inherited_controller_file = 1;
    if (controller_file_environment[0] == '/' &&
        memchr(controller_file_environment, '\0',
               sizeof(host->controller_db)) != NULL &&
        nxcompat_regular_nonempty_file_exists(
            context, controller_file_environment))
      nxcompat_copy_string(host->controller_db, sizeof(host->controller_db),
                           controller_file_environment);
    else if (nxcompat_relative_controller_file_valid(
                 controller_file_environment) &&
             options->game_dir && options->game_dir[0] == '/' &&
             (inherited_controller_candidate_length =
                  snprintf(inherited_controller_candidate,
                           sizeof(inherited_controller_candidate), "%s/%s",
                           options->game_dir,
                           controller_file_environment)) > 0 &&
             (size_t)inherited_controller_candidate_length <
                 sizeof(inherited_controller_candidate) &&
             nxcompat_regular_nonempty_file_exists(
                 context, inherited_controller_candidate))
      nxcompat_copy_string(host->controller_db, sizeof(host->controller_db),
                           inherited_controller_candidate);
  }

  if (!inherited_controller_file && host->portmaster_dir[0]) {
    char candidate[NXCOMPAT_PATH_MAX];
    ...
    for (index = 0; names[index]; ++index) {
      if (snprintf(candidate, sizeof(candidate), "%s/%s",
          ...
          (int)sizeof(candidate))
        continue;
      if (nxcompat_regular_nonempty_file_exists(context, candidate)) {
        nxcompat_copy_string(host->controller_db, sizeof(host->controller_db),
                             candidate);
        break;
      }
  ...
  if (!inherited_controller_file && !host->controller_db[0]) {
    for (database = global_databases; *database; ++database)
      if (nxcompat_regular_nonempty_file_exists(context, *database)) {
        nxcompat_copy_string(host->controller_db, sizeof(host->controller_db),
                             *database);
        break;
      }
