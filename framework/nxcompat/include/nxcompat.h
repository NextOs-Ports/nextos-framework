...
#define NXCOMPAT_CAPABILITY_COUNT 33u
...
typedef enum nxcompat_capability_id_v1 {
  ...
  NXCOMPAT_CAPABILITY_AUDIO_EMBEDDED_OPENAL = 31,
  /* Anexada no fim de proposito: os ids acima ja' viajaram em recibos
     gravados, entao renumerar para colocar GLES1 ao lado de GLES2 quebraria
     a leitura deles. */
  NXCOMPAT_CAPABILITY_GRAPHICS_GLES1 = 32
...
enum nxcompat_graphics_capability {
  ...
  NXCOMPAT_GRAPHICS_GLES1 = UINT64_C(1) << 7
};
...
typedef enum nxcompat_input_mapping_source {
  ...
  /* These two values prove mapping material only, even if a controller is
   * separately open in the same receipt. */
  NXCOMPAT_INPUT_MAPPING_PORTMASTER_ENV = 1,
  NXCOMPAT_INPUT_MAPPING_DATABASE = 2,
  /* These values assert that the opened controller's mapping was resolved by
   * the runtime or engine, allowing OPENED evidence. */
