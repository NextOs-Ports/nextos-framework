...
nxcompat_result_code nxgl_nxcompat_publish_context(
  ...
  nxcompat_graphics_receipt receipt;
  const nxgl_report_v2 *report_v2;
  SDL_Window *window;
  ...
  report_v2 = nxgl_get_report_v2(context);
  if (report_v2) {
    if (report_v2->api_version != NXGL_API_VERSION_V2 ||
        report_v2->struct_size < sizeof(*report_v2) ||
        report_v2->stack_owner != NXGL_STACK_OWNER_V2_SDL_EGL ||
        report_v2->handles.api_version != NXGL_API_VERSION_V2 ||
        report_v2->handles.struct_size < sizeof(report_v2->handles) ||
        report_v2->handles.owner != NXGL_STACK_OWNER_V2_SDL_EGL)
      return NXCOMPAT_FAILED;
    window = report_v2->handles.sdl_window;
    sdl_context = report_v2->handles.sdl_context;
    /* API v2 freezes the provider and deliberately rejects the API-v1
     * make-current accessor. The strong bridge runs immediately after open,
     * so it accepts only the exact SDL window/context that are already
     * current; it never calls another provider or changes ownership. */
    if (!window || !sdl_context || SDL_GL_GetCurrentWindow() != window ||
    ...
    if (!window || !sdl_context ||
        nxgl_make_current(context) != NXGL_SUCCESS ||
