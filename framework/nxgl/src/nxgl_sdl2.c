...
#if !defined(NXGL_M13_TESTING) && !defined(NXGL_CORE_TESTING)
#include <dlfcn.h>
#endif
...
static int nxgl_reset_hint_available(void) {
  return nxgl_test_sdl_reset_hint_available();
}

static int nxgl_reset_hint_compat(const char *name) {
  if (!nxgl_reset_hint_available())
    return -1;
  nxgl_test_sdl_reset_hint(name);
  return 0;
}
#elif !defined(NXGL_CORE_TESTING)
typedef void(SDLCALL *nxgl_sdl_reset_hint_fn)(const char *name);

/* SDL_ResetHint only entered SDL 2.24. Resolve it optionally so merely
 * linking nxgl keeps the framework's SDL 2.0.4 floor. On an older SDL, hint
 * recovery is disabled whenever an absent hint could not be restored exactly;
 * the normal candidate ladder remains available and global hint state is never
 * approximated with an empty string or SDL_ClearHints(). */
static nxgl_sdl_reset_hint_fn nxgl_resolve_reset_hint(void) {
  nxgl_sdl_reset_hint_fn function = NULL;
  void *symbol;

  if (sizeof(function) != sizeof(symbol))
    return NULL;
  (void)dlerror();
  symbol = dlsym(RTLD_DEFAULT, "SDL_ResetHint");
  if (!symbol)
    return NULL;
...
static int nxgl_reset_hint_available(void) {
  return nxgl_resolve_reset_hint() != NULL;
}

static int nxgl_reset_hint_compat(const char *name) {
  nxgl_sdl_reset_hint_fn function = nxgl_resolve_reset_hint();
  if (!function)
    return -1;
  function(name);
  return 0;
...
static int nxgl_snapshot_hint(const char *name, int *existed, char **snapshot) {
  ...
  *snapshot = NULL;
  ...
  if (!value)
    return nxgl_reset_hint_available() ? 0 : -1;
  *existed = 1;
...
static int nxgl_restore_hint(const char *name, int existed,
  ...
  if (existed) {
    ...
    if (nxgl_reset_hint_compat(name) != 0)
      failed = 1;
  }
...
/* An old SDL without SDL_ResetHint cannot restore a hint that was absent on
 * entry.  That makes only the optional hint-recovery retry unavailable: the
 * ordinary candidate round does not mutate either hint and remains safe. */
static int nxgl_prepare_hint_recovery(nxgl_context *context,
                                      nxgl_status_callback status,
                                      void *status_userdata) {
  if (nxgl_save_hint_snapshots(context) == 0)
    return 1;
  nxgl_emit(status, status_userdata, NXGL_STATUS_WARNING,
            "SDL GLES/EGL hints could not be snapshotted exactly; hint "
            "recovery disabled");
  return 0;
}
...
#if defined(NXGL_M13_TESTING)
int nxgl_test_v2_hint_snapshot_policy(int *normal_round_permitted,
                                      int *hint_recovery_permitted,
                                      nxgl_report_v2 *report) {
  nxgl_context context;
  if (!normal_round_permitted || !hint_recovery_permitted || !report)
    return NXGL_ERROR_INVALID_ARGUMENT;
  memset(&context, 0, sizeof(context));
  nxgl_v2_report_init(&context.report_v2);
  context.stack_is_default_sdl = 1;
  *normal_round_permitted = 1;
  *hint_recovery_permitted =
      nxgl_prepare_hint_recovery(&context, NULL, NULL);
...
static int nxgl_v2_sdl_start_video(
  ...
  *initialized_by_stack = 0;
  if (SDL_WasInit(SDL_INIT_VIDEO) == 0) {
    ...
    SDL_ClearError();
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
      /* Camada 7 do Miyoo Flip (19/08): o log de campo dizia apenas
       * "video-init-failed" e a causa real (o unico driver de video do SDL2
       * 32-bit do muOS-pixie e "mali"/fbdev-EGL) ficava invisivel.  O texto
       * do SDL_GetError() e a unica testemunha do driver que recusou. */
      const char *sdl_error = SDL_GetError();
      if (sdl_error && *sdl_error) {
        char detail[NXGL_DETAIL_MAX];
        SDL_snprintf(detail, sizeof(detail), "video-init-failed: %s",
                     sdl_error);
        nxgl_copy_string(error, error_size, detail);
      } else {
...
int nxgl_open_v2(const nxgl_open_options_v2 *options,
...
#if !defined(NXGL_M13_TESTING)
    (void)nxgl_prepare_hint_recovery(context, options->status,
                                     options->status_userdata);
#endif
  }
  ...
  if (status == NXGL_SUCCESS) {
    /* Preserve the successfully started backend even when all later context
     * attempts fail. A caller may use this sanitized fact with a pure,
     * adapter-owned recovery policy; it is not itself authorization. */
    nxgl_copy_string(context->report_v2.legacy.video_backend,
                     sizeof(context->report_v2.legacy.video_backend),
                     backend);
