...
typedef enum nxloader_state {
  ...
  /* Additive 0.9 patch window. Existing numeric values remain unchanged. */
  NXLOADER_STATE_PATCHING = 10
} nxloader_state;
 ...
 * checked ABI implementation. WRITE stores *value and is an explicit
 * resolution even without a default. SKIP leaves the target untouched only
 * when a checked default already exists; it cannot hide an unresolved strong
 * import. REJECT fails the phase before any relocation is committed.
...
enum nxloader_config_flags {
  ...
  /* Opt-in, default-off: after the segments are placed, re-map the read-only
   * executable ones straight from the guest file with MAP_PRIVATE, the way a
   * dynamic linker does.  The bytes are identical -- only the accounting
   * changes: those pages become clean page cache the kernel may evict instead
   * of dirty anonymous memory, and a later write still copies on write.
   *
   * It only ever applies to whole pages that lie entirely inside the file and
   * whose VMA and file offset are page-congruent, which
   * nxloader_file_mapping_matches() proves; anything else keeps the copy.
   * Requires nxloader_module_load_file(): loading from memory has no file to
   * map, and the flag is then simply inert.
   *
   * Trade-off worth stating: clean pages are re-read from the file on demand,
   * so the guest file must stay unchanged while the module is loaded.  That is
   * the same contract the system linker has always had. */
  NXLOADER_CONFIG_FILE_BACKED_TEXT = 1u << 3,
  /* Opt-in, default-off: allow the AArch64 hook emitter to allocate auxiliary
   * trampoline pools near the target when the module's own pool is beyond the
   * reach of a direct branch.
   *
   * The pool sits at the end of the image, and an AArch64 `B` reaches +-128 MiB.
   * In a large guest -- the one measured here has 330 MiB of text alone -- that
   * leaves only the last stretch of the module hookable, which is an accident of
   * where the pool landed rather than a deliberate limit.
   *
   * With the flag on, the emitter first tries the module's own pool (so the
   * common case is unchanged), then any auxiliary pool already in range, and
   * only then maps a fresh page next to the target. Auxiliary pools are R+X and
   * are made writable only while a slot is filled; they are unmapped with the
   * image. If no page can be placed in range the hook is still refused -- the
   * emitter never produces a branch that does not reach.
   *
   * Default-off because the refusals are a documented contract: without this
   * flag an exhausted pool still fails with NXLOADER_EBOUNDS and an
   * out-of-reach target still fails with NXLOADER_EOVERFLOW. */
  NXLOADER_CONFIG_FAR_HOOKS = 1u << 4
};
...
/* Reopen only a FINALIZED module for a deliberate entry/veneer rewrite.
 * Every patchable executable image page becomes RW (never RWX), the mapping
 * and runtime addresses stay unchanged, and the module enters PATCHING. The caller may
 * then use nxloader_module_install_hook() and must call finalize() again before
 * executing any guest address. The trusted adapter must first quiesce every
 * thread that could execute this module and keep it quiesced through the next
 * successful finalize(); nxloader cannot stop guest-created threads itself.
 * INITIALIZED/READY modules are never reopenable. RELRO/non-executable pages
 * retain their final permissions; auxiliary far-hook pools stay RX and are
 * reopened internally only for their bounded slot write, never as RWX. */
...
/* Optional function-entry patching. Valid after resolve or inside the explicit
 * PATCHING window; never valid while executable pages are finalized.
 * `available_bytes` makes the overwrite contract explicit (4 for AArch64,
 * 8 for ARM/Thumb). */
