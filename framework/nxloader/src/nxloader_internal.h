...
struct nxloader_module {
  ...
  void *mapping;
  size_t mapping_size;
  /* Descritor do arquivo de origem enquanto load_file esta' em curso; -1 em
   * qualquer outro caso.  Serve so' ao mapeamento file-backed opcional. */
  int source_fd;
  size_t image_size;
  size_t page_size;
  ...
  /* V4-GRAPHICS-03 fail-closed: how many STRONG imports were left undefined by
   * the last resolve.  Non-zero means the guest still carries raw link-time
   * values in its GOT/PLT slots, which is exactly the state that crashed the
   * OTR 1.0.3 guest on ROCKNIX after its constructors ran anyway.  No guest
   * address may be executed while this is non-zero. */
  size_t unresolved_strong;

  uint8_t *trampoline_pool;
  size_t trampoline_pool_size;
  size_t trampoline_pool_used;
  /* Pools AUXILIARES, criados sob demanda quando o pool principal esta' fora do
   * alcance do desvio direto. Ver nxloader_hooks.c. */
#define NXLOADER_AUX_POOL_MAX 8
  struct {
    uint8_t *base;
    size_t size;
    size_t used;
  } aux_pool[NXLOADER_AUX_POOL_MAX];
  size_t aux_pool_count;
};
