...
static nxloader_result nxloader_aarch64_validate_relocation_target(
  ...
  /* Android's AArch64 linker accepts 64-bit dynamic relocations in packed
   * data whose destination is not naturally aligned. The transaction layer
   * commits through memcpy-based nxloader_write_u64(), so these destinations
   * remain safe under the range, metadata and overlap checks below. */
  if (!segment)
  ...
  for (index = 0; index < entry_count && entries[index].d_tag != DT_NULL;
    ...
    case DT_ANDROID_RELASZ:
      return nxloader_fail(module, NXLOADER_EUNSUPPORTED,
                           "Android packed relocations (DT_ANDROID_REL*) are "
                           "unsupported; relink without --pack-dyn-relocs=android");
    case DT_RELR:
    case DT_RELRSZ:
    case DT_RELRENT:
      return nxloader_fail(module, NXLOADER_EUNSUPPORTED,
                           "DT_RELR packed relocations (NDK r25+/lld default) "
                           "are unsupported by this loader build; relink with "
                           "--pack-dyn-relocs=none or use a so_util-based "
                           "loader with the proven RELR decoder");
    default: break;
    }
  }
