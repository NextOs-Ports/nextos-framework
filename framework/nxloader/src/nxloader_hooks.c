...
static nxloader_result nxloader_finish_trampoline_slot(
    nxloader_module *module, uint8_t *slot) {
  for (size_t i = 0; i < module->aux_pool_count; ++i) {
    uint8_t *base = module->aux_pool[i].base;
    if (slot < base || slot >= base + module->aux_pool[i].size)
      continue;
    return nxloader_close_auxiliary_pool(module, i);
  }
  return NXLOADER_OK;
}
...
static nxloader_result nxloader_install_aarch64_hook(nxloader_module *module,
  ...
  result = nxloader_finish_trampoline_slot(module, slot);
...
nxloader_result nxloader_module_install_hook(nxloader_module *module,
  ...
  if (module->state == NXLOADER_STATE_PATCHING &&
      nxloader_pointer_overlaps_relro(module, target, available_bytes))
