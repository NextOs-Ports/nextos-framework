  ...
  *out_module = NULL;
  ...
  if (config->max_file_size == 0 || config->max_image_size == 0 ||
      (config->flags & ~(NXLOADER_CONFIG_ALLOW_FOREIGN_ARCH |
                         ...
                         NXLOADER_CONFIG_FILE_BACKED_TEXT |
                         NXLOADER_CONFIG_FAR_HOOKS)))
    return NXLOADER_EINVAL;
...
void nxloader_release_image(nxloader_module *module) {
  ...
  for (size_t i = 0; i < module->aux_pool_count; ++i)
    if (module->aux_pool[i].base && module->aux_pool[i].size)
      (void)munmap(module->aux_pool[i].base, module->aux_pool[i].size);
  module->aux_pool_count = 0;
  memset(module->aux_pool, 0, sizeof(module->aux_pool));
...
nxloader_result nxloader_module_load_file(nxloader_module *module,
  ...
  /* O descritor fica aberto durante o load_memory: e' dele que sai o
   * mapeamento file-backed opcional.  Fechamos logo em seguida -- um mapeamento
   * ja' criado sobrevive ao fechamento do descritor. */
  module->source_fd = descriptor;
...
/* Troca a copia anonima dos segmentos R+X por um mapeamento privado do proprio
 * arquivo, quando o dono pediu NXLOADER_CONFIG_FILE_BACKED_TEXT.
 *
 * Roda DEPOIS da copia, de proposito: assim os casos que nao se qualificam
 * (pagina de borda, segmento gravavel, arquivo curto) ja' estao corretos e esta
 * funcao so' precisa tratar do miolo que se qualifica.  Os bytes sao os mesmos
 * dos dois lados, entao um remapeamento que falhe nao muda comportamento
 * nenhum -- e' otimizacao pura, e por isso ela nunca devolve erro.
 *
 * Regra de elegibilidade, deliberadamente estreita:
 *   - o segmento e' R+X, sem PF_W (uma pagina gravavel seria suja de qualquer
 *     jeito, e mapea-la do arquivo nao ganharia nada);
 *   - a faixa cabe inteira dentro do arquivo (nada de BSS);
 *   - VMA e offset de arquivo sao congruentes modulo pagina, o que
 *     nxloader_file_mapping_matches() prova.
 * Fora disso, a copia fica. */
static void nxloader_file_back_text(nxloader_module *module) {
  size_t index;
  size_t page;
  if (!module || module->source_fd < 0 || !module->mapping)
    return;
  page = module->page_size;
  if (!page || !nxloader_is_power_of_two(page))
    return;
  for (index = 0; index < module->segment_count; ++index) {
    const nxloader_segment *segment = &module->segments[index];
    uint64_t first;
    ...
    uint64_t length;
    void *destination;
    if ((segment->flags & (PF_R | PF_W | PF_X)) != (uint32_t)(PF_R | PF_X))
      continue;
    if (!segment->file_size)
      continue;
    /* So' paginas INTEIRAS: as bordas parciais ficam com a copia, senao o
     * mapeamento zeraria o resto da pagina que pertence ao vizinho. */
    if (!nxloader_align_up_u64(segment->vma, page, &first))
      continue;
    last_end = nxloader_align_down_u64(segment->vma + segment->file_size, page);
    if (last_end <= first)
      continue;
    length = last_end - first;
    if (length > SIZE_MAX)
      continue;
    offset = segment->file_offset + (first - segment->vma);
    if (!nxloader_file_mapping_matches(module, first, offset, length))
      continue;
    if ((offset & (uint64_t)(page - 1)) != 0)
      continue;
    destination = nxloader_vma_pointer(module, first, (size_t)length);
    if (!destination)
      continue;
    /* PROT_READ|PROT_WRITE porque as relocacoes ainda nao correram e o
     * nxloader_protect aplica a protecao final depois; MAP_PRIVATE garante que
     * qualquer escrita copie na hora, sem tocar no arquivo do dono. */
    if (mmap(destination, (size_t)length, PROT_READ | PROT_WRITE,
             MAP_PRIVATE | MAP_FIXED, module->source_fd,
             (off_t)offset) == MAP_FAILED) {
      /* A copia continua la' e esta' correta: seguimos sem mapear. */
      (void)nxloader_log(module, NXLOADER_LOG_INFO,
                         "file-backed text declined for %s: %s",
                         module->debug_name, strerror(errno));
      continue;
    }
    (void)nxloader_log(module, NXLOADER_LOG_INFO,
                       "file-backed text: %llu bytes of %s mapped from the file",
                       (unsigned long long)length, module->debug_name);
  }
}
...
nxloader_result nxloader_allocate_image(nxloader_module *module,
  ...
  if (module->config.flags & NXLOADER_CONFIG_FILE_BACKED_TEXT)
    nxloader_file_back_text(module);
  if (trampoline_size) {
...
nxloader_result nxloader_apply_relocation_hook(
  ...
  *write_value = default_value;
  ...
  case NXLOADER_RELOC_SKIP:
    *should_write = 0;
    /* SKIP is only meaningful when the checked backend/provider path already
     * produced a value.  Without that default, accepting SKIP would make an
     * unresolved strong JUMP_SLOT look resolved while leaving its PLT0
     * sentinel callable.  The import backends translate ERELOC with no
     * default into the normal unresolved-strong report/fail-closed path. */
    return has_default ? NXLOADER_OK : NXLOADER_ERELOC;
...
nxloader_result nxloader_module_resolve(
  ...
  /* The count is needed even when the caller passed no report, because
   * call_initializers/call_jni_onload refuse to execute guest code while any
   * strong import is still undefined. */
  if (!active_report) {
    memset(&local_report, 0, sizeof(local_report));
    local_report.struct_size = sizeof(local_report);
    active_report = &local_report;
  ...
  module->unresolved_strong = local_report.unresolved_strong;
...
nxloader_result nxloader_module_finalize(nxloader_module *module) {
  ...
  if (module->state != NXLOADER_STATE_RESOLVED &&
      module->state != NXLOADER_STATE_PATCHING)
    return NXLOADER_ESTATE;
...
nxloader_result nxloader_module_begin_patch(nxloader_module *module) {
  nxloader_result result;
  int final_protections_restored = 1;
  if (!module)
    return NXLOADER_EINVAL;
  if (nxloader_module_callback_guard(module) != NXLOADER_OK)
    return NXLOADER_EREENTRANT;
  if (module->state != NXLOADER_STATE_FINALIZED)
    return NXLOADER_ESTATE;
  result = nxloader_prepare_image_patch(module, &final_protections_restored);
  if (result == NXLOADER_OK)
    module->state = NXLOADER_STATE_PATCHING;
  else if (!final_protections_restored)
    module->state = NXLOADER_STATE_ERROR;
  return result;
}
...
nxloader_result nxloader_module_call_initializers(nxloader_module *module) {
  ...
  if (module->state != NXLOADER_STATE_FINALIZED)
    return NXLOADER_ESTATE;
  /* V4-GRAPHICS-03: never run a constructor over an unresolved GOT/PLT slot.
   * ALLOW_UNRESOLVED is an inspection aid for bring-up, not permission to
   * execute guest code. */
  if (module->unresolved_strong)
...
const char *nxloader_state_string(nxloader_state state) {
  ...
  case NXLOADER_STATE_PATCHING: return "patching";
