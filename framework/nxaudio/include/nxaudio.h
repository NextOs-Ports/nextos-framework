...
typedef enum nxaudio_reason {
  ...
  NXAUDIO_REASON_EXPLICIT_BACKEND_PRESERVED = 16,
  NXAUDIO_REASON_BACKEND_FAILURE_UNPROVEN = 17,
  NXAUDIO_REASON_FALLBACK_UNAVAILABLE = 18,
  NXAUDIO_REASON_FALLBACK_SAME_BACKEND = 19,
  NXAUDIO_REASON_RETRY_EXHAUSTED = 20,
  NXAUDIO_REASON_BACKEND_RETRY = 21,
  /* Onda v2 (aditivo): o device concedeu formato diferente do pedido. */
  NXAUDIO_REASON_FORMAT_MATCH = 22,
  NXAUDIO_REASON_RATE_CHANGED = 23,
  NXAUDIO_REASON_CHANNELS_CHANGED = 24,
  NXAUDIO_REASON_SAMPLE_FORMAT_CHANGED = 25
} nxaudio_reason;
...
/* A retry is an adapter-owned action. This policy only validates a candidate
 * supplied by the adapter; it never reads the environment, discovers a
 * firmware or opens an audio device. `backend_explicit` covers an explicit
 * API/config selection and `environment_explicit` covers an inherited backend
 * environment variable. Either one preserves the current selection. */
typedef struct nxaudio_backend_retry_request {
  uint32_t api_version;
  size_t struct_size;
  const char *current_backend;
  const char *fallback_backend;
  nxaudio_reason failure_reason;
  uint32_t retry_count;
  int backend_explicit;
  int environment_explicit;
  int fallback_available;
  int fallback_compiled;
} nxaudio_backend_retry_request;

typedef struct nxaudio_backend_retry_plan {
  int retry_allowed;
  uint32_t next_retry_count;
  char backend[NXAUDIO_NAME_MAX];
  nxaudio_reason reason;
} nxaudio_backend_retry_plan;
...
/* Onda v2: classifica pedido vs CONCEDIDO. Um shim que sobrescreve as
 * proprias variaveis com o formato do device (44100/mono pedido virando
 * 48000/estereo) toca em pitch errado sem nenhum erro -- aqui a divergencia
 * ganha nome e recibo. Puro; nao toca no stream. Devolve NXAUDIO_OK com
 * reason=FORMAT_MATCH quando iguais; NXAUDIO_UNSUPPORTED com a primeira
 * divergencia (taxa > canais > sample) quando nao. */
