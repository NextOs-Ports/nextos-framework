...
struct nxaudio_stream {
  ...
  /* Tamanho REAL do anel, arredondado para potencia de dois: com contadores
   * livres de 32 bits, "% capacidade" salta descontinuo no wrap de 2^32
   * quando a capacidade nao divide 2^32 -- corrupcao de audio deterministica
   * apos ~25h continuas. Mascara resolve para qualquer capacidade pedida. */
...
static const nxaudio_adapter_contract nxaudio_contracts[] = {
    ...
    {NXAUDIO_STACK_FMOD_EX, "titansouls-fmodex-opensl-sdl-v1",
     "titansouls", 0, 1},
    {NXAUDIO_STACK_FMOD_EX, "titansouls-fmodex-opensl-sdl-v2",
     "titansouls", 0, 1},
...
nxaudio_result nxaudio_stream_create(const nxaudio_stream_options *options,
  ...
  *out_stream = NULL;
  ...
  stream->ring_frames = 1u;
  while (stream->ring_frames < options->capacity_frames)
    stream->ring_frames <<= 1;
  bytes = (size_t)stream->ring_frames * stream->frame_bytes;
...
nxaudio_result nxaudio_worker_submit(nxaudio_stream *stream, const void *pcm,
  ...
  if (frames == 0u)
    return NXAUDIO_FULL;
  first = stream->ring_frames - (write_pos & (stream->ring_frames - 1u));
  if (first > frames)
    first = frames;
  memcpy(stream->ring + (size_t)(write_pos & (stream->ring_frames - 1u)) *
                            stream->frame_bytes,
...
nxaudio_result nxaudio_realtime_pull(nxaudio_stream *stream, void *pcm,
  ...
  first = stream->ring_frames - (read_pos & (stream->ring_frames - 1u));
  if (first > copied)
    first = copied;
  memcpy(pcm,
         stream->ring + (size_t)(read_pos & (stream->ring_frames - 1u)) *
                            stream->frame_bytes,
...
void nxaudio_stream_close(nxaudio_stream **stream_ptr) {
  ...
  *stream_ptr = NULL;
  ...
  memset(stream->ring, 0,
         (size_t)stream->ring_frames * stream->frame_bytes);
  free(stream->ring);
  free(stream);
}
...
static int nxaudio_real_backend_failure(nxaudio_reason reason) {
  return reason == NXAUDIO_REASON_SERVER_UNAVAILABLE ||
         reason == NXAUDIO_REASON_DEVICE_OPEN_FAILED ||
         reason == NXAUDIO_REASON_DEVICE_LOST;
}

nxaudio_result nxaudio_plan_backend_retry(
    const nxaudio_backend_retry_request *request,
    nxaudio_backend_retry_plan *plan) {
  size_t fallback_length;
  if (!request || !plan || request->api_version != NXAUDIO_API_VERSION ||
      request->struct_size < sizeof(*request) ||
      !nxaudio_bounded_string(request->current_backend, NXAUDIO_NAME_MAX) ||
      !nxaudio_bounded_string(request->fallback_backend, NXAUDIO_NAME_MAX) ||
      request->current_backend[0] == '\0' ||
      request->fallback_backend[0] == '\0' ||
      request->failure_reason < NXAUDIO_REASON_NONE ||
      request->failure_reason > NXAUDIO_REASON_BACKEND_RETRY ||
      !nxaudio_bool_valid(request->backend_explicit) ||
      !nxaudio_bool_valid(request->environment_explicit) ||
      !nxaudio_bool_valid(request->fallback_available) ||
      !nxaudio_bool_valid(request->fallback_compiled))
    return NXAUDIO_INVALID;

  memset(plan, 0, sizeof(*plan));
  plan->next_retry_count = request->retry_count;

  if (nxaudio_name_equals(request->current_backend, "dummy") ||
      nxaudio_name_equals(request->current_backend, "disk") ||
      nxaudio_name_equals(request->fallback_backend, "dummy") ||
      nxaudio_name_equals(request->fallback_backend, "disk")) {
    plan->reason = NXAUDIO_REASON_FAKE_BACKEND;
    return NXAUDIO_UNSUPPORTED;
  }
  if (request->backend_explicit || request->environment_explicit) {
    plan->reason = NXAUDIO_REASON_EXPLICIT_BACKEND_PRESERVED;
    return NXAUDIO_UNSUPPORTED;
  }
  if (!nxaudio_real_backend_failure(request->failure_reason)) {
    plan->reason = NXAUDIO_REASON_BACKEND_FAILURE_UNPROVEN;
    return NXAUDIO_UNSUPPORTED;
  }
  if (request->retry_count != 0u) {
    plan->reason = NXAUDIO_REASON_RETRY_EXHAUSTED;
    return NXAUDIO_UNSUPPORTED;
  }
  if (!request->fallback_available || !request->fallback_compiled) {
    plan->reason = NXAUDIO_REASON_FALLBACK_UNAVAILABLE;
    return NXAUDIO_UNSUPPORTED;
  }
  if (nxaudio_name_equals(request->current_backend,
                          request->fallback_backend)) {
    plan->reason = NXAUDIO_REASON_FALLBACK_SAME_BACKEND;
    return NXAUDIO_UNSUPPORTED;
  }

  fallback_length = strlen(request->fallback_backend);
  memcpy(plan->backend, request->fallback_backend, fallback_length + 1u);
  plan->retry_allowed = 1;
  plan->next_retry_count = 1u;
  plan->reason = NXAUDIO_REASON_BACKEND_RETRY;
  return NXAUDIO_OK;
}
...
nxaudio_result nxaudio_classify_granted_format(const nxaudio_format *requested,
                                               const nxaudio_format *granted,
                                               nxaudio_reason *reason) {
  if (!requested || !granted || !reason)
    return NXAUDIO_INVALID;
  *reason = NXAUDIO_REASON_NONE;
  if (requested->frequency == 0u || granted->frequency == 0u ||
      requested->channels == 0u || granted->channels == 0u)
    return NXAUDIO_INVALID;
  if (requested->frequency != granted->frequency) {
    *reason = NXAUDIO_REASON_RATE_CHANGED;
    return NXAUDIO_UNSUPPORTED;
  }
  if (requested->channels != granted->channels) {
    *reason = NXAUDIO_REASON_CHANNELS_CHANGED;
    return NXAUDIO_UNSUPPORTED;
  }
  if (requested->sample_format != granted->sample_format) {
    *reason = NXAUDIO_REASON_SAMPLE_FORMAT_CHANGED;
    return NXAUDIO_UNSUPPORTED;
  }
  *reason = NXAUDIO_REASON_FORMAT_MATCH;
  return NXAUDIO_OK;
}
...
const char *nxaudio_reason_name(nxaudio_reason reason) {
  static const char *const names[] = {
      ...
      "explicit-backend-preserved", "backend-failure-unproven",
      "fallback-unavailable", "fallback-same-backend", "retry-exhausted",
      "backend-retry",
      "format-match", "rate-changed", "channels-changed",
      "sample-format-changed"};
  return reason >= NXAUDIO_REASON_NONE &&
                 reason <= NXAUDIO_REASON_SAMPLE_FORMAT_CHANGED
             ? names[reason]
             : "unknown";
}
