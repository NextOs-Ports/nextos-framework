...
int rcr_log_level = 0;   /* RCR_LOGCAT=1 mirrors the game's own log */
...
{
    fprintf(stderr, "[rcr] abort message: %s\n", m ? m : "(null)");
}
...
/* RCR_FILELOG: every file the game opens.  Its own switch, because it fires
 * often enough that leaving it on RCR_VERBOSE would cost frames. */
int rcr_trace_files;
...
static int my_fclose(void *f)               { return f == (void *)rcr_sF ? 0 : fclose(xf(f)); }
...
static const unsigned char rcr_ctype_tab[1 + 256] = { 0 };
static const unsigned char *rcr_ctype_ptr = rcr_ctype_tab + 1;
...
static char *my_getenv(const char *name)
{
    if (!name)
        return NULL;
    /* Isolate guest SDL from host desktop drivers (Wayland, KMSDRM, Pulse/ALSA) */
    if (strcmp(name, "SDL_VIDEODRIVER") == 0 ||
        strcmp(name, "SDL_AUDIODRIVER") == 0)
        return NULL;
    return getenv(name);
}
...
extern void *rcr_android_sym(const char *name);
extern void *rcr_egl_sym(const char *name);
extern void *rcr_jni_sym(const char *name);
 ...
 */
#define RCR_KEYS_MAX 64
static unsigned guest_keys[RCR_KEYS_MAX];
 ...
 * Diagnostico: so fala com RCR_VERBOSE=1 (nx_log ja e mudo sem ele). */
static void sleep_note(void *caller, long ms)
{
    ...
    for (int i = 0; i < 24 && tab[i].caller; i++) {
        ...
        static const char *known[] = { "libRCRDX.so" };
...
static __thread uintptr_t rcr_delay_caller;
uintptr_t *rcr_delay_caller_slot(void) { return &rcr_delay_caller; }
...
int my_nanosleep(const struct timespec *req, struct timespec *rem)
{
     ...
     * rcr_probe_on -- antes ele custava, POR CHAMADA, uma busca de modulo por
    ...
    if (rcr_probe_on && req) {
        ...
        uint32_t bucket = RCR_SLEEP_OUTRO;
        if (lime) {
            uintptr_t rva = (uintptr_t)((uint8_t *)caller - lime->base);
            if (rva == 0x4c54bc)
                bucket = RCR_SLEEP_WAITEVENT;
            else if (rva == 0x48cbf8)
                bucket = RCR_SLEEP_TIMER;
        }
        rcr_probe_note(RCR_PROBE_SLEEP, (uint32_t)ms, bucket);
...
static uintptr_t rcr_stack_chk_guard = 0x00000aff;
...
/* The C++ runtime the game imports from libstdc++.so.  Only the four global
 * allocation operators are referenced -- no exceptions, no RTTI helpers -- and
 * on this ABI they are exactly malloc/free with a non-null guarantee. */
/* Who ends the process, and from where.  The game exits through libc's exit()
 * on its own error paths, and without this the death is silent: exit status
 * 255 with no last line.  Printing the return address places the call site in
 * the game's own binary. */
static void my_exit(int code)
{
    void *caller = __builtin_return_address(0);
    void *base = NULL;
    const char *mod = rcr_mod_at(caller, &base);
    fprintf(stderr, "[rcr] exit(%d) chamado de %s+%#lx\n", code,
            mod ? mod : "?",
            mod ? (unsigned long)((uint8_t *)caller - (uint8_t *)base) : 0);
    fflush(NULL);
    exit(code);
}
...
static void my_abort(void)
{
    ...
    void *base = NULL;
    const char *mod = rcr_mod_at(caller, &base);
    fprintf(stderr, "[rcr] abort() chamado de %s+%#lx\n", mod ? mod : "?",
            mod ? (unsigned long)((uint8_t *)caller - (uint8_t *)base) : 0);
    fflush(NULL);
    abort();
}

static void *my__Znwm(size_t n)
{
    void *p = malloc(n ? n : 1);
    if (!p)
        nx_die("operator new(%zu) out of memory", n);
    return p;
}
static void my__ZdlPv(void *p) { free(p); }

/* 🚨 O sysroot atual do NextOS traz sinh/cosh reversionados em GLIBC_2.44, e o
 * device roda 2.43: sem pinar, o binario nem carrega ("version not found").
 * As implementacoes 2.17 estao la e sao as mesmas funcoes; pinar a versao
 * antiga e o remedio de sempre para simbolo reversionado. */
__asm__(".symver sinh,sinh@GLIBC_2.17");
__asm__(".symver cosh,cosh@GLIBC_2.17");
...
static nx_import tab[] = {
    ...
    A("__sF", rcr_sF), A("_ctype_", &rcr_ctype_ptr),
    A("__stack_chk_guard", &rcr_stack_chk_guard),
    A("environ", &environ),
    ...
    M(abort), M(exit), E(_exit), E(atexit), M(getenv), E(setenv), E(putenv),
    ...
    { "_Znwm", (void *)(uintptr_t)my__Znwm },
    { "_Znam", (void *)(uintptr_t)my__Znwm },
    { "_ZdlPv", (void *)(uintptr_t)my__ZdlPv },
    { "_ZdaPv", (void *)(uintptr_t)my__ZdlPv },
    { "_ZdlPvm", (void *)(uintptr_t)my__ZdlPv },
    { "_ZdaPvm", (void *)(uintptr_t)my__ZdlPv },
...
static void *my_dlsym(void *h, const char *sym)
{
    ...
    if (!a) a = rcr_android_sym(sym);
    if (!a) a = rcr_egl_sym(sym);
    if (!a) a = rcr_jni_sym(sym);
...
static int my_dladdr(const void *addr, void *info)
{
    ...
    extern const char *rcr_mod_at(const void *addr, void **base_out);
    void *base = NULL;
    const char *name = rcr_mod_at(addr, &base);
...
static int my_dl_iterate_phdr(int (*cb)(void *, size_t, void *), void *data)
{
    ...
    extern int rcr_iterate_mods(int (*cb)(void *, size_t, void *), void *data);
    return rcr_iterate_mods(cb, data);
}
...
size_t rcr_bionic_count(void) { return sizeof tab / sizeof *tab; }

nx_import *rcr_bionic_entries(size_t *n)
{
