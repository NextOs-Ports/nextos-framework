...
#define BRIDGE_MAGIC 0x54523242u   /* "TR2B" */
 ...
 * on end is the "cond_wait with no predicate" failure this project has met
 * before: the wakeup is lost, or the thread that should signal was never
 * started.  Say so, with the object and the call site, instead of leaving a
 * frozen game with a silent log.  Costs one comparison per wait. */
static void note_long_wait(void *o, void *caller)
{
    static __thread void *last_obj;
    static __thread struct timespec since;
    static __thread int reported;
    struct timespec now;
    ...
    if (reported)
        return;
    long ms = (now.tv_sec - since.tv_sec) * 1000 +
    ...
    nx_log("[COND] tid %d has been waiting %ld ms on cond %p (caller %s+%#lx)",
           (int)syscall(SYS_gettid), ms, o, mod ? mod : "?",
           mod ? (unsigned long)((uint8_t *)caller - (uint8_t *)base) : 0UL);
}

static int b_cond_wait(void *o, void *m)
{
    note_long_wait(o, __builtin_return_address(0));
    bcond *b = C(o);
    struct timespec ts;
...
const nx_import *rcr_pthread_table(size_t *n)
{
