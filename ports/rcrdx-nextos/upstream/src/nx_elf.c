...
int nx_protect(nx_mod *m)
{
    ...
    for (size_t i = 0; i < m->phnum; i++) {
        ...
        if (p->p_type != PT_LOAD || !p->p_memsz)
            continue;
        uintptr_t start = page_down(p->p_vaddr);
        size_t len = page_up(p->p_vaddr + p->p_memsz) - start;
        if (mprotect(m->base + start, len, prot_of(p->p_flags)) != 0) {
            nx_log("%s: mprotect seg %zu: %s", m->name, i, strerror(errno));
            failed++;
        }
    }
    nx_log("%s: segment protections restored (%d failed)", m->name, failed);
    return failed;
}
