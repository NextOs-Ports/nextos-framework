 ...
 *
 * The order is fixed by the JNI specification, so these double as a check on
 * the port: libpairipcore's ExecuteProgram loads [JNIEnv + 1336], which must be
 * slot 167 (NewStringUTF).  jni.c asserts that at start-up.
 */

#ifndef RCR_JNI_SLOTS_H
#define RCR_JNI_SLOTS_H
