/*
 * egl_sdl.c -- capability-selected SDL-owned EGL facade.
 *
 ...
 * final page flip.  Selection is based only on whether SDL can create and
 * validate a GLES2 window/context.  If that probe fails, egl.c tries the
 * engine's raw EGL path; firmware, device and driver names are never policy.
 */
...
static void dt_video_log(const char *format, ...)
{
    va_list arguments;
    va_start(arguments, format);
    fprintf(stderr, "[deadtrigger/video] ");
    vfprintf(stderr, format, arguments);
    fputc('\n', stderr);
    va_end(arguments);
}

static int video_trace_enabled(void)
{
    const char *value = getenv("DT_VIDEO_TRACE");
...
static _Thread_local dt_sdl_context *current_context;
...
{
    int width = positive_env("DT_SCREEN_W");
    int height = positive_env("DT_SCREEN_H");
    ...
    fprintf(stderr, "[deadtrigger/video] resolution %dx%d (%s)\n",
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Dead Trigger", SDL_WINDOWPOS_CENTERED,
            SDL_WINDOWPOS_CENTERED, screen_width, screen_height,
            SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN_DESKTOP);
        if (!video_window) {
            fprintf(stderr,
                    "[deadtrigger/video] SDL window depth%d/stencil%d failed: %s\n",
        ...
        if (share_root && !context_is_gles()) {
            fprintf(stderr,
                    "[deadtrigger/video] SDL returned desktop GL; rejecting it\n");
        ...
        if (share_root) {
            ...
            alpha_size = a;
            fprintf(stderr,
                    "[deadtrigger/video] SDL config: R%dG%dB%dA%d depth%d stencil%d\n",
                    r, g, b, a, depth_size, stencil_size);
            break;
        }

        fprintf(stderr,
                "[deadtrigger/video] GLES2 depth%d/stencil%d failed: %s\n",
                formats[i].depth, formats[i].stencil, SDL_GetError());
    ...
    const GLubyte *(*get_string)(GLenum) =
        (const GLubyte *(*)(GLenum))SDL_GL_GetProcAddress("glGetString");
    fprintf(stderr,
            "[deadtrigger/video] SDL GLES2 depth%d/stencil%d drawable=%dx%d\n",
            depth_size, stencil_size, screen_width, screen_height);
    if (get_string) {
        fprintf(stderr, "[deadtrigger/video] GL_VENDOR=%s\n",
                get_string(GL_VENDOR) ?
                    (const char *)get_string(GL_VENDOR) : "?");
        fprintf(stderr, "[deadtrigger/video] GL_RENDERER=%s\n",
                get_string(GL_RENDERER) ?
                    (const char *)get_string(GL_RENDERER) : "?");
        fprintf(stderr, "[deadtrigger/video] GL_VERSION=%s\n",
...
int dt_sdl_video_init(void)
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        ...
        fprintf(stderr,
                "[deadtrigger/video] SDL video unavailable (%s); using raw EGL\n",
                SDL_GetError());
        return 0;
    }
    ...
    if (create_window()) {
        ...
        fprintf(stderr,
                "[deadtrigger/video] SDL GLES2 capability accepted (driver=%s)\n",
    ...
    video_mode = VIDEO_RAW_EGL;
    fprintf(stderr,
            "[deadtrigger/video] SDL GLES2 capability unavailable (%s); "
            "trying raw EGL\n",
            SDL_GetError());
    if (!was_initialized)
...
{
    dt_sdl_surface *surface = calloc(1, sizeof *surface);
    if (!surface)
                                              ...
                                              const EGLint *attributes)
{
    ...
    (void)window;
    if (video_trace_enabled())
        dt_video_log("eglCreateWindowSurface begin");
    EGLSurface surface = make_surface(1, attributes);
    dt_video_log("SDL EGL window surface -> %p", (void *)surface);
    return surface;
}
                                               ...
                                               const EGLint *attributes)
{
    ...
    (void)config;
    if (video_trace_enabled())
        dt_video_log("eglCreatePbufferSurface");
...
{
    (void)display;
    dt_sdl_surface *surface = (dt_sdl_surface *)handle;
                                       ...
                                       const EGLint *attributes)
{
    ...
    (void)attributes;
    if (video_trace_enabled())
        dt_video_log("eglCreateContext begin share=%p", (void *)share);
    dt_sdl_context *context = calloc(1, sizeof *context);
    if (!context)
        return EGL_NO_CONTEXT;

    pthread_mutex_lock(&context_lock);
    if (video_trace_enabled())
        dt_video_log("eglCreateContext acquired host lock");
    set_context_attributes(depth_size, stencil_size, alpha_size);
    SDL_GL_SetAttribute(SDL_GL_SHARE_WITH_CURRENT_CONTEXT, 1);
    SDL_GL_MakeCurrent(video_window, share_root);
    if (video_trace_enabled())
        dt_video_log("eglCreateContext share root current");
    context->context = SDL_GL_CreateContext(video_window);
    if (video_trace_enabled())
        dt_video_log("eglCreateContext SDL result=%p", context->context);
    SDL_GL_SetAttribute(SDL_GL_SHARE_WITH_CURRENT_CONTEXT, 0);
    SDL_GL_MakeCurrent(video_window, NULL);
    pthread_mutex_unlock(&context_lock);

    if (!context->context) {
        dt_video_log("SDL EGL context creation failed: %s", SDL_GetError());
        free(context);
        return EGL_NO_CONTEXT;
    }
    context->magic = CONTEXT_MAGIC;
    context->id = next_context_id++;
    dt_video_log("SDL EGL context #%u -> %p", context->id, (void *)context);
...
{
    (void)display;
    dt_sdl_context *context = (dt_sdl_context *)handle;
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    (void)display;
    if (video_trace_enabled())
        dt_video_log("eglMakeCurrent draw=%p context=%p",
    ...
    dt_sdl_context *context = (dt_sdl_context *)handle;
    dt_sdl_surface *surface = (dt_sdl_surface *)draw;
    if (!context || context->magic != CONTEXT_MAGIC ||
    ...
    if (SDL_GL_MakeCurrent(video_window, context->context) != 0) {
        dt_video_log("SDL EGL MakeCurrent context #%u failed: %s",
               context->id, SDL_GetError());
        return EGL_FALSE;
    }
    current_context = context;
    if (publish_current_graphics() != 0) {
        dt_video_log("SDL EGL current context failed framework publication");
        SDL_GL_MakeCurrent(video_window, NULL);
        current_context = NULL;
        return EGL_FALSE;
    }
...
EGLBoolean dt_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    ...
    if (frame_count <= 3 || frame_count % 600 == 0)
        dt_video_log("SDL page flip #%u", frame_count);
    return EGL_TRUE;
}
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Dead Trigger SDL EGL";
...
static void *sdl_eglGetProcAddress(const char *name)
{
    void *address;

    if (!name)
        return NULL;
    address = dt_sdl_egl_proc(name);
    if (address)
        return address;
