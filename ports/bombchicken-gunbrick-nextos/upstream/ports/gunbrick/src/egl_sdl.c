...
static _Thread_local gb_sdl_context *current_context;
...
static int publish_current_graphics(void)
{
    ...
    return gb_framework_publish_graphics(&evidence);
}
...
{
    int width = positive_env("GB_SCREEN_W");
    int height = positive_env("GB_SCREEN_H");
    ...
    fprintf(stderr, "[gunbrick/video] resolution %dx%d (%s)\n",
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Gunbrick", SDL_WINDOWPOS_CENTERED,
            SDL_WINDOWPOS_CENTERED, screen_width, screen_height,
            SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN_DESKTOP);
        if (!video_window) {
            fprintf(stderr,
                    "[gunbrick/video] SDL window depth%d/stencil%d failed: %s\n",
        ...
        if (share_root && !context_is_gles()) {
            fprintf(stderr,
                    "[gunbrick/video] SDL returned desktop GL; rejecting it\n");
        ...
        if (share_root) {
            ...
            alpha_size = a;
            fprintf(stderr,
                    "[gunbrick/video] SDL config: R%dG%dB%dA%d depth%d stencil%d\n",
                    r, g, b, a, depth_size, stencil_size);
            break;
        }

        fprintf(stderr,
                "[gunbrick/video] GLES2 depth%d/stencil%d failed: %s\n",
                formats[i].depth, formats[i].stencil, SDL_GetError());
    ...
    const GLubyte *(*get_string)(GLenum) =
        (const GLubyte *(*)(GLenum))SDL_GL_GetProcAddress("glGetString");
    fprintf(stderr,
            "[gunbrick/video] SDL GLES2 depth%d/stencil%d drawable=%dx%d\n",
            depth_size, stencil_size, screen_width, screen_height);
    if (get_string) {
        fprintf(stderr, "[gunbrick/video] GL_VENDOR=%s\n",
                get_string(GL_VENDOR) ?
                    (const char *)get_string(GL_VENDOR) : "?");
        fprintf(stderr, "[gunbrick/video] GL_RENDERER=%s\n",
                get_string(GL_RENDERER) ?
                    (const char *)get_string(GL_RENDERER) : "?");
        fprintf(stderr, "[gunbrick/video] GL_VERSION=%s\n",
...
int gb_sdl_video_init(void)
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        ...
        fprintf(stderr,
                "[gunbrick/video] SDL video unavailable (%s); using raw EGL\n",
                SDL_GetError());
        return 0;
    }
    ...
    if (create_window()) {
        ...
        fprintf(stderr,
                "[gunbrick/video] SDL GLES2 capability accepted (driver=%s)\n",
    ...
    video_mode = VIDEO_RAW_EGL;
    fprintf(stderr,
            "[gunbrick/video] SDL GLES2 capability unavailable (%s); "
            "trying raw EGL\n",
            SDL_GetError());
    if (!was_initialized)
...
{
    gb_sdl_surface *surface = calloc(1, sizeof *surface);
    if (!surface)
...
{
    (void)display;
    gb_sdl_surface *surface = (gb_sdl_surface *)handle;
                                       ...
                                       const EGLint *attributes)
{
    ...
    (void)share;
    (void)attributes;
    gb_sdl_context *context = calloc(1, sizeof *context);
    if (!context)
...
{
    (void)display;
    gb_sdl_context *context = (gb_sdl_context *)handle;
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    ...
    gb_sdl_context *context = (gb_sdl_context *)handle;
    gb_sdl_surface *surface = (gb_sdl_surface *)draw;
    if (!context || context->magic != CONTEXT_MAGIC ||
    ...
    if (publish_current_graphics() != 0) {
        nx_log("SDL EGL current context failed framework publication");
        SDL_GL_MakeCurrent(video_window, NULL);
        current_context = NULL;
        return EGL_FALSE;
    }
...
EGLBoolean gb_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    (void)display;
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Gunbrick SDL EGL";
