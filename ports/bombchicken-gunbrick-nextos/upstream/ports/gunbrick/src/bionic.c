...
int gb_log_level = 0;   /* GB_LOGCAT=1 mirrors the game's own log */
...
{
    fprintf(stderr, "[gunbrick] abort message: %s\n", m ? m : "(null)");
}
...
static int my_fclose(void *f)               { return f == (void *)gb_sF ? 0 : fclose(xf(f)); }
...
static void my___stack_chk_fail(void)
{
    ...
    void *base = NULL;
    extern const char *gb_mod_at(const void *addr, void **base_out);
    const char *module = gb_mod_at(caller, &base);
    if (module && base)
...
static const unsigned char gb_ctype_tab[1 + 256] = { 0 };
static const unsigned char *gb_ctype_ptr = gb_ctype_tab + 1;
...
extern void *gb_android_sym(const char *name);
extern void *gb_egl_sym(const char *name);
extern void *gb_jni_sym(const char *name);
...
static nx_import tab[] = {
    ...
    A("__sF", gb_sF), A("_ctype_", &gb_ctype_ptr),
...
static void *my_dlopen(const char *name, int flags)
{
    ...
    for (size_t i = 0; i < sizeof fake_libs / sizeof *fake_libs; i++)
        if (strcmp(fake_libs[i], b) == 0) {
            ...
            if (module && nx_run_init(module) != 0) {
                nx_log("dlopen(%s) rejected: initializer transaction failed",
                       name);
                return NULL;
            }
...
static int32_t guest_jni_onload(const char *soname, void *java_vm,
                                void *reserved)
{
    nx_mod *module;
    int32_t version = -1;
    ...
    if (reserved)
        return -1;
    module = nx_find_mod(soname);
    if (!module || nx_call_jni_onload(module, java_vm, &version) != 0)
        return -1;
    return version;
}

static int32_t main_jni_onload(void *java_vm, void *reserved)
{
    return guest_jni_onload("libmain.so", java_vm, reserved);
}

static int32_t unity_jni_onload(void *java_vm, void *reserved)
{
    return guest_jni_onload("libunity.so", java_vm, reserved);
}

static int32_t il2cpp_jni_onload(void *java_vm, void *reserved)
{
    return guest_jni_onload("libil2cpp.so", java_vm, reserved);
}

static void *jni_onload_wrapper(const nx_mod *module)
{
    if (!module)
        return NULL;
    if (strcmp(module->name, "libmain.so") == 0)
        return (void *)main_jni_onload;
    if (strcmp(module->name, "libunity.so") == 0)
        return (void *)unity_jni_onload;
    if (strcmp(module->name, "libil2cpp.so") == 0)
        return (void *)il2cpp_jni_onload;
    return NULL;
}
...
static void *my_dlsym(void *h, const char *sym)
{
    ...
    if (handle >= first && handle < first + fake_count) {
        ...
        if (module && strcmp(sym, "JNI_OnLoad") == 0)
            a = jni_onload_wrapper(module);
        else if (module)
            a = nx_lookup_in(module, sym);
    }
    if (!a) a = nx_resolve_import(sym);
    if (!a) a = gb_android_sym(sym);
    if (!a) a = gb_egl_sym(sym);
    if (!a) a = gb_jni_sym(sym);
...
static int my_dladdr(const void *addr, void *info)
{
    ...
    extern const char *gb_mod_at(const void *addr, void **base_out);
    void *base = NULL;
    const char *name = gb_mod_at(addr, &base);
...
static int my_dl_iterate_phdr(int (*cb)(void *, size_t, void *), void *data)
{
    ...
    extern int gb_iterate_mods(int (*cb)(void *, size_t, void *), void *data);
    return gb_iterate_mods(cb, data);
}
...
size_t gb_bionic_count(void) { return sizeof tab / sizeof *tab; }

nx_import *gb_bionic_entries(size_t *n)
{
