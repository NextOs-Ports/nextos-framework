...
static gb_window the_window = { 1, 1280, 720, 1 /* RGBA_8888 */ };
...
static void refresh_fb_size(void)
{
    ...
    if (ioctl(fd, FBIOGET_VSCREENINFO, &v) == 0 &&
        ...
        gb_input_set_screen_size((int)v.xres, (int)v.yres);
    }
    close(fd);
}

void gb_window_set_size(int w, int h)
{
    the_window.width = w;
    the_window.height = h;
    if (w > 0 && h > 0 && w < 32768 && h < 32768) {
        ...
        gb_input_set_screen_size(w, h);
    }
}

void *gb_window_handle(void) { return &the_window; }
...
{
    if (w) __atomic_fetch_add(&((gb_window *)w)->refs, 1, __ATOMIC_RELAXED);
}
static void a_window_release(void *w)
{
    if (w) __atomic_fetch_sub(&((gb_window *)w)->refs, 1, __ATOMIC_RELAXED);
}
static int32_t a_window_getWidth(void *w)  { return w ? ((gb_window *)w)->width : 0; }
static int32_t a_window_getHeight(void *w) { return w ? ((gb_window *)w)->height : 0; }
static int32_t a_window_getFormat(void *w) { return w ? ((gb_window *)w)->format : 1; }
...
typedef struct { int32_t refs; } gb_looper;
static __thread gb_looper *tls_looper;
static gb_looper main_looper = { 1 };
...
static void a_looper_acquire(void *l) { if (l) ((gb_looper *)l)->refs++; }
static void a_looper_release(void *l) { if (l) ((gb_looper *)l)->refs--; }
...
const nx_import *gb_android_table(size_t *n)
{
