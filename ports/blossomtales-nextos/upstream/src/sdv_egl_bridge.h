...
#ifndef SB_EGL_BRIDGE_H
#define SB_EGL_BRIDGE_H
...
/* Polling SDL de ate quatro controles independentes. O retorno e uma mascara
 * de slots conectados (bit 0=P1 ... bit 3=P4), ou -1 para SDL_QUIT. Os bits
 * de buttons usam SDL_CONTROLLER_BUTTON_* (A=0 ... DPAD_RIGHT=14). */
int sdv_egl_poll_gamepads(SdvGamepadState *states, int capacity);

/* Mascara mais recente, usada pelo fake InputDevice.GetDevice/GetDeviceIds
 * para o MonoGame observar conexao e remocao de cada controle. */
unsigned int sdv_egl_gamepad_mask(void);
/* Definida pelo dono dos pads (bt_input): bit 0 = P1 (uniao dos admitidos). */
