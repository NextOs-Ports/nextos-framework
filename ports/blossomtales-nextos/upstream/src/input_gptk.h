...
/* BLOSSOMTALES-CONTROLS-LIVE (1.4.0) — NEXTOSCONTROLLERS.gptk como runtime.
 *
 ...
 */
#ifndef BT_INPUT_GPTK_H
#define BT_INPUT_GPTK_H

#include <stddef.h>

/* Espelhos dos valores públicos do framework (verificados em tempo de
 * compilação em input_gptk.c). */
#define BT_GPTK_DECIDE_NONE 0
#define BT_GPTK_DECIDE_ACTION 1
#define BT_GPTK_DECIDE_SUPPRESS 2
#define BT_GPTK_DECIDE_NATIVE 3

#define BT_GPTK_CONTEXT_MENU 0
#define BT_GPTK_CONTEXT_GAMEPLAY 1
#define BT_GPTK_CONTEXT_CURSOR 2

#define BT_GPTK_LIVE_PASSTHROUGH 0
#define BT_GPTK_LIVE_DELIVERED 1
#define BT_GPTK_LIVE_SUPPRESSED 2
#define BT_GPTK_LIVE_FATAL (-1)
...
typedef int (*bt_gptk_button_sink_fn)(void *user, const char *action,
                                      int pressed, float value);
typedef int (*bt_gptk_vector_sink_fn)(void *user, const char *action,
                                      float x, float y);
...
int bt_gptk_context(void);          /* -1 quando não provado */
