...
#include <time.h>
#include <unistd.h>
#include <sys/resource.h>   /* setpriority: prioridade da thread de pump */
...
#define debugPrintf(...) nx_log(__VA_ARGS__)
#define logPrintf(...) fprintf(stderr, __VA_ARGS__)

/* The BufferQueue interface has two entry points at the same vtable slot in
 * different NDK revisions.  Telling them apart needs to know whether the first
 * argument is a code address in the game image, so the loader publishes the
 * span of the object that calls us. */
static void *text_base;
...
typedef struct {
  ...
  int headatend_fired;
  int decoder_done;
  int dry_cb;     /* callbacks consecutivos que não produziram (histerese p/ decoder_done) */
  ...
  /* INANIÇÃO: ring vazio quando o callback do SDL pediu dado. É o caso que dói
   * (o callback inteiro sai mudo => estalo), e era o único que NÃO era contado:
   * o 'continue' do got==0 pulava antes da conta de underrun. */
  volatile uint32_t starve_count;
  volatile uint32_t starve_done_count;  /* dos quais o shim já achava o decoder no fim */
  volatile uint32_t ring_low;       /* menor enchimento visto no callback (bytes) */
...
static volatile int g_logged_guest_engine = 0;
static volatile int g_logged_nonzero_buffer = 0;
...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    /* A atenuação aqui era margem para vários players somarem sem estourar.
     * O openal-soft abre UM player de saída (medido: active=1 o tempo todo) e
     * já traz o próprio limiter, então essa margem só deixava o jogo baixo.
     * O compressor de joelho abaixo continua segurando o pico. */
    if (src_channels == 1) {
      vol *= 0.5f;
    }
    player_vol[i] = vol;
    ...
    /* marca d'água do ring ANTES de consumir: é o sinal que prova se o
     * produtor está acompanhando o consumidor. */
    {
      uint32_t fill = ring_readable(p);
      if (fill < p->ring_low) p->ring_low = fill;
    }
    ...
    if (src_frames_got == 0) {
      /* ring seco com o player tocando: este callback sai mudo = estalo.
       * Contar SEMPRE -- filtrar por decoder_done escondia justamente o caso
       * que dói, e foi o que fez a 1a medição sair starve=0 com low=0. */
      p->starve_count++;
      if (p->decoder_done) p->starve_done_count++;
      continue;
    }
    queue_consume(p, got);
    p->played_bytes += got;
   ...
   * Ganho master: 0.30 era tuning de outro port (muitos SFX somando). O scaffold
  ...
  {
    static int perfaud = -1;
    if (perfaud < 0) perfaud = getenv("TR_PERFAUD") ? 1 : 0;
    ...
    if (callback_count - last_report_cb >= cbs_5s) {
      ...
      uint32_t under = 0, starve = 0, starve_done = 0, low = 0xffffffffu;
      int ddone = 0;
      ...
      for (int i = 0; i < MAX_PLAYERS; i++) {
        ...
        starve += p->starve_count;
        starve_done += p->starve_done_count;
        if (p->active && p->decoder_done) ddone++;
        if (p->active && p->play_state == SL_PLAYSTATE_PLAYING) {
          if (p->ring_low < low) low = p->ring_low;
          p->ring_low = 0xffffffffu;   /* marca d'água por janela de relatório */
        }
        if (p->cb_dead) dead++;
        if (p->active && p->play_state == SL_PLAYSTATE_PLAYING && rp < 100)
          rp += snprintf(rings + rp, sizeof(rings) - rp, " p%d=%u", i, ring_readable(p));
      }
      if (low == 0xffffffffu) low = 0;
      /* Imprimir de DENTRO do callback de audio e onde um log ja nos custou
       * fps antes: fica atras de TR_PERFAUD, nao ligado por padrao.  Os
       * contadores continuam (sao somas de inteiro) para o relatorio sair
       * completo quando alguem pedir. */
      if (perfaud)
          fprintf(stderr, "[PERFAUD] cb=%u underruns=%u starve=%u(done=%u) "
                  "low=%u clicks=%u active=%d ddone=%d dead=%d ring:%s\n",
                  callback_count, under, starve, starve_done, low, click_count,
                  num_active, ddone, dead, rings);
    }
  }
}

/* Initialize SDL2 audio */
static int audio_error_is_busy(void) {
  const char *error = SDL_GetError();
  return error && (strstr(error, "Device or resource busy") ||
                   strstr(error, "device is busy") ||
                   strstr(error, "Device is busy"));
}

static SDL_AudioDeviceID open_audio_with_busy_grace(
    const SDL_AudioSpec *want, SDL_AudioSpec *have) {
  SDL_AudioDeviceID device = 0;
  for (int attempt = 0; attempt < 5; attempt++) {
    device = SDL_OpenAudioDevice(NULL, 0, want, have, 0);
    if (device || !audio_error_is_busy())
      break;
    if (attempt == 0)
      fprintf(stderr, "[audio] ALSA device is transiently busy; retrying for "
                      "up to one second\n");
    usleep(250000);
  }
  return device;
...
static void ensure_audio_initialized(void) {
   ...
   * mesmo se o ALSA pedir mais. TS_AUDIO_SAMPLES ajusta sem rebuild. */
  {
    const char *e = getenv("TS_AUDIO_SAMPLES");
    int s = e ? atoi(e) : 1024;
  ...
  g_audio_dev = open_audio_with_busy_grace(&want, &have);
  if (g_audio_dev == 0) {
    ...
    fprintf(stderr, "[opensles_shim] audio auto ('%s') failed: %s — sweeping drivers\n",
            cur ? cur : "?", SDL_GetError());
    /* 🚨 A varredura de DISPOSITIVO tem de acontecer AQUI DENTRO, com o driver
     * ainda inicializado.  SDL_AudioInit derruba o driver anterior antes de
     * subir o proximo: tentar 'dsp' depois de 'alsa' matava o alsa que tinha
     * carregado, e a varredura de dispositivo que rodava DEPOIS do laco morria
     * com "Audio subsystem is not initialized".  Medido no R36S. */
    /* O backend ALSA da SDL NAO aceita nome de PCM cru em SDL_OpenAudioDevice
     * (responde "No such device": ele so conhece o que enumerou).  O nome do
     * PCM entra por AUDIODEV, que e' o que esse backend le.  Medido no R36S. */
    static const char *devs[] = { NULL, "plughw:0,0", "hw:0,0" };
    for (unsigned i = 0; i < sizeof(cands) / sizeof(cands[0]) && g_audio_dev == 0; i++) {
      ...
      for (unsigned d = 0; d < sizeof devs / sizeof *devs && g_audio_dev == 0; d++) {
        if (devs[d]) setenv("AUDIODEV", devs[d], 1);
        else         unsetenv("AUDIODEV");
        g_audio_dev = open_audio_with_busy_grace(&want, &have);
        fprintf(stderr, "[opensles_shim] driver '%s' AUDIODEV=%s -> %s\n",
                cands[i], devs[d] ? devs[d] : "(herdado)",
                g_audio_dev ? "OK" : SDL_GetError());
      }
    }
    if (g_audio_dev == 0)
      unsetenv("AUDIODEV");   /* nao deixar sujeira para o resto do processo */
  }
  if (g_audio_dev == 0) {
    logPrintf("[audio] SDL_OpenAudioDevice FALHOU: %s (jogo seguira mudo)\n",
              SDL_GetError());
    g_audio_initialized = 1;
    return;
  }

  /* Nivel de RELEASE: "mudo" nao pode ser um estado sem diagnostico. */
  logPrintf("[audio] SDL aberto: driver=%s %dHz %dch %d samples\n",
   ...
   * dedicada a ~4ms mantém o ring cheio. TS_NO_AUDIOPUMP=1 desliga. */
  if (!getenv("TS_NO_AUDIOPUMP")) {
    if (!g_pump_started) {
      extern void *sl_pump_thread_fn(void *);
      g_pump_stop = 0;
      if (pthread_create(&g_pump_thread, NULL, sl_pump_thread_fn, NULL) == 0) {
        g_pump_started = 1;
        debugPrintf("opensles_shim: pump thread iniciada (4ms)\n");
      } else {
        fprintf(stderr, "[audio] could not start the OpenSL pump thread\n");
      }
    }
  }
}
void *sl_pump_thread_fn(void *a) {
  (void)a;
  /* INVERSÃO DE PRIORIDADE: o consumidor (thread de callback do SDL) roda em
   * nice -20, mas ESTA thread -- que é quem enche o ring -- nascia em nice 0,
   * disputando CPU com a thread de render do jogo. Preempção aqui seca o ring e
   * o callback seguinte sai mudo (estalo). Produtor tem que acompanhar o
   * consumidor. Medido no Mali-450: era a origem da pipoca. */
  if (setpriority(PRIO_PROCESS, 0, -16) != 0)
    setpriority(PRIO_PROCESS, 0, -10);   /* sem privilégio: pega o que der */
  while (!g_pump_stop) {
...
void tr_audio_shutdown(void) {
  g_pump_stop = 1;
  if (g_pump_started && !pthread_equal(pthread_self(), g_pump_thread)) {
  ...
  /* The guest may already have destroyed the OpenSL engine and cleared our
   * bookkeeping after closing the device.  SDL's subsystem can still be live,
   * so always finish the firmware-side teardown here. */
  SDL_AudioQuit();
...
static void player_reset_meta(AudioPlayer *p) {
  ...
  p->underrun_count = 0;
  p->starve_count = 0;
  p->starve_done_count = 0;
  p->ring_low = 0xffffffffu;
  p->fadeout_count = 0;
  p->stopping = 0;
  p->frames_played = 0;
  p->play_state = SL_PLAYSTATE_STOPPED;
...
static SLresult play_SetPlayState(void *self, SLuint32 state) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].play_ptr == itf_ptr) {
      ...
      if (nofadestop < 0) nofadestop = getenv("COI_FADEOUT_STOP") ? 0 : 1;
      if (g_audio_dev) SDL_LockAudioDevice(g_audio_dev);
      if (state == SL_PLAYSTATE_STOPPED && p->play_state == SL_PLAYSTATE_PLAYING
        ...
        if (state == SL_PLAYSTATE_PLAYING && p->play_state != SL_PLAYSTATE_PLAYING) {
           ...
           * (se COI_AUDIO_CBGUARD estiver ligado, sem isso o slot ficava morto
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (written > 0) {
        ...
        p->enqueue_counter++;
        p->ever_enqueued = 1;
        if (!g_logged_nonzero_buffer) {
          const unsigned char *bytes = (const unsigned char *)pBuffer;
          int nonzero = 0;
          for (uint32_t k = 0; k < written; k++) {
            if (bytes[k] != 0) {
              nonzero = 1;
              break;
            }
          }
          if (nonzero && __sync_bool_compare_and_swap(
                  &g_logged_nonzero_buffer, 0, 1))
            logPrintf("[audio] first non-silent guest buffer queued: "
                      "%u bytes, %uHz %uch %ubit\n",
                      written, p->sample_rate, p->num_channels,
                      p->bits_per_sample);
        }
        /* O player VOLTOU a produzir: se alguém o tinha dado por encerrado,
         * destrava. Sem isto, 'decoder_done' era trava de mão única e um único
         * instante seco calava o fluxo contínuo do openal-soft para sempre. */
        p->decoder_done = 0;
        p->dry_cb = 0;
        p->headatend_fired = 0;
...
static SLresult engine_CreateOutputMix(void *self, void **pMix,
  ...
  /* Recibo AUDIO (E1): sem esta linha, um guest que para entre o engine e o
   * player deixa o log indecifravel (caso de campo AmberELEC). */
  static int outmix_logged = 0;
  if (!outmix_logged) {
    outmix_logged = 1;
    logPrintf("[audio] guest created the output mix\n");
  }
  init_outmix();
...
static SLresult engine_CreateAudioPlayer(void *self, void **pPlayer,
  ...
  if (!p) {
    logPrintf("[audio] CreateAudioPlayer FATAL: no player slots (guest fica mudo)\n");
  ...
  logPrintf("[audio] guest created audio player: %u ch %u Hz %u bit float=%d\n",
            p->num_channels, p->sample_rate, p->bits_per_sample, p->is_float);
...
static void opensles_pump_locked(void) {
  ...
  if (cbguard < 0) cbguard = getenv("COI_AUDIO_CBGUARD") ? 1 : 0;
  for (int i = 0; i < MAX_PLAYERS; i++) {
    AudioPlayer *p = &g_players[i];
    if (!p->active || p->play_state != SL_PLAYSTATE_PLAYING) continue;
    ...
    /* Folga do ring. Com 2x sobravam ~2 callbacks (~46 ms): qualquer preempção
     * do produtor secava o ring e o callback saía mudo. 4x (~93 ms) absorve a
     * hiccup de escalonamento sem virar atraso audível no gameplay. */
    uint32_t refill_threshold = callback_threshold * 4;
    if (refill_threshold > RING_BUFFER_SIZE / 2) refill_threshold = RING_BUFFER_SIZE / 2;

    /* Call callback multiple times to fill buffer ahead.  Com o alvo mais
     * fundo, 4 chamadas não alcançavam o alvo numa passada só. */
    int max_calls = 8;
    while (p->callback && !(cbguard && p->cb_dead) && readable <= refill_threshold && max_calls > 0) {
      ...
      if (p->ever_enqueued && p->enqueue_counter == counter_before) {
        /* Uma passada seca NÃO quer dizer "acabou": o openal-soft é fluxo
         * CONTÍNUO e às vezes não tem amostra pronta no instante do pump.
         * Declarar o fim na 1a passada travava o som para sempre (medido:
         * starve=100% com done=100%). Exigir insistência -- 4ms por passada,
         * então 64 passadas ~= 256ms sem produzir nada. */
        if (++p->dry_cb >= 64 && !p->decoder_done) {
          p->decoder_done = 1;
          /* debugPrintf("opensles_shim: player %d decoder_done apos %d passadas secas\n",
                      i, p->dry_cb); */
        }
        break;
      }
...
SLresult slCreateEngine_shim(void **pEngine, SLuint32 numOptions,
  ...
  if (__sync_bool_compare_and_swap(&g_logged_guest_engine, 0, 1))
    logPrintf("[audio] guest requested the native OpenSL ES engine\n");
  init_engine();
