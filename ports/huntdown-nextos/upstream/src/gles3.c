 ...
 *
 * The list this file covers was MEASURED, not guessed: hd_gl_sym() logs every
...
extern void *hd_gl_raw_sym(const char *name);
...
extern GLuint hd_gles3_draw_framebuffer(void);
extern GLuint hd_gles3_read_framebuffer(void);
extern void hd_gles3_texture_storage_2d(GLenum target, GLsizei levels,
...
static int gl3_trace_enabled(void)
{
    ...
    if (enabled < 0) {
        const char *value = getenv("HD_GLES3_TRACE");
...
GLenum hd_gles3_texture_target(GLenum target)
{
    return target == GL_TEXTURE_2D_ARRAY ? GL_TEXTURE_2D : target;
}

GLenum hd_gles3_texture_format(GLenum format)
{
    switch (format) {
...
static void gl3_TexImage3D(GLenum target, GLint level, GLint internal,
                           ...
                           const void *pixels)
{
    GLenum physical = hd_gles3_texture_target(target);
    GLenum es2_format = hd_gles3_texture_format(format);
    GLenum es2_internal = hd_gles3_texture_format(internal);
                             ...
                             GLsizei w, GLsizei h)
{
    ...
    GLenum physical = hd_gles3_texture_target(target);
    GLenum fmt = hd_gles3_texture_format(internal);
    ...
    hd_gles3_texture_storage_2d(target, levels, internal, w, h);
...
static void gl3_TexStorage3D(GLenum target, GLsizei levels, GLenum internal,
                             GLsizei width, GLsizei height, GLsizei depth)
{
    GLenum format = hd_gles3_texture_format(internal);
    if (format == internal)
        format = 0x1908;
...
void hd_gles3_texture_bound(GLenum active_texture, GLenum target,
                            GLuint texture)
{
...
static void gl3_GetIntegerv(GLenum pname, GLint *data)
{
    ...
    case GL_DRAW_FRAMEBUFFER_BINDING:
        value = (GLint)hd_gles3_draw_framebuffer();
        break;
    case GL_READ_FRAMEBUFFER_BINDING:
        value = (GLint)hd_gles3_read_framebuffer();
        break;
...
void *hd_gles3_fallback(const char *name)
{
    ...
    if (!known && seen_n < 256) {
        ...
        fprintf(stderr, "[HD-GL] STUB %s (returns 0)\n", name);
        fflush(stderr);
    }
