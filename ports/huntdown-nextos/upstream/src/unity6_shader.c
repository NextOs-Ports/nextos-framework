...
static const char *texture_replacement(enum hd_shader_stage stage,
                                       enum sampler_kind kind, int lod,
                                       char *reason, size_t reason_size)
{
    if (stage == HD_SHADER_STAGE_VERTEX) {
...
static int rewrite_tokens(const text_buf *body, enum hd_shader_stage stage,
                          ...
                          char *reason, size_t reason_size)
{
    ...
    for (size_t i = 0; i < length;) {
        ...
        if (word_equal(source + begin, token_len, "roundEven")) {
            if (!buf_add(result, "hd_roundEven")) return 0;
            continue;
        }
...
static int add_preamble(text_buf *output, const char *source, size_t length)
{
    ...
    if (!buf_add(output,
                 ...
                 "precision highp int;\n"
                 "#define HD_UNITY6_GLES2 1\n"
                 "int hd_maxi(int a, int b) { return a > b ? a : b; }\n"
                 "int hd_mini(int a, int b) { return a < b ? a : b; }\n"))
        return 0;
    if (contains_word(source, length, "roundEven")) {
        static const char helper[] =
            "highp float hd_roundEven(highp float x) {\n"
            "    highp float f = floor(x);\n"
            "    highp float d = x - f;\n"
            "    return d < 0.5 ? f : (d > 0.5 ? f + 1.0 : "
            "(mod(f, 2.0) == 0.0 ? f : f + 1.0));\n"
            "}\n"
            "highp vec2 hd_roundEven(highp vec2 x) { return vec2("
            "hd_roundEven(x.x), hd_roundEven(x.y)); }\n"
            "highp vec3 hd_roundEven(highp vec3 x) { return vec3("
            "hd_roundEven(x.x), hd_roundEven(x.y), hd_roundEven(x.z)); }\n"
            "highp vec4 hd_roundEven(highp vec4 x) { return vec4("
            "hd_roundEven(x.x), hd_roundEven(x.y), hd_roundEven(x.z), "
            "hd_roundEven(x.w)); }\n";
...
static int reject_known_unsupported(enum hd_shader_stage stage,
                                  ...
                                  char *reason, size_t reason_size)
{
    ...
    if (!source || !translated || !translated_len ||
        (stage != HD_SHADER_STAGE_VERTEX &&
         stage != HD_SHADER_STAGE_FRAGMENT)) {
        set_reason(reason, reason_size, "invalid translator arguments");
        return HD_SHADER_UNSUPPORTED;
    }
    ...
    while (position < source_len) {
        ...
        if (stage == HD_SHADER_STAGE_FRAGMENT &&
        ...
        if (depth == 0) {
            ...
            if (is_attr || is_in || is_out) {
                ...
                if (stage == HD_SHADER_STAGE_FRAGMENT && is_out) {
                    char name[96];
                ...
                if (stage == HD_SHADER_STAGE_VERTEX && (is_attr || (is_in && !is_out))) {
                    char attr_name[64];
    ...
    if (stage == HD_SHADER_STAGE_FRAGMENT && output_location > 0) {
...
int hd_unity6_translate_shader(enum hd_shader_stage stage,
                               ...
                               char *reason, size_t reason_size)
{
    return hd_unity6_translate_shader_ex(stage, source, source_len,
