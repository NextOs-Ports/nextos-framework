...
static int astc_load_library(void) {
  ...
  if (!handle) {
    fprintf(stderr, "[HD-ASTC] libastcUtil.so não pôde ser carregada: %s\n",
            dlerror());
    g_load_state = -1;
    return -1;
  }
  ...
  if (!p_config_init || !p_context_alloc || !p_decompress_image) {
    fprintf(stderr,
            "[HD-ASTC] símbolos ausentes cfg=%p ctx=%p dec=%p reset=%p\n",
  ...
  g_load_state = 1;
  fprintf(stderr, "[HD-ASTC] decoder Arm astcenc carregado\n");
  return 0;
}
...
static astcenc_context *astc_get_context(int block_x, int block_y) {
  ...
  if (result) {
    fprintf(stderr, "[HD-ASTC] config %dx%d falhou: %d\n", block_x, block_y,
            result);
    return NULL;
  }
  ...
  if (result || !context) {
    fprintf(stderr, "[HD-ASTC] contexto %dx%d falhou: %d\n", block_x,
            block_y, result);
    return NULL;
  }
  g_contexts[block_x][block_y] = context;
  fprintf(stderr, "[HD-ASTC] contexto %dx%d pronto\n", block_x, block_y);
  return context;
}
...
int astc_decode_rgba(void *destination, size_t destination_size,
  ...
  if (result) {
    fprintf(stderr, "[HD-ASTC] decode %dx%d/%dx%d falhou: %d\n", width,
