...
/* Opt-in flow proof: an open device is not sound.  HD_AUDIO_TRACE reports the
...
static int audio_trace_on(void)
{
    static int cached = -1;
    if (cached < 0)
        cached = getenv("HD_AUDIO_TRACE") != NULL;
    return cached;
}
...
typedef struct HdAAudioSymbol {
    const char *name;
    void *addr;
} HdAAudioSymbol;

static const HdAAudioSymbol aa_tab[] = {
