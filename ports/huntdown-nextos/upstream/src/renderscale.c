 ...
 *
 * Gated por HD_RENDERSCALE=N (divisor; 2 = metade = 640x360). default OFF.
...
void rs_init(void) {
  const char *e = getenv("HD_RENDERSCALE");
  if (!e) return;
  g_rs_div = atoi(e);
  if (g_rs_div < 2) g_rs_div = 2;
  (void)hd_display_size_detect(&g_scr_w, &g_scr_h, NULL);
  if (getenv("HD_RS_W")) g_scr_w = atoi(getenv("HD_RS_W"));
  if (getenv("HD_RS_H")) g_scr_h = atoi(getenv("HD_RS_H"));
...
void rs_present(void) {
  ...
  if (getenv("HD_RS_DBGCLEAR")) {  /* teste: magenta na tela real antes do quad */
