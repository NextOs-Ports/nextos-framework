...
/*
 * bionic arm64 uses one 64-bit word for sigset_t, whereas glibc uses 128
 * bytes. Passing a Unity/IL2CPP sigset_t directly to glibc therefore reads
 * beyond the Android object and, for output functions such as sigemptyset,
 * overwrites 120 bytes of adjacent memory.
 *
 * Keep these entry points separate from the host functions: this file itself
 * must continue to use glibc sigset_t for sigaction/pthread operations.
 */
typedef unsigned long hd_bionic_sigset_t;
_Static_assert(sizeof(hd_bionic_sigset_t) == 8,
               "Huntdown requires the arm64 bionic sigset_t ABI");

static void hd_bionic_sigset_to_host(hd_bionic_sigset_t bset,
                                     sigset_t *host) {
  sigemptyset(host);
  for (int signal_number = 1; signal_number <= 64; signal_number++) {
    if (bset & (1UL << (signal_number - 1)))
      sigaddset(host, signal_number);
  }
}

int hd_bionic_sigemptyset(hd_bionic_sigset_t *set) {
  if (!set) {
    errno = EINVAL;
    return -1;
  }
...
int hd_bionic_sigfillset(hd_bionic_sigset_t *set) {
  if (!set) {
    errno = EINVAL;
    return -1;
  }
...
int hd_bionic_sigaddset(hd_bionic_sigset_t *set, int signal_number) {
...
int hd_bionic_sigdelset(hd_bionic_sigset_t *set, int signal_number) {
  if (!set || signal_number < 1 || signal_number > 64) {
    errno = EINVAL;
    return -1;
  }
...
int hd_bionic_sigsuspend(const hd_bionic_sigset_t *set) {
  if (!set) {
    errno = EINVAL;
    return -1;
  }
  sigset_t host;
  hd_bionic_sigset_to_host(*set, &host);
