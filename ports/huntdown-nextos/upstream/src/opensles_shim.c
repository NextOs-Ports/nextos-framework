...
typedef struct {
  /* Allocate PCM storage only for live OpenSL players.  Keeping sixteen 4 MiB
   * rings inline reserved 64 MiB even before Unity loaded its first scene. */
  uint8_t *ring;
...
/* Reset player metadata without touching PCM pages. */
static int player_reset_meta(AudioPlayer *p) {
  if (!p->ring) {
    p->ring = malloc(RING_BUFFER_SIZE);
    if (!p->ring) {
      fprintf(stderr, "[SL] sem memória para ring PCM de %u bytes\n",
...
static void engine_obj_Destroy(void *self) {
  ...
  pthread_mutex_lock(&g_players_lock);
  for (int i = 0; i < MAX_PLAYERS; ++i) {
    free(g_players[i].ring);
    g_players[i].ring = NULL;
    g_players[i].ring_head = g_players[i].ring_tail = 0;
