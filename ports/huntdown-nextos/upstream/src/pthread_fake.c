...
/* ===== HD_CONDTRACE: descobre se o cond em que a MAIN espera é ALGUMA VEZ sinalizado.
...
static int condtrace_on(void) { if (g_condtrace < 0) g_condtrace = getenv("HD_CONDTRACE") ? 1 : 0; return g_condtrace; }
...
/* HD_CONDPOLL=ms: defesa contra lost-wakeup em condition variables (mesma classe
   do fix HD_SEMPOLL p/ semáforos). Em vez de esperar p/ sempre, faz timedwait
...
/* default OFF: as threads do GC são bionic-static e bloqueiam SIGPWR via syscall inline,
   bypassando nossos shims — desbloquear no nosso wait NÃO as alcança. Liga via HD_GCSAFEWAIT. */
int g_gc_safe_wait = 0;
void set_gc_safe_wait(int on) { g_gc_safe_wait = on; }
__attribute__((constructor)) static void gc_safe_wait_env(void) { if (getenv("HD_GCSAFEWAIT")) g_gc_safe_wait = 1; }
...
int pthread_cond_wait_fake(void **cslot, void **mslot) {
  if (getenv("HD_CONDWHO")) condwho_log();
  if (condtrace_on()) {
...
#include <sys/syscall.h>
#define HD_MAXTHR 96
static struct { pthread_t t; int tid; char comm[20]; } g_thrreg[HD_MAXTHR];
static int g_thrreg_n;
static pthread_mutex_t g_thrreg_lock = PTHREAD_MUTEX_INITIALIZER;
static void hd_thread_reg(void) {
  pthread_mutex_lock(&g_thrreg_lock);
  if (g_thrreg_n < HD_MAXTHR) {
    int i = g_thrreg_n++;
...
const char *hd_thread_comm(pthread_t t) {
  static char buf[24];
...
static void *thr_trampoline(void *p) {
  ...
  free(p);
  hd_thread_reg();   /* registra pthread_self()->comm assim que a thread sobe */
  /* HD_JOBLOG: loga start_routine (offset libunity) + arg + deref do contexto p/ cada thread
     criada pela engine. Correlacionado com HD_FUTEXLOG (uaddr da wait), identifica o ponteiro
     do JobQueue/contexto de CADA worker → resolve "1 ou 2 instâncias de JobQueue" do handoff. */
  if (getenv("HD_JOBLOG")) {
    extern uintptr_t hd_unity_base(void);
    uintptr_t ub = hd_unity_base();
  ...
  /* OPT-IN HD_SIGUNBLK: desbloqueia SIGPWR/SIGXCPU nesta thread. NÃO usar por padrão — as
...
/* ======== GAP arm64: rwlock/once/key/thread-misc/sem-extras ======== */
#include <signal.h>
#include <sched.h>
#include <time.h>
...
int pthread_sigmask_fake(int how, const void *set, void *old) {
  ...
  if (getenv("HD_SIGMASKLOG")) { static int n; if (n++ < 12)
  ...
  if (set) {
    ...
    /* OPT-IN HD_SIGFILTER (default OFF — ligar REGREDIA p/ frame 1): o il2cpp BLOQUEIA
    ...
    if (getenv("HD_SIGFILTER") && (how == SIG_BLOCK || how == SIG_SETMASK)) {
      bm &= ~(1UL << (30 - 1));   /* SIGPWR */
      bm &= ~(1UL << (24 - 1));   /* SIGXCPU */
    }
