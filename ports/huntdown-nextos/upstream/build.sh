...
[ -x "$STRIP" ] || { echo "strip não encontrado: $STRIP" >&2; exit 1; }
...
"$CC" --sysroot="$SR" -I src -I "$SR/usr/include" \
  ...
  -rdynamic -Wl,--gc-sections \
  -o huntdown "${SRCS[@]}" \
  -lSDL2 -ldl -lm -lpthread -lgcc_s
# Objetos do toolchain podem carregar DWARF com caminhos absolutos do host.
# O pacote privado preserva símbolos de runtime, mas nunca distribui debug local.
"$STRIP" --strip-debug huntdown

echo "NEXTOS BUILD OK -> $(file huntdown | cut -d, -f1-3)"
echo "toolchain: $TC"
