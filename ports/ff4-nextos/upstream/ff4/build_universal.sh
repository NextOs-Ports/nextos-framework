...
#
# Build universal aarch64 do Final Fantasy IV 3D Remake.
#
...
OUTPUT=${FF4_UNIVERSAL_OUTPUT:-ff4}
...
if [ "${FF4_BUSTER_IN_CONTAINER:-0}" != "1" ]; then
  ...
  exec docker run --rm --network none \
    -e FF4_BUSTER_IN_CONTAINER=1 \
    -e FF4_UNIVERSAL_OUTPUT="$OUTPUT" \
    -e FF4_HOST_UID="$(id -u)" \
    -e FF4_HOST_GID="$(id -g)" \
...
FRAMEWORK_ROOT=${FF4_FRAMEWORK_ROOT:-/framework}
cd /repo
...
chown "${FF4_HOST_UID:-0}:${FF4_HOST_GID:-0}" "$OUTPUT" 2>/dev/null || true
printf 'ff4 universal: %s, GLIBC max %s\n' "$OUTPUT" "$MAX_GLIBC"
