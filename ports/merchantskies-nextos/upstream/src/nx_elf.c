...
void nx_die(const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    fputs("[merchantskies] FATAL: ", stderr);
    vfprintf(stderr, fmt, ap);
    fputc('\n', stderr);
    va_end(ap);
    fflush(stderr);
...
static void apply(nx_mod *m, Elf64_Rela *r, size_t n, int *missing)
{
    for (size_t i = 0; i < n; i++) {
        ...
        case R_AARCH64_ABS64: {
            ...
            if (!a) {
                /* A weak undefined symbol legitimately stays NULL: the guest
                 * tests it before calling.  Name it anyway -- an unexplained
                 * zero in the GOT is exactly how the stdout data-symbol bug
                 * hid, and "4 unresolved" means nothing without the list. */
                if (name && *name)
                    nx_log("  unresolved %s <- %s%s", m->name, name,
