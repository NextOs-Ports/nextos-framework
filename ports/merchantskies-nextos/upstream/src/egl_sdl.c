...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Merchant of the Skies", SDL_WINDOWPOS_CENTERED,
...
/* Read-only identity for context-scoped policy adapters.  SDL owns creation,
 * binding and destruction; exposing the opaque token changes no GL state. */
...
EGLBoolean st_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    ...
    frame_count++;
    if (frame_count == 1) {
        extern void st_graphics_evidence_emit(void);
        extern void st_health_receipt_publish(void);
        /* Primeiro present real: contexto vivo, drawable válido — o momento
         * canônico da medição do contrato gráfico V3 e da prova de saúde
         * pending→active do launcher. */
        st_graphics_evidence_emit();
        st_health_receipt_publish();
    }
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Merchant of the Skies SDL EGL";
