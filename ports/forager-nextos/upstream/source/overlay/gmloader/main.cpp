...
extern DynLibFunction symtable_audiodbg[];
...
DynLibFunction *so_dynamic_libraries[32] = {
    symtable_audiodbg, /* [NextOS/forager] sonda libm do audio — vence a libc na resolucao */
    symtable_libc,
    symtable_zlib,
    symtable_gles2,
    NULL
...
/* [NextOS/forager] Captura de frame por glReadPixels — SO liga com GMLOADER_SHOT_FRAME=N.
 * Regra #26: experimento fora do caminho do binario de release; sem a env var isto e
 * um `if` que nunca dispara. Serve pra PROVAR imagem real na tela sem depender de
 * captura do /dev/fb0 (que no KMSDRM/Mali nao reflete o scanout).
...
static void forager_maybe_shot(int w, int h)
{
     ...
     * gesto de controle NA MESMA execucao — duas execucoes nao comparam mundo igual. */
    static long wants[16];
    static int nwants = -1;
    static long frame = 0;
    if (nwants == -1) {
        nwants = 0;
        const char *e = getenv("GMLOADER_SHOT_FRAME");
        while (e && *e && nwants < 16) {
            long v = atol(e);
            if (v > 0) wants[nwants++] = v;
            e = strchr(e, ',');
            if (e) e++;
        }
    }
    ...
    const char *base = getenv("GMLOADER_SHOT_PATH");
    if (!base || !*base)
        base = "/tmp/forager_shot.rgba";
    /* lista => um arquivo por frame; frame unico => caminho intacto (compat) */
    char out[PATH_MAX];
    if (nwants > 1)
        snprintf(out, sizeof(out), "%s.f%ld", base, frame);
    else
        snprintf(out, sizeof(out), "%s", base);
    FILE *f = fopen(out, "wb");
    if (f) { fwrite(buf, 1, n, f); fclose(f); }

    /* estatistica por canal, pra decidir sem abrir imagem */
    unsigned long long sr=0,sg=0,sb=0,sa=0;
    unsigned long nonblack=0, opaque=0;
    for (size_t i = 0; i < n; i += 4) {
        sr+=buf[i]; sg+=buf[i+1]; sb+=buf[i+2]; sa+=buf[i+3];
        if (buf[i]|buf[i+1]|buf[i+2]) nonblack++;
        if (buf[i+3] == 255) opaque++;
    }
    size_t px = n/4;
    char meta[512];
    snprintf(meta, sizeof(meta),
        "frame=%ld w=%d h=%d px=%zu\n"
        "media R=%.1f G=%.1f B=%.1f A=%.1f\n"
        "pixels nao-pretos=%lu (%.2f%%)\n"
        "pixels alpha=255=%lu (%.2f%%)\n",
        frame, w, h, px,
        (double)sr/px, (double)sg/px, (double)sb/px, (double)sa/px,
        nonblack, 100.0*nonblack/px,
        opaque, 100.0*opaque/px);
    fputs(meta, stderr);

    char metapath[PATH_MAX];
    snprintf(metapath, sizeof(metapath), "%s.txt", out);
    f = fopen(metapath, "w");
...
int main(int argc, char *argv[])
{
    ...
    /* [NextOS/forager] launcher nxbootstrap roda o executavel SEM argumentos
     * (argument_mode none) com cwd no gamedir: carregar ./config.json por
     * padrao quando existir e nenhum -c foi passado. */
    if (config_file_path.empty() && fs::exists(work_dir / CONFIG_FILE)) {
        config_file_path = work_dir / CONFIG_FILE;
        if (gmloader_config.parse_file(config_file_path.c_str()) < 0)
            warning("Error while loading the default config file\n");
    }
    ...
    /* [NextOS/forager] Licao do Chrono (regra do teardown): com as threads do guest
     * (audio, GM) ainda vivas, desmontar GL/SDL SIGSEGVa DEPOIS da saida (provado no
     * dmesg: translation fault em 0x0 apos o chord SELECT+START). O save ja esta no
     * disco (o jogo grava durante o gameplay). Saida normal = _exit(0) direto; o
     * desmonte completo so roda no caminho de relaunch, que precisa do SDL vivo. */
