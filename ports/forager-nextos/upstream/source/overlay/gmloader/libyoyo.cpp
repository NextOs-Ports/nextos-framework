...
/* [NextOS/forager] O Android runner consulta estes dois helpers antes de o
 * script GenerateLanguage escolher assets/local/*.json.  O APK 1.0.13 traz
 * as doze tabelas, mas o falso Android não publica locale e o fallback acaba
 * sempre em inglês.  O launcher valida a escolha e entrega somente códigos
 * curtos; manter o hook aqui preserva o fluxo nativo do jogo e não modifica o
 * ZIP selado pelo NXExtract. */
static const char *forager_language = NULL;
static const char *forager_region = NULL;
static const char *forager_language_asset = NULL;

void YYCreateStringHelper(RValue *rval, const char *str);
void YYThingDerefHelper(RValue *rval);

ABI_ATTR static const char *forager_get_language()
{
    return forager_language ? forager_language : "en";
}

ABI_ATTR static const char *forager_get_region()
{
    return forager_region ? forager_region : "US";
}

ReentrantHook REHPrepareGame = {};
ReentrantHook REHFunctionAdd = {};
static ReentrantHook REHSelectLanguage = {};

typedef ABI_ATTR void (*forager_select_language_t)(void *, void *, RValue &,
        int, RValue **);
static forager_select_language_t forager_select_language_original = NULL;

/* Forager carrega a configuracao persistente depois de consultar o locale do
 * Android e chama SelectLanguage com o valor antigo. Isso torna insuficiente
 * corrigir apenas os_get_language: o ingles salvo vence a escolha do launcher.
 * Preserve toda a rotina YYC original e troque somente seu primeiro argumento
 * pelo nome de tabela validado pelo adapter. */
ABI_ATTR static void forager_select_language_hook(void *self, void *other,
        RValue &ret, int argc, RValue **args)
{
    RValue forced = {};
    forced.kind = VALUE_UNDEFINED;
    RValue *saved = NULL;

    if (forager_language_asset && argc > 0 && args) {
        YYCreateStringHelper(&forced, forager_language_asset);
        saved = args[0];
        args[0] = &forced;
        static int logged = 0;
        if (!logged) {
            printf("[forager-language] SelectLanguage forced=%s\n",
                   forager_language_asset);
            logged = 1;
        }
    }

    rehook_unhook(&REHSelectLanguage);
    forager_select_language_original(self, other, ret, argc, args);
...
ABI_ATTR static void forager_os_get_language(RValue *ret, void *self,
        void *other, int argc, RValue *args)
{
    YYCreateStringHelper(ret, forager_language ? forager_language : "en");
}

ABI_ATTR static void forager_os_get_region(RValue *ret, void *self,
        void *other, int argc, RValue *args)
{
    YYCreateStringHelper(ret, forager_region ? forager_region : "US");
}
...
void Function_Add_Hook(const char* f_name, routine_t func, int arg_count, char reg)
{
    ...
    /* GameMaker registers these built-ins during Startup, after its init array
     * has restored the Android defaults. Override the native GML boundary so
     * GenerateLanguage observes the launcher selection on every firmware. */
    if (forager_language && strcmp(f_name, "os_get_language") == 0) {
        func = (routine_t)&forager_os_get_language;
        printf("[forager-language] os_get_language registered=%s\n",
               forager_language);
    } else if (forager_region && strcmp(f_name, "os_get_region") == 0) {
        func = (routine_t)&forager_os_get_region;
        printf("[forager-language] os_get_region registered=%s\n",
               forager_region);
    }
...
/* [NextOS/forager] resposta do hasSystemFeature sem passar pelo falso-JNI.
 * Verdade do device: handheld sem touchscreen, com controle. Loga cada feature
 * consultada (dedup simples) para o proximo ciclo saber o que o jogo decide com isso. */
ABI_ATTR static int pm_has_system_feature(const char *feature)
{
    int ret = 0;
    if (feature) {
        if (strstr(feature, "gamepad"))
            ret = 1;
        else if (strstr(feature, "touchscreen"))
            ret = 0;
    }

    static char seen[8][96];
    static int nseen = 0;
    int logged = 0;
    for (int i = 0; i < nseen; i++)
        if (feature && strncmp(seen[i], feature, sizeof(seen[0]) - 1) == 0)
            logged = 1;
    if (!logged) {
        printf("PackageManagerHasSystemFeature('%s') -> %d\n", feature ? feature : "(null)", ret);
        if (feature && nseen < 8) {
            strncpy(seen[nseen], feature, sizeof(seen[0]) - 1);
...
/* [NextOS/forager] Instrumento do MUDO: o runner inicializa a saida de audio em
 * ALCdevice_android::openPlayback (AudioTrack via JNI ou OpenSLES via dlopen).
 * Nenhum dos dois deixa rastro quando falha cedo — este hook loga entrada, retorno e
 * os estaticos JNI da classe. Log so; comportamento intacto. */
static ReentrantHook REHOpenPlayback = {};
static ReentrantHook REHStartPlayback = {};
static ReentrantHook REHThreadFuncAT = {};
static uintptr_t ALC_openPlayback_addr = 0;
static uintptr_t ALC_startPlayback_addr = 0;
static uintptr_t ALC_ThreadFuncAT_addr = 0;
static uint32_t *alc_ms_javaVM, *alc_ms_classAudioTrack, *alc_ms_Write, *alc_ms_pCreateEngine;
static uint32_t *alc_ms_dlHandle, *alc_ms_AudioTrack;
static uint8_t *alc_ms_fUseOpenSL;

ABI_ATTR static void ALC_startPlayback_hook(void *thiz)
{
    printf("[audio] startPlayback ENTER (ms_AudioTrack=%p)\n",
           alc_ms_AudioTrack ? (void *)(uintptr_t)*alc_ms_AudioTrack : (void *)-1);
    fflush(stdout);
    rehook_unhook(&REHStartPlayback);
    ((void (*)(void *))ALC_startPlayback_addr)(thiz);
    printf("[audio] startPlayback EXIT (ms_AudioTrack=%p)\n",
           alc_ms_AudioTrack ? (void *)(uintptr_t)*alc_ms_AudioTrack : (void *)-1);
    fflush(stdout);
}

ABI_ATTR static void *ALC_ThreadFuncAT_hook(void *cthread)
{
    printf("[audio] ThreadFuncAudioTrack ENTER (stopflag=%d)\n",
           cthread ? ((unsigned char *)cthread)[8] : -1);
    fflush(stdout);
    rehook_unhook(&REHThreadFuncAT);
    void *r = ((void *(*)(void *))ALC_ThreadFuncAT_addr)(cthread);
    printf("[audio] ThreadFuncAudioTrack EXIT (stopflag=%d)\n",
           cthread ? ((unsigned char *)cthread)[8] : -1);
    fflush(stdout);
    return r;
}

static ReentrantHook REHStopPlayback = {};
static uintptr_t ALC_stopPlayback_addr = 0;
ABI_ATTR static void ALC_stopPlayback_hook(void *thiz)
{
    printf("[audio] stopPlayback chamado\n");
    fflush(stdout);
    rehook_unhook(&REHStopPlayback);
    ((void (*)(void *))ALC_stopPlayback_addr)(thiz);
    rehook_hook(&REHStopPlayback);
}

static ReentrantHook REHMixData = {};
static uintptr_t ALC_aluMixData_addr = 0;
ABI_ATTR static void ALC_aluMixData_hook(void *dev, void *buf, int size)
{
    static int dbg = -1;
    if (dbg < 0)
        dbg = getenv("GMLOADER_AUDIO_DEBUG") != NULL;
    static int n = 0;
    if (dbg && (n++ % 200) == 0) {
        printf("[audio] aluMixData #%d (dev=%p buf=%p size=%d)\n", n - 1, dev, buf, size);
        fflush(stdout);
    }
    rehook_unhook(&REHMixData);
    ((void (*)(void *, void *, int))ALC_aluMixData_addr)(dev, buf, size);
    rehook_hook(&REHMixData);
    if (dbg && ((n - 1) % 200) == 0 && buf) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (a > fpeak && a < 1e10f) fpeak = a;
        }
        printf("[audio] mix #%d pico_s16=%d pico_float=%g\n", n - 1, peak, (double)fpeak);
        fflush(stdout);
    }
}

ABI_ATTR static int ALC_openPlayback_hook(void *thiz, char *devname)
{
    printf("[audio] openPlayback ENTER dev='%s' fUseOpenSL=%d javaVM=%p classAT=%p Write=%p pCreateEngine=%p dlHandle=%p\n",
           devname ? devname : "(null)",
           alc_ms_fUseOpenSL ? *alc_ms_fUseOpenSL : -1,
           alc_ms_javaVM ? (void *)(uintptr_t)*alc_ms_javaVM : (void *)-1,
           alc_ms_classAudioTrack ? (void *)(uintptr_t)*alc_ms_classAudioTrack : (void *)-1,
           alc_ms_Write ? (void *)(uintptr_t)*alc_ms_Write : (void *)-1,
           alc_ms_pCreateEngine ? (void *)(uintptr_t)*alc_ms_pCreateEngine : (void *)-1,
           alc_ms_dlHandle ? (void *)(uintptr_t)*alc_ms_dlHandle : (void *)-1);
    fflush(stdout);
    rehook_unhook(&REHOpenPlayback);
    int ret = ((int (*)(void *, char *))ALC_openPlayback_addr)(thiz, devname);
    printf("[audio] openPlayback EXIT ret=%d javaVM=%p classAT=%p Write=%p\n", ret,
           alc_ms_javaVM ? (void *)(uintptr_t)*alc_ms_javaVM : (void *)-1,
           alc_ms_classAudioTrack ? (void *)(uintptr_t)*alc_ms_classAudioTrack : (void *)-1,
           alc_ms_Write ? (void *)(uintptr_t)*alc_ms_Write : (void *)-1);
    fflush(stdout);
...
{
    const char *language = getenv("FORAGER_LANGUAGE");
    const char *region = getenv("FORAGER_REGION");
    const char *language_asset = getenv("FORAGER_LANGUAGE_ASSET");
    if (language && *language && region && *region &&
        language_asset && *language_asset) {
        forager_language = language;
        forager_region = region;
        forager_language_asset = language_asset;
        /* The 1.0.13 runner initializes these exported settings during its
         * init array, before the adapter can install function hooks.  Some
         * language call sites read the settings directly, so patch both
         * canonical writable slots as well as the public getters. */
        const uintptr_t language_slot_addr =
            so_symbol(mod, "g_LanguageSetting");
        const uintptr_t region_slot_addr =
            so_symbol(mod, "g_RegionSetting");
        if (!language_slot_addr || !region_slot_addr) {
            fatal_error("Forager locale settings are unavailable.\n");
            exit(-1);
        }
        *(const char **)language_slot_addr = forager_language;
        *(const char **)region_slot_addr = forager_region;
        hook_symbol(mod, "_Z17GetLanguageStringv",
                    (uintptr_t)&forager_get_language, 0);
        hook_symbol(mod, "_Z15GetRegionStringv",
                    (uintptr_t)&forager_get_region, 0);
        printf("[forager-language] android locale=%s-%s settings=patched\n",
               forager_language, forager_region);

        const uintptr_t select_language_addr = so_symbol(mod,
            "_Z25gml_Script_SelectLanguageP9CInstanceS0_R8YYRValueiPPS1_");
        if (!select_language_addr) {
            fatal_error("Forager SelectLanguage is unavailable.\n");
            exit(-1);
        }
        forager_select_language_original =
            (forager_select_language_t)select_language_addr;
        rehook_new(mod, &REHSelectLanguage, select_language_addr,
                   (uintptr_t)&forager_select_language_hook);
        rehook_hook(&REHSelectLanguage);
    }
    ...
    /* [NextOS/forager] PackageManagerHasSystemFeature: o runner faz a danca
     * Context->getPackageManager->hasSystemFeature via JNI (Classes que o falso-JNI
     * nao tem -> NULL). Hookar a funcao NA RAIZ evita os NULLs e responde a verdade
     * do device: sem touchscreen, com gamepad. Cada consulta e logada uma vez. */
    hook_symbol(mod, "_Z30PackageManagerHasSystemFeaturePKc",
                (uintptr_t)&pm_has_system_feature, 1);

    // Instrumento do mudo (ver comentario no ALC_openPlayback_hook)
    ALC_openPlayback_addr = so_symbol(mod, "_ZN17ALCdevice_android12openPlaybackEPc");
    alc_ms_javaVM = (uint32_t *)so_symbol(mod, "_ZN17ALCdevice_android9ms_javaVME");
    alc_ms_classAudioTrack = (uint32_t *)so_symbol(mod, "_ZN17ALCdevice_android18ms_classAudioTrackE");
    alc_ms_Write = (uint32_t *)so_symbol(mod, "_ZN17ALCdevice_android8ms_WriteE");
    alc_ms_pCreateEngine = (uint32_t *)so_symbol(mod, "_ZN17ALCdevice_android16ms_pCreateEngineE");
    alc_ms_fUseOpenSL = (uint8_t *)so_symbol(mod, "_ZN17ALCdevice_android13ms_fUseOpenSLE");
    alc_ms_dlHandle = (uint32_t *)so_symbol(mod, "_ZN17ALCdevice_android11ms_dlHandleE");
    alc_ms_AudioTrack = (uint32_t *)so_symbol(mod, "_ZN17ALCdevice_android13ms_AudioTrackE");
    if (ALC_openPlayback_addr) {
        rehook_new(mod, &REHOpenPlayback, ALC_openPlayback_addr, (uintptr_t)&ALC_openPlayback_hook);
        rehook_hook(&REHOpenPlayback);
    }
    ALC_startPlayback_addr = so_symbol(mod, "_ZN17ALCdevice_android13startPlaybackEv");
    if (ALC_startPlayback_addr) {
        rehook_new(mod, &REHStartPlayback, ALC_startPlayback_addr, (uintptr_t)&ALC_startPlayback_hook);
        rehook_hook(&REHStartPlayback);
    }
    ALC_ThreadFuncAT_addr = so_symbol(mod, "_ZN17ALCdevice_android20ThreadFuncAudioTrackEP7CThread");
    if (ALC_ThreadFuncAT_addr) {
        rehook_new(mod, &REHThreadFuncAT, ALC_ThreadFuncAT_addr, (uintptr_t)&ALC_ThreadFuncAT_hook);
        rehook_hook(&REHThreadFuncAT);
    }
    ALC_stopPlayback_addr = so_symbol(mod, "_ZN17ALCdevice_android12stopPlaybackEv");
    if (ALC_stopPlayback_addr) {
        rehook_new(mod, &REHStopPlayback, ALC_stopPlayback_addr, (uintptr_t)&ALC_stopPlayback_hook);
        rehook_hook(&REHStopPlayback);
    }
    ALC_aluMixData_addr = so_symbol(mod, "_Z10aluMixDataP16ALCdevice_structPvi");
    if (ALC_aluMixData_addr) {
        rehook_new(mod, &REHMixData, ALC_aluMixData_addr, (uintptr_t)&ALC_aluMixData_hook);
        rehook_hook(&REHMixData);
    }
