...
ABI_ATTR void *AAssetManager_open_impl(void *mgr, const char *filename, int mode)
{
    ...
    */
    ...
    /* [NextOS/forager] A versao Nuclear pede local/english.json diretamente,
     * mesmo quando o locale Android ja foi publicado. O launcher entrega um
     * basename allowlisted e o adapter troca somente esse caminho exato. O
     * APK continua imutavel e qualquer outro asset segue o fluxo upstream. */
    const char *requested = filename;
    std::string localized_path;
    if (filename && strcmp(filename, "local/english.json") == 0) {
        const char *table = getenv("FORAGER_LANGUAGE_ASSET");
        static const char *const allowed[] = {
            "english", "portuguese", "spanish", "french", "german",
            "russian", "turkish", "thai", "japanese", "korean",
            "chinese", "chinese_traditional"
        };
        bool valid = false;
        if (table && *table) {
            for (const char *candidate : allowed) {
                if (strcmp(table, candidate) == 0) {
                    valid = true;
                    break;
                }
            }
        }
        if (valid && strcmp(table, "english") != 0) {
            localized_path = std::string("local/") + table + ".json";
            requested = localized_path.c_str();
            printf("[forager-language] asset local/english.json -> %s\n",
                   requested);
        }
    }

    fs::path filename_restored = fs::path("assets") / requested;
...
DynLibFunction symtable_zlib[] = {
    ...
    THUNK_DIRECT(crc32),
    /* [NextOS/forager] o libyoyo do Forager importa zError e ele NAO estava aqui.
     * Mesmo simbolo que faltou no port katanazero (ver project_katanazero_mali450):
     * sem ele o resolve falha e o boot morre. */
    THUNK_DIRECT(zError),
