...
ABI_ATTR static jobjectArray iface_NewObjectArray(JNIEnv *env, jsize len, jclass clz, jobject init)
{
    ...
    /* [NextOS/forager] iface_FindClass devolve NULL para classe que o shim nao
     * implementa, e o jogo passava esse NULL direto pra ca -> SIGSEGV no deref de
     * instance_size. Mesma convencao do resto do shim (ver GetMethodID): logar e
     * devolver NULL, nunca estourar. Um array nulo o runner trata; um segfault nao. */
    if (clazz == NULL) {
        warning("jni: NewObjectArray com classe NULL (len=%d), devolvendo NULL.\n", (int)len);
        return NULL;
    }
...
ABI_ATTR static jclass iface_FindClass(JNIEnv *env, const char* name)
{
    ...
    if (it == registry.end()) {
        /* [NextOS/forager] logar QUAL classe o jogo procurou: e a lista do que o
         * falso-JNI ainda precisa responder (extensoes Java do Forager). */
        warning("jni: FindClass sem registro para '%s' -> NULL\n", name);
        return NULL;
    }
...
/* [NextOS/forager] Extrair o char* de um jstring COM VERIFICACAO DE TIPO.
 *
 * Ext_Call (despachante de extensao do GameMaker) converte o retorno de
 * CallExtensionFunction conforme o tipo declarado da funcao na extensao. Para uma
 * funcao declarada como ty_string ele trata o objeto devolvido como String e faz
 * strdup(str->str). Como devolvemos um java/lang/Double inerte, esse cast lia o primeiro
 * word do double (0.0 -> 0) como ponteiro => strdup(NULL) => SIGSEGV, logo depois de
 * blitworks_cloud_set_playtime.
 *
 * Conferir a classe real pelo _getClass() virtual resolve na origem: se nao for String,
 * devolvemos string vazia — inerte, nunca falha dura (regra da casa).
 */
static const char *jstring_cstr_safe(jstring jstr)
{
    if (jstr == NULL)
        return "";

    Object *obj = (Object *)jstr;
    if (obj->_getClass() != &String::clazz) {
        warning("jni: objeto que nao e java/lang/String usado como string; devolvendo \"\"\n");
        return "";
    }
...
ABI_ATTR static const jchar* iface_GetStringChars(JNIEnv *env, jstring jstr, jboolean* isCopy)
{
    ...
    return (jchar *)strdup(jstring_cstr_safe(jstr));
}
...
{
    return strlen(jstring_cstr_safe(jstr));
}
...
ABI_ATTR static const char* iface_GetStringUTFChars(JNIEnv *env, jstring jstr, jboolean* isCopy)
{
    ...
    return strdup(jstring_cstr_safe(jstr));
}
...
ABI_ATTR static T iface_CallNonVirtualMethodV(JNIEnv *env, jobject obj, jclass jclazz, jmethodID meth, va_list va)
{
    ...
    if (!method || !method->addr_variadic) {
        if (getenv("GMLOADER_AUDIO_DEBUG"))
            printf("[jni] CallNonVirtualMethodV metodo NULO (meth=%p addr=%p)\n",
                   (void *)method, method ? method->addr_variadic : NULL);
        if constexpr (std::is_same_v<T, void>)
            return;
        else
            return (T)0;
    ...
    printf("TRACE JNI: %s...\n", method->name);
#endif
    if (getenv("GMLOADER_AUDIO_DEBUG") && method->name && strcmp(method->name, "write") == 0) {
        static int wn = 0;
        if ((wn++ % 200) == 0)
            printf("[jni] despachando write #%d\n", wn - 1);
    }
