...
#define __USE_MISC
#include <algorithm>   // GCC 16: std::find nao vem mais transitivo
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <errno.h>
...
{
#ifndef NDEBUG
    if (string_starts_with(filename, "libm.so"))
      return 0x10000000;
    if (string_starts_with(filename, "libcompiler_rt.so"))
      return 0x14000000;
    if (string_starts_with(filename, "libstdc++.so") || string_starts_with(filename, "libc++.so") || string_starts_with(filename, "libc++_shared.so"))
      return 0x18000000;
    if (string_starts_with(filename, "libopenal.so"))
      return 0x1C000000;
    if (string_starts_with(filename, "libyoyo.so"))
      return 0x40000000;
#endif
...
{
    if (string_starts_with(filename, "libc.so") ||
        string_starts_with(filename, "libdl.so") ||
        string_starts_with(filename, "liblog.so") ||
        string_starts_with(filename, "libandroid.so") ||
        string_starts_with(filename, "libz.so"))
      return true;
    /* PATCH NextOS/armhf (receita project_hld_armhf_gmloader, patch 008):
     * o libyoyo do Forager tem DT_NEEDED para estas, que no Android sao do
     * sistema/bionic e NAO vem dentro do APK. Aqui quem responde por elas sao os thunks
     * do proprio loader (libc/aeabi, khronos, openal), entao declarar como builtin. */
    if (string_starts_with(filename, "libm.so") ||
        string_starts_with(filename, "libstdc++.so") ||
        string_starts_with(filename, "libOpenSLES.so") ||
        string_starts_with(filename, "libopenal.so") ||
        string_starts_with(filename, "libGLESv2.so") ||
        string_starts_with(filename, "libGLESv1_CM.so") ||
        string_starts_with(filename, "libEGL.so"))
      return true;
    return false;
}
...
so_module *so_load_module(const char *filename, struct zip *apk, void *vm) {
    ...
    for (int i = 0; i < modules.size(); i++) {
      ...
      /* PATCH NextOS/armhf (receita project_hld_armhf_gmloader, patch 2):
       * NAO e fatal. O libyoyo do Android tem DT_NEEDED para libs que NAO vem dentro do
       * APK (libm.so, libdl.so, liblog.so, libstdc++.so, libz.so ...) porque no Android
       * elas sao do sistema/bionic. Aqui quem responde por elas sao os nossos thunks de
       * libc, entao basta AVISAR e seguir; o que sobrar sem dono vira plt0_stub.
       * Abortar aqui derruba o boot antes mesmo de carregar o runtime. */
      warning("Skipping module '%s': nao esta no APK — sera atendido pelos thunks.\n",
              current.c_str());
      continue;   // 🚨 continue, NAO return: isto e um laco sobre as dependencias.
                  // Dar return aqui aborta o carregamento do libyoyo inteiro.
