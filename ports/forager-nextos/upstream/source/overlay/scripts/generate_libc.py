...
# PATCH NextOS/armhf (receita project_hld_armhf_gmloader, patch 4):
# o clang do host (22.x) choka nos constexpr de <cmath> da libstdc++ do GCC 16 do
# toolchain ("constexpr function never produces a constant expression"). Isso NAO afeta
# a extracao de assinaturas que este script faz — so precisamos das declaracoes.
# Ignorar diagnostico vindo de cmath/type_traits do proprio toolchain.
IGNORED_DIAG_FILES = ("/cmath", "/type_traits", "/bits/cpp_type_traits.h",
                      "/bits/specfun.h", "/math.h", "/bits/stl_iterator.h",
                      "/bits/basic_string.h", "/bits/basic_string.tcc",
                      "/bits/stl_algobase.h", "/bits/char_traits.h",
                      "/string_view", "/bits/string_view.tcc")

def _is_ignorable(diag):
    if diag.location.file:
        name = diag.location.file.name
        for frag in IGNORED_DIAG_FILES:
...
# PATCH NextOS/armhf: os bindings python do clang instalados sao mais antigos que o
# clang 22 do host e levantam ValueError("Unknown template argument kind N") em cursores
# novos. Nao queremos abortar por isso — o cursor desconhecido nunca e um FUNCTION_DECL
# que nos interessa, entao basta pular ele e continuar descendo na arvore.
def safe_kind(node):
    try:
        return node.kind
    except ValueError:
...
def extract_function_signatures(node):
    if safe_kind(node) == clang.cindex.CursorKind.FUNCTION_DECL:
        ...
        arguments = [p for p in safe_children(node) if safe_kind(p) == clang.cindex.CursorKind.PARM_DECL]
...
if __name__ == '__main__':
    ...
    for i in glob.glob("thunks/libc/*.cpp"):
        ...
        translation_unit = index.parse(i, args=["-std=c++17", "-ferror-limit=0", *includes, target_platform, *sysroot])
    ...
    translation_unit = index.parse("thunks/libc/common.hpp", args=["-std=c++17", "-ferror-limit=0", *includes, target_platform, *sysroot])
    handle_diagnostics(translation_unit.diagnostics)
    ...
    translation_unit = index.parse("thunks/libc/c_prototypes.h", args=["-std=c11", "-ferror-limit=0", *includes, "-DIS_TAB_GEN=1", target_platform, *sysroot])
    handle_diagnostics(translation_unit.diagnostics)
