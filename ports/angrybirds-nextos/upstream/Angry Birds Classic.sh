#!/bin/bash
# PORTMASTER: angrybirds, Angry Birds Classic.sh
# Angry Birds Classic — generated by nxbootstrap 0.6.26.
...
nxbootstrap_write_early_log() {
  ...
  for log_dir in "${GAMEDIR:-}" "$NXBOOTSTRAP_LAUNCHER_DIR" \
    ...
    log_path="$log_dir/angrybirds-launcher-error.$$.log"
    if (umask 077; set -C; printf '%s\n' \
      '== Angry Birds Classic | nxbootstrap 0.6.26 | pre-runtime failure ==' \
...
GAMEDIR="/$directory/ports/angrybirds"
[ -d "$GAMEDIR" ] || GAMEDIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)/angrybirds"
...
if ! echo "== Angry Birds Classic | nxbootstrap 0.6.26 | cfw=${CFW_NAME:-none} =="; then
  exit 1
fi
...
nxbootstrap_phase_event() {
  ...
  if ! (umask 077; set -C; printf '%s\n' \
    "{\"schema\":\"org.nextos.nxbootstrap.phase-result\",\"schema_version\":1,\"nxbootstrap_version\":\"0.6.26\",\"sequence\":$NXBOOTSTRAP_PHASE_SEQUENCE,\"run_id\":\"angrybirds-$$\",\"source\":\"$source\",\"phase\":\"$phase\",\"boundary\":\"$boundary\",\"status\":\"$event_status\",\"reason_code\":$reason_code,\"child_status\":$child_json,\"pid\":$$}" \
...
NXBOOTSTRAP_EXECUTABLE=$(readlink -f "$GAMEDIR/angrybirds-nextos" 2>/dev/null) || NXBOOTSTRAP_EXECUTABLE=""
case "$NXBOOTSTRAP_EXECUTABLE" in
  "$GAMEDIR"/*) ;;
  *) NXBOOTSTRAP_EXECUTABLE="" ;;
esac
if [ -z "$NXBOOTSTRAP_EXECUTABLE" ] || [ ! -f "$NXBOOTSTRAP_EXECUTABLE" ] || \
   [ ! -s "$NXBOOTSTRAP_EXECUTABLE" ] || [ -L "$GAMEDIR/angrybirds-nextos" ]; then
  echo "ERROR: executable is missing or unsafe: angrybirds-nextos"
  nxbootstrap_finish
  exit 1
fi
...
if [ -n "$NXBOOTSTRAP_INTERP" ] && [ ! -e "$NXBOOTSTRAP_INTERP" ]; then
  ...
  if [ -n "$NXBOOTSTRAP_INTERP_ALT" ]; then
    ...
    echo "NOTE: interpreter $NXBOOTSTRAP_INTERP is absent at its default path, but $NXBOOTSTRAP_INTERP_ALT exists; launching angrybirds-nextos through it."
  else
    echo "ERROR: angrybirds-nextos cannot run here: dynamic loader $NXBOOTSTRAP_INTERP is missing."
...
NXBOOTSTRAP_LOCK_FILE="$NXBOOTSTRAP_LOCK_DIR/nxport-angrybirds.flock"
...
NXBOOTSTRAP_REQUIRED_FILES='angrybirds-nextos
nxsplash-nextos
game.apk'
...
if NX_SPLASH_TTY="${CUR_TTY:-}" \
   ...
   $NXBOOTSTRAP_SPLASH_PREFIX "$NXBOOTSTRAP_SPLASH" 'Angry Birds Classic'; then
...
export NXCOMPAT_PORT_ID=angrybirds
