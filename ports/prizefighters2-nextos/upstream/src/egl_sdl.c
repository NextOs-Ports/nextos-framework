...
static _Thread_local pf2_sdl_context *current_context;
...
static void capture_frame_if_requested(void)
{
    ...
    if (finished)
        return;
    const char *path = getenv("PF2_SCREENSHOT");
    if (!path || !*path)
        return;
    int wanted = positive_env("PF2_SCREENSHOT_FRAME");
    if (!wanted)
        wanted = 300;
...
{
    int width = positive_env("PF2_SCREEN_W");
    int height = positive_env("PF2_SCREEN_H");
    ...
    fprintf(stderr, "[pf2/video] resolution %dx%d (%s)\n",
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Prizefighters 2", SDL_WINDOWPOS_CENTERED,
            SDL_WINDOWPOS_CENTERED, screen_width, screen_height,
            SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN_DESKTOP);
        if (!video_window) {
            fprintf(stderr,
                    "[pf2/video] SDL window depth%d/stencil%d failed: %s\n",
        ...
        if (share_root && !context_is_gles()) {
            fprintf(stderr,
                    "[pf2/video] SDL returned desktop GL; rejecting it\n");
        ...
        fprintf(stderr,
                "[pf2/video] GLES2 depth%d/stencil%d failed: %s\n",
                formats[i].depth, formats[i].stencil, SDL_GetError());
    ...
    const GLubyte *(*get_string)(GLenum) =
        (const GLubyte *(*)(GLenum))SDL_GL_GetProcAddress("glGetString");
    fprintf(stderr,
            "[pf2/video] SDL GLES2 depth%d/stencil%d drawable=%dx%d\n",
            depth_size, stencil_size, screen_width, screen_height);
    if (get_string) {
        fprintf(stderr, "[pf2/video] GL_VENDOR=%s\n",
                get_string(GL_VENDOR) ?
                    (const char *)get_string(GL_VENDOR) : "?");
        fprintf(stderr, "[pf2/video] GL_RENDERER=%s\n",
                get_string(GL_RENDERER) ?
                    (const char *)get_string(GL_RENDERER) : "?");
        fprintf(stderr, "[pf2/video] GL_VERSION=%s\n",
...
int pf2_sdl_video_init(void)
{
    if (video_mode != VIDEO_UNDECIDED)
        return video_mode == VIDEO_SDL;

    if (env_on("PF2_RAW_EGL_CONTEXTS")) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr, "[pf2/video] raw EGL selected by override\n");
        return 0;
    }
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr,
                "[pf2/video] SDL video unavailable (%s); using raw EGL\n",
                SDL_GetError());
        return 0;
    }

    const char *driver = SDL_GetCurrentVideoDriver();
    int force_sdl = env_on("PF2_PURE_SDL_CONTEXTS");
    if (!force_sdl && driver && strcasecmp(driver, "mali") == 0) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr,
                "[pf2/video] backend '%s' -> raw EGL/fbdev\n", driver);
        if (!was_initialized)
    ...
    fprintf(stderr, "[pf2/video] backend '%s' -> SDL-owned contexts%s\n",
...
int pf2_sdl_video_active(void)
{
    return video_mode == VIDEO_SDL;
}

void *pf2_sdl_gl_proc(const char *name)
{
...
{
    pf2_sdl_surface *surface = calloc(1, sizeof *surface);
    if (!surface)
...
{
    (void)display;
    pf2_sdl_surface *surface = (pf2_sdl_surface *)handle;
                                       ...
                                       const EGLint *attributes)
{
    ...
    (void)share;
    (void)attributes;
    pf2_sdl_context *context = calloc(1, sizeof *context);
    if (!context)
...
{
    (void)display;
    pf2_sdl_context *context = (pf2_sdl_context *)handle;
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    ...
    pf2_sdl_context *context = (pf2_sdl_context *)handle;
    pf2_sdl_surface *surface = (pf2_sdl_surface *)draw;
    if (!context || context->magic != CONTEXT_MAGIC ||
...
EGLBoolean pf2_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    (void)display;
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Prizefighters 2 SDL EGL";
...
void *pf2_sdl_egl_proc(const char *name)
{
