...
int pf2_log_level = 0;   /* PF2_LOGCAT=1 mirrors the game's own log */
...
{
    fprintf(stderr, "[pf2] abort message: %s\n", m ? m : "(null)");
}
...
static int my_fclose(void *f)               { return f == (void *)pf2_sF ? 0 : fclose(xf(f)); }
...
static void my___stack_chk_fail(void)
{
    ...
    void *base = NULL;
    extern const char *pf2_mod_at(const void *addr, void **base_out);
    const char *module = pf2_mod_at(caller, &base);
    if (module && base)
...
static const unsigned char pf2_ctype_tab[1 + 256] = { 0 };
static const unsigned char *pf2_ctype_ptr = pf2_ctype_tab + 1;
...
extern void *pf2_android_sym(const char *name);
extern void *pf2_egl_sym(const char *name);
extern void *pf2_jni_sym(const char *name);
extern void *pf2_protected_sym(const char *name);
...
static nx_import tab[] = {
    ...
    A("__sF", pf2_sF), A("_ctype_", &pf2_ctype_ptr),
...
static void *my_dlsym(void *h, const char *sym)
{
    ...
    if (!a) a = pf2_android_sym(sym);
    if (!a) a = pf2_egl_sym(sym);
    if (!a) a = pf2_jni_sym(sym);
    if (!a) a = pf2_protected_sym(sym);
...
static int my_dladdr(const void *addr, void *info)
{
    ...
    extern const char *pf2_mod_at(const void *addr, void **base_out);
    void *base = NULL;
    const char *name = pf2_mod_at(addr, &base);
...
static int my_dl_iterate_phdr(int (*cb)(void *, size_t, void *), void *data)
{
    ...
    extern int pf2_iterate_mods(int (*cb)(void *, size_t, void *), void *data);
    return pf2_iterate_mods(cb, data);
}
...
size_t pf2_bionic_count(void) { return sizeof tab / sizeof *tab; }

nx_import *pf2_bionic_entries(size_t *n)
{
