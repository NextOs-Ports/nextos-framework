...
OUTPUT=${PF2_UNIVERSAL_OUTPUT:-pf2-universal}

if [ "${PF2_BUSTER_IN_CONTAINER:-0}" != "1" ]; then
  ...
  if [ -n "${PF2_BUSTER_IMAGE:-}" ]; then
    BUSTER_IMAGE=$PF2_BUSTER_IMAGE
  ...
  exec docker run --rm \
    -e PF2_BUSTER_IN_CONTAINER=1 \
    -e PF2_UNIVERSAL_OUTPUT="$OUTPUT" \
    -e PF2_HOST_UID="$(id -u)" \
    -e PF2_HOST_GID="$(id -g)" \
    -v "$PORT_DIR":/repo \
...
if [ -n "${PF2_HOST_UID:-}" ] && [ -n "${PF2_HOST_GID:-}" ]; then
  chown "$PF2_HOST_UID:$PF2_HOST_GID" "$OUTPUT" 2>/dev/null || true
fi
