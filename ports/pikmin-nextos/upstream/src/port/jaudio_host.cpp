...
constexpr u64 kMovieHandoffNs     = 20000000ULL;
...
void blockingDelayNs(long nanoseconds)
{
	const timespec delay = { nanoseconds / 1000000000L, nanoseconds % 1000000000L };
	OSPortBlockingEnter();
...
{
#if defined(__GNUC__)
	if (PikiMovieAudioActive == nullptr) {
		return false;
	}
#endif
	return PikiMovieAudioActive() != 0;
}
...
OSThread sDvdThread {};
alignas(32) std::array<u8, AUDIO_STACK_SIZE> sAudioStack {};
alignas(32) std::array<u8, AUDIO_STACK_SIZE> sDvdStack {};
bool sAudioThreadCreated = false;
bool sDvdThreadCreated   = false;
...
void* audioProducer(void*)
{
	std::array<s16, kFrameSamples * 2> pcm {};
	sAudioDiagnostics.start();
	bool sinkReady          = false;
	bool sinkWasMovie       = false;
	size_t prebuffered      = 0;
	u64 nextSinkAttemptNs   = 0;
	u64 nextSoftwareFrameNs = 0;

	while (sAudioRunning.load(std::memory_order_acquire)) {
		const bool movieActive = movieOwnsSink();
		if (movieActive) {
			/*
			 * H4M opened (and therefore replaced) the singleton stream.  Do
			 * not close or queue into it; keep the JAudio clock moving.
			 */
			sinkReady    = false;
			sinkWasMovie = true;
			prebuffered  = 0;
			...
			if (sinkWasMovie) {
				/*
				 * The ownership flag and singleton close happen next to each
				 * other, but on different threads.  Leave one short handoff
				 * window before replacing the stream.
				 */
				sinkWasMovie = false;
				nextSinkAttemptNs = now + kMovieHandoffNs;
			}
			if (nextSinkAttemptNs == 0 || now >= nextSinkAttemptNs) {
				sinkReady = PikiAudioSinkTryOpen(kHostSampleRate) != 0;
				prebuffered = 0;
				nextSinkAttemptNs
				    = sinkReady ? 0 : monotonicNs() + kSinkRetryPeriodNs;
			}
		}
		...
		if (sinkReady) {
			...
			if (!PikiAudioSinkGetProgress(&submitted, &queued)) {
				...
				sinkReady = false;
				prebuffered = 0;
				nextSinkAttemptNs = monotonicNs() + kSinkRetryPeriodNs;
			} else if (queued >= kMaxQueuedFrames) {
				blockingDelayNs(2000000L);
				continue;
			}
		}

		if (sinkReady) {
			nextSoftwareFrameNs = 0;
		} else {
			const u64 now = monotonicNs();
			if (nextSoftwareFrameNs == 0 || now > nextSoftwareFrameNs) {
				nextSoftwareFrameNs = now;
			} else {
				blockingDelayNs(static_cast<long>(nextSoftwareFrameNs - now));
			}
		}
		...
		if (sinkReady) {
			if (!PikiAudioSinkQueue(pcm.data(), kFrameSamples)) {
				...
				prebuffered = 0;
				nextSinkAttemptNs = monotonicNs() + kSinkRetryPeriodNs;
				nextSoftwareFrameNs = monotonicNs() + kAudioFramePeriodNs;
				continue;
			}
			prebuffered += kFrameSamples;
			if (prebuffered >= kPrebufferFrames && !PikiAudioSinkResume()) {
				PikiAudioSinkClose();
				sinkReady = false;
				prebuffered = 0;
				nextSinkAttemptNs = monotonicNs() + kSinkRetryPeriodNs;
			...
			nextSoftwareFrameNs += kAudioFramePeriodNs;
		}
	}
...
{
	if (sAudioThreadCreated) {
		sAudioRunning.store(false, std::memory_order_release);
		OSJoinThread(&sAudioThread, nullptr);
		sAudioThreadCreated = false;
	}
}
...
void StartAudioThread(void* heap, s32 heapSize, u32 aramSize, u32 flags)
{
	...
	/*
	 * Jac_Start configures aux buses immediately after this function returns.
	 * Initialise the native core synchronously so the producer can never wipe
	 * those settings later when its cooperative thread first gets scheduled.
	 */
	Jac_Init();
	Jac_InitSinTable();
	ResetPlayerCallback();
	...
	if ((flags & AUDIO_THREAD_FLAG_DVD) != 0 && !sDvdThreadCreated) {
		jac_dvdproc_init();
		const s32 priority = OSGetThreadPriority(OSGetCurrentThread()) - 2;
		if (OSCreateThread(&sDvdThread, &jac_dvdproc, nullptr, sDvdStack.data() + sDvdStack.size(),
		                   static_cast<u32>(sDvdStack.size()), priority, OS_THREAD_ATTR_DETACH)) {
			OSResumeThread(&sDvdThread);
			sDvdThreadCreated = true;
		}
	}

	if ((flags & AUDIO_THREAD_FLAG_AUDIO) != 0) {
		const s32 priority = OSGetThreadPriority(OSGetCurrentThread()) - 3;
		sAudioRunning.store(true, std::memory_order_release);
		if (OSCreateThread(&sAudioThread, &audioProducer, nullptr, sAudioStack.data() + sAudioStack.size(),
		                   static_cast<u32>(sAudioStack.size()), priority, 0)) {
			OSResumeThread(&sAudioThread);
			sAudioThreadCreated = true;
		} else {
			sAudioRunning.store(false, std::memory_order_release);
			std::fprintf(stderr, "[jaudio] failed to create host producer thread\n");
		}
	}
}
