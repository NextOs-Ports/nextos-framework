...
static void *jni_CallStaticObjectMethod(void *env, void *clazz,
  ...
  if (methodID == &g_method_tags[MID_DEVICE_LANG]) {
    /* Idioma escolhido no launcher (GAME_LANGUAGE -> NXPORT_LANGUAGE). O jogo
     * (oz::Locale) le' um codigo ISO de 2 letras. Aceitos: en/fr/it/de/es
     * (EFIGS do Castle of Illusion HD); japones NUNCA (regra da casa).
     * auto/vazio/desconhecido -> en. */
    const char *lang = getenv("NXPORT_LANGUAGE");
    static const char *ok[] = { "en", "fr", "it", "de", "es", NULL };
    const char *chosen = "en";
    if (lang && *lang)
      for (int i = 0; ok[i]; i++)
        if (strcmp(lang, ok[i]) == 0) { chosen = ok[i]; break; }
    static int logged = 0;
    if (!logged) { logged = 1;
      debugPrintf("[coi/idioma] NXPORT_LANGUAGE=%s -> getDeviceLanguage=%s\n",
                  lang && *lang ? lang : "(vazio)", chosen); }
    return make_jstring(chosen);
  }
