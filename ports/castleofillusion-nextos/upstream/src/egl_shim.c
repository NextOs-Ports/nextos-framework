...
EGLBoolean egl_shim_SwapBuffers(EGLDisplay dpy, EGLSurface surface) {
  ...
  if (has_real_gl && current_context && !current_context->is_pbuffer) {
    { extern void coi_profile_start(void); coi_profile_start(); }
    coi_maybe_screenshot();
    { static unsigned fs = 0; if ((++fs % 900) == 0) coi_stack_shrink(); }
    /* [PERF] frame-time entre swaps, SEPARANDO o que é trabalho nosso do que é
     * espera de vsync. O tempo DENTRO de SDL_GL_SwapWindow é onde o driver
     * bloqueia até o próximo vblank; o de fora é engine + submissão GL.
     *   swap alto + cpu baixo  -> preso no degrau do vsync (33.3 / 50.0 ms):
     *                             cortar pouco trabalho pula um degrau inteiro.
     *   cpu alto               -> gargalo real (engine/driver), medir os
     *                             contadores draws/bufdata/glget por frame.
     * Custo: 3 clock_gettime/frame + 1 fprintf/5s. */
    struct timespec t0, t1, c0, p0;
    ...
    {
      extern unsigned long coi_c_draws, coi_c_bufdata, coi_c_glget,
          coi_c_texsub, coi_c_uniform;
      static struct timespec last = {0, 0}, lastc = {0, 0}, lastp = {0, 0};
      static double sum = 0, mx = 0, swapsum = 0, cpusum = 0, procsum = 0;
      static unsigned n = 0, s20 = 0, s40 = 0;
      double swapms = (t1.tv_sec - t0.tv_sec) * 1e3 +
                      (t1.tv_nsec - t0.tv_nsec) / 1e6;
      /* 🔬 CPU DE VERDADE vs relógio de parede. Tempo fora do swap pode ser
       * trabalho OU sono (limiter interno da engine / espera de outra thread) —
       * o relógio de parede não distingue, o CLOCK_*_CPUTIME distingue.
       *   busy≈wall  -> gargalo é CPU (engine/driver) — resolução interna NÃO ajuda
       *   busy<<wall -> a engine está esperando/dormindo — atacar o pacing
       *   allcpu>>busy -> o custo está em OUTRA thread (áudio/worker) */
      if (lastc.tv_sec || lastc.tv_nsec) {
        cpusum += (c0.tv_sec - lastc.tv_sec) * 1e3 + (c0.tv_nsec - lastc.tv_nsec) / 1e6;
        procsum += (p0.tv_sec - lastp.tv_sec) * 1e3 + (p0.tv_nsec - lastp.tv_nsec) / 1e6;
      }
      lastc = c0; lastp = p0;
      if (last.tv_sec) {
        double ms = (t1.tv_sec - last.tv_sec) * 1e3 +
                    (t1.tv_nsec - last.tv_nsec) / 1e6;
        { extern void coi_profile_frame(double); coi_profile_frame(ms); }
        sum += ms; swapsum += swapms; n++;
        if (ms > mx) mx = ms;
        if (ms > 20) s20++;
        if (ms > 40) s40++;
        if (sum >= 5000) {
          fprintf(stderr,
                  "[PERF] fps=%.1f avg=%.1fms fora_do_swap=%.1fms swapwait=%.1fms "
                  "busy=%.1fms allcpu=%.1fms max=%.0fms >20ms=%u >40ms=%u\n",
                  n * 1000.0 / sum, sum / n, (sum - swapsum) / n, swapsum / n,
                  cpusum / n, procsum / n, mx, s20, s40);
          fprintf(stderr,
                  "[PERFLOAD] por frame: draws=%.0f bufdata=%.1f glget=%.1f "
                  "texsub=%.1f unifm4=%.0f\n",
                  (double)coi_c_draws / n, (double)coi_c_bufdata / n,
                  (double)coi_c_glget / n, (double)coi_c_texsub / n,
                  (double)coi_c_uniform / n);
          { extern void coi_profile_report(void); coi_profile_report(); }
          sum = 0; swapsum = 0; n = 0; mx = 0; s20 = 0; s40 = 0;
          cpusum = 0; procsum = 0;
          coi_c_draws = coi_c_bufdata = coi_c_glget = 0;
          coi_c_texsub = coi_c_uniform = 0;
        }
      }
      last = t1;
    }
