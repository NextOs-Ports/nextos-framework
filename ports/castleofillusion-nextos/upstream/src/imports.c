...
static void my_glBlendEquation(unsigned);
static void my_glBlendEquationSeparate(unsigned, unsigned);
void *coi_gl_proc_override(const char *name) {
  ...
  /* 🏎️ shadow de estado: se a engine resolvesse estes por procaddress em vez da
   * tabela, o glGetIntegerv responderia de um shadow desatualizado. */
  if (name && strcmp(name, "glGetIntegerv") == 0) return (void *)my_glGetIntegerv;
  if (name && strcmp(name, "glBlendEquation") == 0) return (void *)my_glBlendEquation;
  if (name && strcmp(name, "glBlendEquationSeparate") == 0) return (void *)my_glBlendEquationSeparate;
  return NULL;
}
...
/* estado GL rastreado p/ [DRAWATTR] (diag passes do Mickey) e p/ RESPONDER
 * glGetIntegerv sem ir no driver (ver my_glGetIntegerv). */
static unsigned g_st_depthfunc = 0x0201, g_st_bsrc = 1, g_st_bdst = 0;
static unsigned g_st_bsrcA = 1, g_st_bdstA = 0;          /* separate alpha */
static unsigned g_st_beq = 0x8006, g_st_beqA = 0x8006;   /* GL_FUNC_ADD */
static int g_st_depthmask = 1, g_st_blend = 0, g_st_depth = 0, g_st_cull = 0,
           g_st_stencil = 0, g_st_cmask = 0xf;

/* 🏎️ getenv() CACHEADO: getenv é varredura linear do environ e estes wrappers
 * estão no caminho de frame (milhares de chamadas/segundo). Um flag de debug lido
 * uma vez e guardado num static custa zero; lido a cada draw custa ms/frame. */
/* 🏎️ contadores de carga por frame (lidos e zerados pelo [PERF] do egl_shim):
 * dizem SE o frame é caro por draw call, por upload de geometria ou por leitura
 * de estado — sem precisar de perf/strace no device. */
unsigned long coi_c_draws = 0, coi_c_bufdata = 0, coi_c_glget = 0,
              coi_c_texsub = 0, coi_c_uniform = 0;
static int g_attr_log = -1, g_stride_dbg = -1, g_tint_green = -1, g_vert_dump = -1,
           g_tex_half = -1, g_texid_log = -1, g_buf_log = -1;
static int envflag(const char *name, int *slot) {
  if (*slot < 0) *slot = getenv(name) ? 1 : 0;
  return *slot;
}
...
static int g_unif_log = -1, g_unif_gate = -1;
static int unif_log_on(void) {
  /* 🏎️ o getenv aqui rodava POR glUniform* — centenas por frame. Cacheado. */
  if (!envflag("COI_UNIF_LOG", &g_unif_log)) return 0;
  if (envflag("COI_UNIF_GATE", &g_unif_gate) && access("/dev/shm/coi_uniflog", F_OK) != 0)
    return 0;
  return 1;
}
...
static int my_glGetUniformLocation(unsigned prog, const char *nm) {
  ...
  static int n = 0;
  if (envflag("COI_UNIF_LOG", &g_unif_log) && n < 2500 && nm) { /* sem gate: locs vem no link, pre-gameplay */
...
static void my_glBindAttribLocation(unsigned prog, unsigned idx, const char *nm) {
  ...
  static int n = 0;
  if (envflag("COI_ATTR_LOG", &g_attr_log) && n < 30) {
...
static void my_glVertexAttribPointer(unsigned idx, int sz, unsigned typ,
  ...
  static int n = 0;
  if (envflag("COI_ATTR_LOG", &g_attr_log) && n < 40) {
  ...
  if (envflag("COI_ATTR_LOG", &g_attr_log) && typ == 0x1401 && norm) {
  ...
  if (envflag("COI_STRIDE_DBG", &g_stride_dbg)) {
    static int d = 0;
...
static void my_glBindFramebuffer(unsigned tgt, unsigned fb) {
  ...
  static int fbodump = -1;
  if (envflag("COI_FBO_DUMP", &fbodump) && g_cur_fbo != 0 && fb == 0 &&
      g_draws_fbo > 2000) {
    static int done = 0;
    if (!done) {
      done = 1;
...
static void coi_attr_dump_once(unsigned prog, int cnt) {
  ...
  static int sn = 0;
  if (!envflag("COI_UNIF_LOG", &g_unif_log) ||
      access("/dev/shm/coi_uniflog", F_OK) != 0)
    return;
  for (int i = 0; i < sn; i++)
...
static void my_glBufferData(unsigned tgt, long size, const void *data, unsigned usage) {
  ...
  /* 🏎️ REGRA MALI: glGetError é round-trip SÍNCRONO no blob. Isto rodava DUAS
   * vezes por upload de buffer — e a engine sobe geometria a cada frame. O dreno
   * de erro agora só existe quando o diagnóstico está ligado (COI_BUF_LOG=1). */
  coi_c_bufdata++;
  envflag("COI_BUF_LOG", &g_buf_log);
  static unsigned (*gerr)(void) = NULL;
  if (g_buf_log && !gerr) rgl("glGetError", (void **)&gerr);
  if (g_buf_log && gerr) while (gerr()) {}
  if (real) real(tgt, size, data, usage);
  unsigned e = (g_buf_log && gerr) ? gerr() : 0;
  ...
  if (envflag("COI_UNIF_LOG", &g_unif_log) && tgt == 0x8892 && data && size > 20000 &&
      (size % 44) == 0) {
    static int vn = 0;
    if (vn < 6) {
      vn++;
  ...
  if (tgt == 0x8892 && data && envflag("COI_TINT_GREEN", &g_tint_green) && size > 2000 &&
      (size % 40) == 0) {
  ...
  if (envflag("COI_VERT_DUMP", &g_vert_dump) && data && size > 200000 && tgt == 0x8892) {
    static int vd = 0;
    if (vd < 2) {
      const unsigned char *b = (const unsigned char *)data;
...
static void my_glBlendFunc(unsigned sf, unsigned df) {
  ...
  if (envflag("COI_NO_ADD", &g_no_add)) {
    if ((sf==1 && df==1) || (sf==0x302 && df==1)) { sf=0x302; df=0x303; } /* SRC_ALPHA,1-SRC_ALPHA */
  }
  if (real) real(sf, df);
}
/* glBlendEquation(Separate): não eram interceptados — sem eles o shadow de
 * BLEND_EQUATION_RGB/ALPHA que o my_glGetIntegerv responde envelheceria. */
static void my_glBlendEquation(unsigned mode) {
  static void (*real)(unsigned) = NULL; rgl("glBlendEquation", (void **)&real);
  g_st_beq = mode; g_st_beqA = mode;
  if (real) real(mode);
}
static void my_glBlendEquationSeparate(unsigned rgb, unsigned a) {
  static void (*real)(unsigned, unsigned) = NULL;
  rgl("glBlendEquationSeparate", (void **)&real);
  g_st_beq = rgb; g_st_beqA = a;
  if (real) real(rgb, a);
}
static void my_glBlendFuncSeparate(unsigned sf, unsigned df, unsigned sa, unsigned da) {
  g_st_bsrc = sf; g_st_bdst = df; g_st_bsrcA = sa; g_st_bdstA = da;
...
static int g_depth_always = -1, g_no_deptheq = -1;
static void my_glDepthFunc(unsigned f) {
  static void (*real)(unsigned) = NULL; rgl("glDepthFunc", (void **)&real);
  g_st_depthfunc = f;
  if (envflag("COI_DEPTH_ALWAYS", &g_depth_always)) f = 0x0207; /* GL_ALWAYS */
  ...
  if (f == 0x0202 && !envflag("COI_NO_DEPTHEQ", &g_no_deptheq)) {
    static int dn = 0;
...
static void my_glEnable(unsigned cap) {
  ...
  coi_track_cap(cap, 1);
  if (envflag("COI_DEPTH_ALWAYS", &g_depth_always) && cap == 0x0B71 /*DEPTH_TEST*/) {
    const char *d = getenv("COI_DEPTH_ALWAYS");
    if (d && atoi(d) == 2) return; /* não habilita */
  }
  if (real) real(cap);
}
...
static void my_glClearColor(float r, float g, float b, float a) {
  ...
  static int ct = -1;
  const char *t = envflag("COI_CLEAR_TEST", &ct) ? getenv("COI_CLEAR_TEST") : NULL;
  if (t) {
    int m = atoi(t);
...
static void my_glTexParameteri(unsigned tgt, unsigned pname, int param) {
  ...
  static int n = 0, tplog = -1;
  if (envflag("COI_TEXPARAM_LOG", &tplog) && n < 40) {
...
static void my_glTexImage2D(unsigned tgt, int lvl, int ifmt, int w, int h,
  ...
  if (envflag("COI_TEX_HALF", &g_tex_half) && px && lvl == 0 && fmt == 0x1908 &&
  ...
  unsigned e = gerr ? gerr() : 0;
  if (envflag("COI_TEXID", &g_texid_log))
...
static void my_glGetIntegerv(unsigned pname, int *params) {
  ...
  /* 🏎️ SHADOW STATE (regra Mali: NUNCA glGet no caminho de frame). Esta engine
   * lê o estado de blend/depth de volta do driver antes de cada bloco de estado:
   * medido no R36S, 20 glGetIntegerv POR FRAME (6 de blend × 3 + depth × 2). No
   * blob Mali cada leitura dessas é round-trip síncrono no driver. Respondemos
   * dos nossos próprios shadows — os mesmos que já alimentam o [DRAWATTR] — e o
   * driver nem é chamado. Devolvemos o valor que o JOGO pediu (pré-remap do
   * COI_NO_ADD/DEPTHEQ), que é o que o cache de estado dele espera ler de volta;
   * o remap continua acontecendo no wrapper quando ele re-aplica.
   * COI_GLGET_REAL=1 volta a perguntar pro driver (bisseção). */
  if (params) {
    static int noshadow = -1;
    if (!envflag("COI_GLGET_REAL", &noshadow)) {
      switch (pname) {
      case 0x80C8: params[0] = (int)g_st_bsrc;  return; /* BLEND_SRC_RGB */
      case 0x80C9: params[0] = (int)g_st_bdst;  return; /* BLEND_DST_RGB */
      case 0x80CA: params[0] = (int)g_st_bsrcA; return; /* BLEND_SRC_ALPHA */
      case 0x80CB: params[0] = (int)g_st_bdstA; return; /* BLEND_DST_ALPHA */
      case 0x8009: params[0] = (int)g_st_beq;   return; /* BLEND_EQUATION_RGB */
      case 0x883D: params[0] = (int)g_st_beqA;  return; /* BLEND_EQUATION_ALPHA */
      case 0x0B74: params[0] = (int)g_st_depthfunc; return; /* DEPTH_FUNC */
      default: break;
      }
    }
  }
...
static void my_glShaderSource(unsigned sh, int count, const char *const *str,
  ...
  /* 🏎️ COI_MEDIUMP=1: troca TODO `highp` do fonte por `mediump`. Alavanca de
   * "menos qualidade por mais fps" explícita: no Utgard (Mali-450) o highp no
   * fragmento é emulado e no Bifrost (G31) custa o dobro de um mediump. O risco
   * é banding/artefato de precisão em UV grande ou cálculo de luz — por isso
   * fica atrás de flag e não entra ligado no release sem teste no device.
   * Só faz sentido se o [PERF] mostrar cpu baixo e o gargalo estiver na GPU. */
  {
    static int mp = -1;
    if (envflag("COI_MEDIUMP", &mp) && count == 1 && str[0] &&
        strstr(str[0], "highp")) {
      static char mb[32768];
      size_t L = (len && len[0] > 0) ? (size_t)len[0] : strlen(str[0]);
      const char *s = str[0];
      if (L * 2 < sizeof(mb) - 1) {
        size_t oi = 0; int nrep = 0;
        for (size_t i = 0; i < L; ) {
          if (i + 5 <= L && memcmp(s + i, "highp", 5) == 0) {
            memcpy(mb + oi, "mediump", 7); oi += 7; i += 5; nrep++;
          } else {
            mb[oi++] = s[i++];
          }
        }
        mb[oi] = 0;
        static int mn = 0;
        if (mn < 6) { fprintf(stderr, "[MEDIUMP] shader #%u: %d highp -> mediump\n", sh, nrep); mn++; }
        const char *p = mb; int nl = (int)oi;
        if (real) real(sh, 1, &p, &nl);
        return;
      }
    }
...
DynLibFunction coi_overrides[] = {
  ...
  {"glBlendEquation", (uintptr_t)my_glBlendEquation},
  {"glBlendEquationSeparate", (uintptr_t)my_glBlendEquationSeparate},
