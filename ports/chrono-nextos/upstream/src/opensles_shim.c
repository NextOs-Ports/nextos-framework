...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  /* DEBUG: dump do PCM de saida p/ analise no host (CHRONO_PCMDUMP=1).
     CHRONO_PCMSKIP=<segundos> pula o comeco (splash e' silencioso de fabrica)
     e mede o audio DO GAMEPLAY, que e' o que interessa. */
  if (getenv("CHRONO_PCMDUMP")) {
    static FILE *pf = NULL; static size_t cap = 0; static size_t skipped = 0;
    static size_t skip_bytes = (size_t)-1;
    if (skip_bytes == (size_t)-1) {
      const char *sk = getenv("CHRONO_PCMSKIP");
      skip_bytes = sk ? (size_t)atoi(sk) * 44100 * 2 * 2 : 0;
    }
    if (skipped < skip_bytes) skipped += out_samples * sizeof(int16_t);
    if (skipped >= skip_bytes && !pf) {
      char dumppath[1024];
      snprintf(dumppath, sizeof dumppath, "%s/out.pcm", ct_gamedir());
      pf = fopen(dumppath, "wb"); cap = 0;
    }
