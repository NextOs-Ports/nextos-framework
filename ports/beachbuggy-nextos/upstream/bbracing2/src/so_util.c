      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        case R_AARCH64_ABS64: {
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (!resolved) {
              // GLES2 nao esta' na tabela estatica: o binario nao declara
              // DT_NEEDED de GL. A engine recebe as funcoes de despacho
              // estaveis do nxgl, resolvidas depois que ELA criar o contexto.
              resolved = (uintptr_t)nxgl_gles2_dispatch_lookup(name);
            }
            if (!resolved) {
...
uintptr_t so_find_addr(const char *symbol) {
  ...
  if (!addr) {
    fatal_error("Error: could not find symbol: %s", symbol);
  }
  return addr;
}
...
DynLibFunction *so_snapshot_symbols(int *out_count) {
  ...
  for (int i = 0; i < num_syms; i++) {
    ...
    if (syms[i].st_name == 0)
      continue;
    tbl[k].symbol = strdup(dynstrtab + syms[i].st_name); // .dynstr permanece mapeado
