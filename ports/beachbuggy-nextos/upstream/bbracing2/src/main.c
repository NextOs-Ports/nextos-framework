...
/* Android/Bionic keeps ai_canonname before ai_addr, while glibc keeps those
 * two pointers in the opposite order.  Passing glibc's list directly makes
 * Android libcurl treat the address as an empty canonical name. */
...
static void my_freeaddrinfo(struct bionic_addrinfo *list) {
  while (list) {
...
static int my_getaddrinfo(const char *node, const char *service,
  ...
  const struct addrinfo *host_hints_ptr = NULL;
  if (android_hints) {
    memset(&host_hints, 0, sizeof(host_hints));
    host_hints.ai_flags = android_hints->ai_flags;
    host_hints.ai_family = android_hints->ai_family;
    host_hints.ai_socktype = android_hints->ai_socktype;
    host_hints.ai_protocol = android_hints->ai_protocol;
    host_hints_ptr = &host_hints;
  }

  struct addrinfo *host_result = NULL;
  const int rc = getaddrinfo(node, service, host_hints_ptr, &host_result);
  logPrintf("[net] getaddrinfo('%s','%s') -> %d (%s)\n",
            node ? node : "", service ? service : "", rc,
            rc == 0 ? "ok" : gai_strerror(rc));
  if (rc != 0) return rc;

  struct bionic_addrinfo *head = NULL;
  struct bionic_addrinfo **tail = &head;
  for (const struct addrinfo *src = host_result; src; src = src->ai_next) {
    struct bionic_addrinfo *dst = calloc(1, sizeof(*dst));
    if (!dst) {
      freeaddrinfo(host_result);
      my_freeaddrinfo(head);
      return EAI_MEMORY;
    }
    ...
    if (src->ai_canonname) dst->ai_canonname = strdup(src->ai_canonname);
    if (src->ai_addr && src->ai_addrlen) {
      dst->ai_addr = malloc(src->ai_addrlen);
      if (dst->ai_addr) memcpy(dst->ai_addr, src->ai_addr, src->ai_addrlen);
    }
    *tail = dst;
    tail = &dst->ai_next;
  }
...
static bool input_trace_enabled(void) {
  ...
  if (enabled < 0) {
    const char *value = getenv("BB_INPUT_TRACE");
    enabled = value && *value && *value != '0';
  }
  return enabled != 0;
}

static SDL_JoystickID *my_SDL_GetJoysticks(int *count) {
  SDL_JoystickID *ids = SDL_GetJoysticks(count);
  if (input_trace_enabled()) {
    const int total = count ? *count : 0;
    logPrintf("[input] SDL_GetJoysticks -> %d\n", total);
    for (int i = 0; ids && i < total; ++i) {
      logPrintf("[input] joystick id=%u name='%s' gamepad=%d\n",
                (unsigned)ids[i],
...
static bool my_SDL_IsGamepad(SDL_JoystickID id) {
  const bool result = SDL_IsGamepad(id);
  if (input_trace_enabled())
    logPrintf("[input] SDL_IsGamepad(%u) -> %d\n", (unsigned)id, result);
  return result;
}

static SDL_Gamepad *my_SDL_OpenGamepad(SDL_JoystickID id) {
  SDL_Gamepad *gamepad = SDL_OpenGamepad(id);
  if (input_trace_enabled())
    logPrintf("[input] SDL_OpenGamepad(%u) -> %p name='%s'\n",
              (unsigned)id, (void *)gamepad,
              SDL_GetGamepadNameForID(id) ? SDL_GetGamepadNameForID(id) : "");
  return gamepad;
}

static SDL_GamepadType my_SDL_GetGamepadType(SDL_Gamepad *gamepad) {
  const SDL_GamepadType detected = SDL_GetGamepadType(gamepad);
  if (input_trace_enabled())
    logPrintf("[input] SDL_GetGamepadType detected=%d -> Xbox360\n",
              (int)detected);
  return SDL_GAMEPAD_TYPE_XBOX360;
}

static bool my_SDL_GetGamepadButton(SDL_Gamepad *gamepad,
                                    SDL_GamepadButton button) {
  const bool pressed = SDL_GetGamepadButton(gamepad, button);
  cursor_bridge_track_button(button, pressed);
  static bool known[SDL_GAMEPAD_BUTTON_COUNT];
  static bool previous[SDL_GAMEPAD_BUTTON_COUNT];
  if (input_trace_enabled() && button >= 0 &&
      button < SDL_GAMEPAD_BUTTON_COUNT &&
      (!known[button] || previous[button] != pressed)) {
    known[button] = true;
    previous[button] = pressed;
    if (pressed)
      logPrintf("[input] polled button=%d DOWN\n", (int)button);
  }
  return pressed;
}

static Sint16 my_SDL_GetGamepadAxis(SDL_Gamepad *gamepad,
                                    SDL_GamepadAxis axis) {
  const Sint16 value = SDL_GetGamepadAxis(gamepad, axis);
  cursor_bridge_track_axis(axis, value);
  return value;
}
...
static bool my_SDL_PollEvent(SDL_Event *event) {
  ...
  if (result) {
    cursor_bridge_handle_event(event);
    if (input_trace_enabled()) {
      if (event->type == SDL_EVENT_GAMEPAD_ADDED ||
          event->type == SDL_EVENT_GAMEPAD_REMOVED) {
        logPrintf("[input] gamepad device event type=0x%x which=%u\n",
                  event->type, (unsigned)event->gdevice.which);
      } else if (event->type == SDL_EVENT_GAMEPAD_BUTTON_DOWN ||
                 event->type == SDL_EVENT_GAMEPAD_BUTTON_UP) {
        logPrintf("[input] gamepad button which=%u button=%u %s\n",
                  (unsigned)event->gbutton.which,
                  (unsigned)event->gbutton.button,
                  event->type == SDL_EVENT_GAMEPAD_BUTTON_DOWN ? "DOWN" : "UP");
      } else if (event->type == SDL_EVENT_FINGER_DOWN ||
                 event->type == SDL_EVENT_FINGER_UP ||
                 event->type == SDL_EVENT_FINGER_MOTION) {
        logPrintf("[input] finger type=0x%x x=%.3f y=%.3f pressure=%.3f\n",
                  event->type, event->tfinger.x, event->tfinger.y,
                  event->tfinger.pressure);
      } else if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN ||
                 event->type == SDL_EVENT_MOUSE_BUTTON_UP) {
        logPrintf("[input] mouse button type=0x%x button=%u x=%.1f y=%.1f\n",
                  event->type, (unsigned)event->button.button,
                  event->button.x, event->button.y);
      } else if (event->type == SDL_EVENT_MOUSE_MOTION) {
        logPrintf("[input] mouse motion x=%.1f y=%.1f\n",
                  event->motion.x, event->motion.y);
      }
    }
    track_exit_gamepad_event(event);
  }
  return result;
}
...
static DynLibFunction base_symbols[] = {
  ...
  { "SDL_GetJoysticks", (uintptr_t)my_SDL_GetJoysticks },
  ...
  { "SDL_IsGamepad", (uintptr_t)my_SDL_IsGamepad },
  { "SDL_OpenGamepad", (uintptr_t)my_SDL_OpenGamepad },
  { "SDL_GetGamepadType", (uintptr_t)my_SDL_GetGamepadType },
  { "SDL_CloseGamepad", (uintptr_t)SDL_CloseGamepad },
  { "SDL_GamepadConnected", (uintptr_t)SDL_GamepadConnected },
  { "SDL_GetGamepadButton", (uintptr_t)my_SDL_GetGamepadButton },
  { "SDL_GetGamepadAxis", (uintptr_t)my_SDL_GetGamepadAxis },
...
int main(int argc, char *argv[]) {
  ...
  logPrintf("\n=== Beach Buggy Racing 2 (Vector Unit) — NextOS ===\n");
  ...
  uintptr_t gs_signin = so_find_addr_safe("Java_com_vectorunit_VuGameServicesHelper_nativeOnSignInSuccess");
