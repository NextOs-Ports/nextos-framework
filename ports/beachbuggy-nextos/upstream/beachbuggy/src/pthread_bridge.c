...
/* Bionic aceita pilhas menores que o mínimo da glibc aarch64. O FMOD usa
 * pthread_attr_setstacksize para cada thread e converte EINVAL em
 * FMOD_ERR_INTERNAL, abortando System::init. Preserve o atributo real e eleve
 * apenas os pedidos Android abaixo do mínimo informado pelo host. */
int b_attr_setstacksize(void *attr, size_t requested) {
  long minimum = sysconf(_SC_THREAD_STACK_MIN);
  if (minimum <= 0)
    minimum = 128 * 1024;

  size_t actual = requested;
  if (actual < (size_t)minimum)
    actual = (size_t)minimum;

  int rc = pthread_attr_setstacksize((pthread_attr_t *)attr, actual);
  if (actual != requested) {
    fprintf(stderr,
            "[pthread] stack Android %zu -> minimo glibc %zu (rc=%d)\n",
            requested, actual, rc);
  }
  return rc;
}
...
// Extra stubs for condattr and rwlocks
int b_condattr_init(void *a) { if (a) *(int *)a = 0; return 0; }
int b_condattr_destroy(void *a) { (void)a; return 0; }
int b_rwlockattr_init(void *a) { if (a) *(int *)a = 0; return 0; }
int b_rwlockattr_destroy(void *a) { (void)a; return 0; }
int b_rwlock_init(void *r, const void *attr) { (void)attr; rw_real(r); return 0; }
int b_rwlock_destroy(void *r) {
  pthread_mutex_lock(&g_lock);
  pthread_rwlock_t **slot = (pthread_rwlock_t **)r;
  if (*slot) {
...
int b_rwlock_tryrdlock(void *r) { return pthread_rwlock_tryrdlock(rw_real(r)); }
int b_rwlock_trywrlock(void *r) { return pthread_rwlock_trywrlock(rw_real(r)); }
