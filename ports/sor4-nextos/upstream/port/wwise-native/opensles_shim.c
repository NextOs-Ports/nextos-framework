...
static FILE *opensles_log_open(void) {
  const char *path = getenv("WWISE_LOG");
  if (!path || !*path) path = "/tmp/sor4-wwise.log";
  return fopen(path, "a");
}

static pthread_once_t g_opensles_diag_once = PTHREAD_ONCE_INIT;
static int g_opensles_diag_enabled = 0;

static void opensles_diag_init(void) {
  const char *value = getenv("SOR4_NATLOG");
  g_opensles_diag_enabled = (value && value[0] == '1') ? 1 : 0;
}

static int opensles_diag(void) {
  pthread_once(&g_opensles_diag_once, opensles_diag_init);
  return g_opensles_diag_enabled;
}
...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  // Peak/click scans traverse the whole mix again. Keep them completely off in
  // normal gameplay; SOR4_NATLOG=1 restores the diagnostic path when requested.
  if (diag && out_samples >= 2) {
    ...
    /* pico de saida (out) p/ diagnostico silencio-vs-som */
    {
      int32_t outpeak = 0;
      for (int s = 0; s < out_samples; s++) {
        int32_t sample = (int32_t)out[s];
        int32_t peak = sample < 0 ? -sample : sample;
        if (peak > outpeak) outpeak = peak;
      }
      if (callback_count % 200 == 0) {
        FILE* lf=opensles_log_open();
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (written > 0) {
        ...
        p->enqueue_counter++;
        p->ever_enqueued = 1;
        if (opensles_diag() && size >= 16 &&
            (p->enqueue_counter == 1 || p->enqueue_counter % 300 == 0)) {
          const int16_t* sp=(const int16_t*)pBuffer; int32_t pk=0; uint32_t ns=size/2;
          for(uint32_t k=0;k<ns;k++){
            int32_t sample=(int32_t)sp[k]; int32_t peak=sample<0?-sample:sample;
            if(peak>pk)pk=peak;
          }
...
SLresult slCreateEngine_shim(void **pEngine, SLuint32 numOptions,
  ...
  { FILE* f=opensles_log_open(); if(f){fprintf(f,"[opensles] slCreateEngine chamado\n"); fclose(f);} }
  init_engine();
