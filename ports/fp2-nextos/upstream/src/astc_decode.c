 ...
 *
 * Reused from the approved Horizon Chase v1.2.2 snapshot
 * 59d2d38dc496ae0a71726181d3ccc80923e4144d.  Its identical decoder/loader
 * line was physically validated on Mali-450 through a complete race and is a
 * current-public positive reference in framework/catalog/ports-v1.json.
 *
 * The decoder itself is Arm's astcenc library, shipped as libastcUtil.so. This
 * bridge keeps the Unity host independent of the C++ ABI except for the four
 * stable entry points used by that proven decoder.
 */
#include <dlfcn.h>
#include <pthread.h>
#include <stddef.h>
...
static int astc_load_library(void)
{
    ...
    if (!handle) {
        fprintf(stderr, "[fp2/astc] cannot load libastcUtil.so: %s\n",
                dlerror());
        g_load_state = -1;
        return -1;
    }
    ...
    if (!p_config_init || !p_context_alloc || !p_decompress_image) {
        fprintf(stderr,
                "[fp2/astc] missing symbols cfg=%p ctx=%p dec=%p reset=%p\n",
    ...
    g_load_state = 1;
    fprintf(stderr, "[fp2/astc] Arm astcenc decoder loaded\n");
    return 0;
}
...
static astcenc_context *astc_get_context(int block_x, int block_y)
{
    ...
    int result = p_config_init(ASTCENC_PRF_LDR, (unsigned)block_x,
                               (unsigned)block_y, 1, ASTCENC_PRE_FASTEST,
                               ASTCENC_FLG_DECOMPRESS_ONLY, config);
    if (result) {
        fprintf(stderr, "[fp2/astc] config %dx%d failed: %d\n",
                block_x, block_y, result);
        return NULL;
    }
    ...
    if (result || !context) {
        fprintf(stderr, "[fp2/astc] context %dx%d failed: %d\n",
                block_x, block_y, result);
        return NULL;
    }
    g_contexts[block_x][block_y] = context;
    fprintf(stderr, "[fp2/astc] context %dx%d ready\n", block_x, block_y);
    return context;
}
                     ...
                     int height, int block_x, int block_y)
{
    ...
    /* Some astcenc versions write complete edge blocks. Decode to aligned
     * storage, then copy the exact Unity mip dimensions. */
    int padded_width = blocks_x * block_x;
    int padded_height = blocks_y * block_y;
    if ((size_t)padded_width > SIZE_MAX / 4 / (size_t)padded_height) {
        pthread_mutex_unlock(&g_astc_lock);
        return -8;
    }
    size_t padded_size =
    ...
    int result = p_decompress_image(
        context, source, expected_source, &image, &swizzle, 0);
    if (p_decompress_reset)
        p_decompress_reset(context);
    pthread_mutex_unlock(&g_astc_lock);

    if (result) {
        fprintf(stderr, "[fp2/astc] decode %dx%d/%dx%d failed: %d\n",
                width, height, block_x, block_y, result);
        free(padded);
        return -9;
    }
    ...
    for (int y = 0; y < height; y++) {
        memcpy(output + (size_t)y * (size_t)width * 4,
