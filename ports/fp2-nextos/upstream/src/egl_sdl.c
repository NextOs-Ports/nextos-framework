...
static uintptr_t next_context_id = 1;
...
static _Thread_local fp2_sdl_context *current_context;

static int positive_env(const char *name);

/* Grava o backbuffer real como PPM. Usado pela captura automática de bancada
 * (FP2_SCREENSHOT, só fora do release) e pelo shot sob demanda do pad virtual
 * (fp2_shot_request, compilado sempre mas inerte sem pedido). */
static void write_backbuffer_ppm(const char *path)
{
...
/* Shot sob demanda do pad virtual de bancada (token `shot`, FP2_VPAD=1):
 * shot_NNN.ppm em FP2_SHOT_DIR ou no gamedir.  Sem pedido nada acontece. */
extern int fp2_shot_request;
extern char fp2_gamedir[1024];
static void capture_on_demand_if_requested(void)
{
    ...
    if (!dir || !*dir)
        dir = fp2_gamedir;
    char path[1200];
    snprintf(path, sizeof path, "%.1024s/shot_%03u.ppm", dir, ++shot_seq);
    write_backbuffer_ppm(path);
}
...
static void capture_frame_if_requested(void)
{
    ...
    if (finished)
        return;
    const char *path = getenv("FP2_SCREENSHOT");
    if (!path || !*path)
        return;
    int wanted = positive_env("FP2_SCREENSHOT_FRAME");
    if (!wanted)
        wanted = 300;
...
{
    int width = positive_env("FP2_SCREEN_W");
    int height = positive_env("FP2_SCREEN_H");
    ...
    fprintf(stderr, "[fp2/video] resolution %dx%d (%s)\n",
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Freedom Planet 2", SDL_WINDOWPOS_CENTERED,
            SDL_WINDOWPOS_CENTERED, screen_width, screen_height,
            SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN_DESKTOP);
        if (!video_window) {
            fprintf(stderr,
                    "[fp2/video] SDL window depth%d/stencil%d failed: %s\n",
        ...
        if (share_root && !context_is_gles()) {
            fprintf(stderr,
                    "[fp2/video] SDL returned desktop GL; rejecting it\n");
        ...
        if (share_root) {
            ...
            alpha_size = a;
            fprintf(stderr,
                    "[fp2/video] SDL config: R%dG%dB%dA%d depth%d stencil%d\n",
                    r, g, b, a, depth_size, stencil_size);
            break;
        }

        fprintf(stderr,
                "[fp2/video] GLES2 depth%d/stencil%d failed: %s\n",
                formats[i].depth, formats[i].stencil, SDL_GetError());
    ...
    const GLubyte *(*get_string)(GLenum) =
        (const GLubyte *(*)(GLenum))SDL_GL_GetProcAddress("glGetString");
    fprintf(stderr,
            "[fp2/video] SDL GLES2 depth%d/stencil%d drawable=%dx%d\n",
            depth_size, stencil_size, screen_width, screen_height);
    if (get_string) {
        fprintf(stderr, "[fp2/video] GL_VENDOR=%s\n",
                get_string(GL_VENDOR) ?
                    (const char *)get_string(GL_VENDOR) : "?");
        fprintf(stderr, "[fp2/video] GL_RENDERER=%s\n",
                get_string(GL_RENDERER) ?
                    (const char *)get_string(GL_RENDERER) : "?");
        fprintf(stderr, "[fp2/video] GL_VERSION=%s\n",
    ...
    nxgl_frame_proof_set_video_context(
        ...
        get_string && get_string(GL_RENDERER)
            ? (const char *)get_string(GL_RENDERER) : NULL,
        get_string && get_string(GL_VERSION)
            ? (const char *)get_string(GL_VERSION) : NULL);
    fp2_graphics_contract_set_sdl_window(video_window);
...
int fp2_sdl_video_init(void)
{
    ...
    int force_sdl = env_on("FP2_PURE_SDL_CONTEXTS");
#ifdef FP2_RELEASE_BUILD
    /* The public artifact was proven only through SDL-owned contexts.  Make
     * that ownership a property of the executable itself, so a missing hook
     * or hostile environment cannot reopen the unproved raw-EGL route. */
    force_sdl = 1;
#endif

    if (env_on("FP2_RAW_EGL_CONTEXTS")) {
        if (force_sdl)
            nx_die("FP2 public release forbids the raw EGL override");
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr, "[fp2/video] raw EGL selected by override\n");
        return 0;
    }
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        if (force_sdl)
            nx_die("FP2 public release requires SDL video: %s",
                   SDL_GetError());
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr,
                "[fp2/video] SDL video unavailable (%s); using raw EGL\n",
                SDL_GetError());
        return 0;
    }
    ...
    fprintf(stderr, "[fp2/video] probing SDL-owned contexts on backend '%s'%s\n",
    ...
    fprintf(stderr,
            "[fp2/video] SDL GLES2 probe failed on backend '%s'; trying raw EGL\n",
            driver ? driver : "?");
    if (!was_initialized)
...
{
    fp2_sdl_surface *surface = calloc(1, sizeof *surface);
    if (!surface)
...
{
    (void)display;
    fp2_sdl_surface *surface = (fp2_sdl_surface *)handle;
                                       ...
                                       const EGLint *attributes)
{
    ...
    (void)share;
    (void)attributes;
    fp2_sdl_context *context = calloc(1, sizeof *context);
    if (!context)
    ...
    context->id = __atomic_fetch_add(&next_context_id, 1,
                                     __ATOMIC_RELAXED);
    nx_log("SDL EGL context #%" PRIuPTR " -> %p", context->id,
           (void *)context);
...
{
    (void)display;
    fp2_sdl_context *context = (fp2_sdl_context *)handle;
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    ...
    fp2_sdl_context *context = (fp2_sdl_context *)handle;
    fp2_sdl_surface *surface = (fp2_sdl_surface *)draw;
    if (!context || context->magic != CONTEXT_MAGIC ||
    ...
    if (SDL_GL_MakeCurrent(video_window, context->context) != 0) {
        nx_log("SDL EGL MakeCurrent context #%" PRIuPTR " failed: %s",
               context->id, SDL_GetError());
        return EGL_FALSE;
    }
    current_context = context;
    if (surface->is_window &&
        !fp2_graphics_contract_pre_present((uintptr_t)context->id))
        nx_die("live GLES/ESSL100 graphics pre-present contract failed");
    return EGL_TRUE;
}
...
EGLBoolean fp2_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    (void)display;
    ...
    capture_on_demand_if_requested();
    /* This is the SDL path's real present boundary.  A common call in
     * my_eglSwapBuffers would be too early because the guards above can
     * legitimately suppress SDL_GL_SwapWindow. */
    nxgl_frame_proof_before_present(screen_width, screen_height);
    SDL_GL_SwapWindow(video_window);
    if (!fp2_graphics_contract_after_present(
            (uintptr_t)current_context->id))
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Freedom Planet 2 SDL EGL";
