/*
 * Unity 6 only ships an ESSL300 graphics backend.  Stranger Things's compiled
...
static const char *texture_replacement(enum fp2_shader_stage stage,
                                       enum sampler_kind kind, int lod,
                                       char *reason, size_t reason_size)
{
    if (stage == FP2_SHADER_STAGE_VERTEX) {
...
static int rewrite_tokens(const text_buf *body, enum fp2_shader_stage stage,
                          ...
                          char *reason, size_t reason_size)
{
    ...
    for (size_t i = 0; i < length;) {
        ...
        if (word_equal(source + begin, token_len, "roundEven")) {
            if (!buf_add(result, "fp2_roundEven")) return 0;
            continue;
        }
...
static int add_preamble(text_buf *output, const char *source, size_t length)
{
    ...
    if (!buf_add(output,
                 ...
                 "precision highp int;\n"
                 "#define FP2_UNITY6_GLES2 1\n"
                 "int fp2_maxi(int a, int b) { return a > b ? a : b; }\n"
                 "int fp2_mini(int a, int b) { return a < b ? a : b; }\n"))
        return 0;
    if (contains_word(source, length, "roundEven")) {
        static const char helper[] =
            "highp float fp2_roundEven(highp float x) {\n"
            "    highp float f = floor(x);\n"
            "    highp float d = x - f;\n"
            "    return d < 0.5 ? f : (d > 0.5 ? f + 1.0 : "
            "(mod(f, 2.0) == 0.0 ? f : f + 1.0));\n"
            "}\n"
            "highp vec2 fp2_roundEven(highp vec2 x) { return vec2("
            "fp2_roundEven(x.x), fp2_roundEven(x.y)); }\n"
            "highp vec3 fp2_roundEven(highp vec3 x) { return vec3("
            "fp2_roundEven(x.x), fp2_roundEven(x.y), fp2_roundEven(x.z)); }\n"
            "highp vec4 fp2_roundEven(highp vec4 x) { return vec4("
            "fp2_roundEven(x.x), fp2_roundEven(x.y), fp2_roundEven(x.z), "
            "fp2_roundEven(x.w)); }\n";
...
static int reject_known_unsupported(enum fp2_shader_stage stage,
                                  ...
                                  char *reason, size_t reason_size)
{
    ...
    if (!source || !translated || !translated_len ||
        (stage != FP2_SHADER_STAGE_VERTEX &&
         stage != FP2_SHADER_STAGE_FRAGMENT)) {
        set_reason(reason, reason_size, "invalid translator arguments");
        return FP2_SHADER_UNSUPPORTED;
    }
    ...
    while (position < source_len) {
        ...
        if (stage == FP2_SHADER_STAGE_FRAGMENT &&
        ...
        if (depth == 0) {
            ...
            int is_attr = starts_word(at, "ATTRIBUTE_IN") || starts_word(at, "attribute");
            int is_in = starts_word(at, "in") || starts_word(at, "VARYING_IN");
            int is_out = starts_word(at, "out") || starts_word(at, "VARYING_OUT");
            if (is_attr || is_in || is_out) {
                size_t qualifier_len = 0;
                if (starts_word(at, "ATTRIBUTE_IN")) qualifier_len = strlen("ATTRIBUTE_IN");
                else if (starts_word(at, "attribute")) qualifier_len = strlen("attribute");
                else if (starts_word(at, "VARYING_IN")) qualifier_len = strlen("VARYING_IN");
                else if (starts_word(at, "VARYING_OUT")) qualifier_len = strlen("VARYING_OUT");
                else if (starts_word(at, "out")) qualifier_len = 3;
                else if (starts_word(at, "in")) qualifier_len = 2;

                if (stage == FP2_SHADER_STAGE_FRAGMENT && is_out) {
                    char name[96];
                ...
                if (stage == FP2_SHADER_STAGE_VERTEX && (is_attr || (is_in && !is_out))) {
                    char attr_name[64];
                    if (last_identifier_before_semicolon(at, attr_name, sizeof attr_name)) {
                        if (attribs &&
                            attribs->count < sizeof attribs->bindings / sizeof *attribs->bindings) {
                            attribs->bindings[attribs->count].location = location;
                            snprintf(attribs->bindings[attribs->count].name,
                                     sizeof attribs->bindings[attribs->count].name,
                                     "%s", attr_name);
                            attribs->count++;
                        }
                    }
    ...
    if (stage == FP2_SHADER_STAGE_FRAGMENT && output_location > 0) {
    ...
    /* Nao inventar location de atributo.  Os vertex shaders do HLSLcc deste
     * jogo declaram `in vec4 in_POSITION0;` sem `layout(location=)`, entao
     * qualquer numeracao aqui e' chute: ela sobrescrevia o bind da propria
     * Unity e embaralhava COLOR/TEXCOORD, apagando a arte do menu e os icones
     * do HUD.  As locations parseadas seguem so' como diagnostico. */
    free(body.data);
    *translated = output.data;
    *translated_len = output.len;
    return FP2_SHADER_TRANSLATED;
...
int fp2_unity6_translate_shader(enum fp2_shader_stage stage,
                               ...
                               char *reason, size_t reason_size)
{
    return fp2_unity6_translate_shader_ex(stage, source, source_len,
                                         translated, translated_len,
                                         NULL, reason, reason_size);
}
