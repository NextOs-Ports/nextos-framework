 ...
 *
 * The list this file covers was MEASURED, not guessed: fp2_gl_sym() logs every
...
extern GLuint fp2_gles3_draw_framebuffer(void);
extern GLuint fp2_gles3_read_framebuffer(void);
extern void fp2_gles3_texture_storage_2d(GLenum target, GLsizei levels,
...
static int gl3_trace_enabled(void)
{
    ...
    if (enabled < 0) {
        const char *value = getenv("FP2_GLES3_TRACE");
                            ...
                            GLint value)
{
    if (gl3_trace_enabled())
        fprintf(stderr, "[fp2/gles3] %s object=%u pname=0x%x -> %d\n",
 ...
 * textures, but Stranger Things's measured runtime shader set has no sampler2DArray.
...
GLenum fp2_gles3_texture_target(GLenum target)
{
    return target == GL_TEXTURE_2D_ARRAY ? GL_TEXTURE_2D : target;
}

GLenum fp2_gles3_texture_format(GLenum format)
{
    switch (format) {
...
static void gl3_TexImage3D(GLenum target, GLint level, GLint internal,
                           ...
                           const void *pixels)
{
    GLenum physical = fp2_gles3_texture_target(target);
    GLenum es2_format = fp2_gles3_texture_format(format);
    GLenum es2_internal = fp2_gles3_texture_format(internal);
...
static void gl3_CopyTexSubImage3D(GLenum target, GLint level, GLint xoffset,
...
static void gl3_CompressedTexImage3D(GLenum target, GLint level,
                                     ...
                                     const void *data)
{
    (void)target; (void)level; (void)internal; (void)width; (void)height;
    (void)depth; (void)border; (void)image_size; (void)data;
}
...
{
    (void)target; (void)level; (void)xoffset; (void)yoffset; (void)zoffset;
    (void)width; (void)height; (void)depth; (void)format; (void)image_size;
    (void)data;
}
                             ...
                             GLsizei w, GLsizei h)
{
    ...
    GLenum physical = fp2_gles3_texture_target(target);
    GLenum fmt = fp2_gles3_texture_format(internal);
    GLenum type = 0x1401 /* GL_UNSIGNED_BYTE */;
    ...
    fp2_gles3_texture_storage_2d(target, levels, internal, w, h);
...
static void gl3_TexStorage3D(GLenum target, GLsizei levels, GLenum internal,
                             GLsizei width, GLsizei height, GLsizei depth)
{
    GLenum format = fp2_gles3_texture_format(internal);
    if (format == internal)
        format = 0x1908;
                                ...
                                GLbitfield mask, GLenum filter)
{
    ...
    if (gl3_trace_enabled() && calls <= 64)
        fprintf(stderr,
                "[fp2/gles3] blit #%lu src=%d,%d..%d,%d "
                "dst=%d,%d..%d,%d mask=%#x filter=%#x\n",
                calls, src_x0, src_y0, src_x1, src_y1,
                dst_x0, dst_y0, dst_x1, dst_y1, mask, filter);
}

/* ES3 clear-buffer calls select the attachment directly.  Stranger Things has one
...
void fp2_gles3_texture_bound(GLenum active_texture, GLenum target,
                            GLuint texture)
{
...
static const GLubyte *gl3_GetString(GLenum name)
{
    static const GLubyte version[] =
        "OpenGL ES 3.0 Stranger Things GLES2 bridge";
    static const GLubyte shading[] =
        "OpenGL ES GLSL ES 3.00 Stranger Things GLES2 bridge";
    if (name == GL_VERSION)
        return version;
 ...
 * Stranger Things GLES3 shaders carry Unity's loose-uniform branch, which the
...
static void gl3_GetIntegerv(GLenum pname, GLint *data)
{
    ...
    case GL_DRAW_FRAMEBUFFER_BINDING:
        value = (GLint)fp2_gles3_draw_framebuffer();
        break;
    case GL_READ_FRAMEBUFFER_BINDING:
        value = (GLint)fp2_gles3_read_framebuffer();
        break;
    ...
    default: {
        ...
        if (gl3_trace_enabled())
            fprintf(stderr, "[fp2/gles3] glGetIntegerv passthrough "
                    "pname=0x%x -> %d\n", pname, *data);
        return;
    }
    }
...
{
    if (gl3_trace_enabled())
        fprintf(stderr, "[fp2/gles3] glGetUniformBlockIndex program=%u "
                                    ...
                                    GLint *params)
{
    ...
    if (gl3_trace_enabled())
        fprintf(stderr, "[fp2/gles3] glGetActiveUniformsiv program=%u "
                "count=%d pname=0x%x\n", program, count, pname);
}
...
{
    if (gl3_trace_enabled())
        fprintf(stderr, "[fp2/gles3] ignored glBindBufferBase "
...
{
    if (gl3_trace_enabled())
        fprintf(stderr, "[fp2/gles3] ignored glBindBufferRange "
...
{
    if (gl3_trace_enabled())
        fprintf(stderr, "[fp2/gles3] ignored glUniformBlockBinding "
...
void *fp2_gles3_sym(const char *name)
{
    if (!name)
        return NULL;
...
void *fp2_gles3_override_sym(const char *name)
{
    if (name) {
...
void *fp2_gles3_fallback(const char *name)
{
    ...
    if (!known && seen_n < 256) {
        ...
        fprintf(stderr, "[fp2/gl] STUB %s (returns 0)\n", name);
        fflush(stderr);
    }
