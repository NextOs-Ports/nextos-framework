#!/usr/bin/env python3
"""Install FP2's exact, policy-approved GLES2 shader translations."""
...
from shader_core import (  # noqa: E402
    GPU_SPIRV,
    PLATFORM_VULKAN,
    blob_contexts,
...
class ApprovedSkip(RuntimeError):
    """A task whose exact Vulkan program is pinned as unavailable on GLES2."""
...
# number and the running character in the tutorial are the two physical
# witnesses.
#
# Keep the exception deliberately narrow by shader name and by the translated
# input's ESSL structure.  NXExtract authenticates the source tree, and its
# post-hook checkpoint authenticates the complete transformed tree.
...
# Exact measured policy for the NXExtract-authenticated payload.  These omissions
...
def correct_backend_fragment(shader_name, fragment):
    ...
    required_fragments = (
        "#version 100\n",
        "texture2D(_MainTex, vs_TEXCOORD1)",
        "texture2D(_AlphaTex, vs_TEXCOORD1).x",
        ".yzwx * vs_TEXCOORD0",
        "gl_FragData[0]",
        "void main()",
    )
    if ("\0" in fragment or
            any(fragment.count(token) < 1 for token in required_fragments) or
            fragment.count(".yzwx * vs_TEXCOORD0") != 1):
        raise ValueError(
            "unexpected translated stencil-sprite structure for %s" % shader_name)
...
class ExactInjector:
    ...
    def validate_correction(self, shader_name, vertex, fragment):
        """Validate the narrow StencilSprite correction without host tools.

        NXExtract authenticates the input source tree and the post-hook tree.
        These structural checks catch truncated or misrouted translator output
        before it reaches the serialized file.
        """
        identity = (vertex, fragment)
        if identity in self.validated_corrections:
            return
        for stage, source in (("vertex", vertex), ("fragment", fragment)):
            if not source.startswith("#version 100\n"):
                raise ValueError("%s correction is not ESSL100 for %s" %
                                 (stage, shader_name))
            if "void main" not in source or "\0" in source:
                raise ValueError("invalid %s correction for %s" %
                                 (stage, shader_name))
        self.validated_corrections.add(identity)
