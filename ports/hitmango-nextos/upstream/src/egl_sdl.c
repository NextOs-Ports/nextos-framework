...
static _Thread_local hgo_sdl_context *current_context;
static int portable_provider_retry_active;

static int positive_env(const char *name);

/* A few KMSDRM firmwares expose the real GPU provider only through the
 * unversioned runtime names.  Keep the firmware/default selection
 * authoritative and try these names once only after that attempt fails.
 * An explicit provider -- even one explicit half of the pair -- always wins. */
static int enable_portable_provider_retry(void)
{
    ...
    portable_provider_retry_active = 1;
    fprintf(stderr,
            "[hgo/video] retrying portable EGL/GLES provider names\n");
    return 1;
}

static void drop_portable_provider_retry(void)
{
    if (!portable_provider_retry_active)
        return;
    unsetenv("SDL_VIDEO_EGL_DRIVER");
    unsetenv("SDL_VIDEO_GL_DRIVER");
    portable_provider_retry_active = 0;
}

static void copy_sdl_error(char *output, size_t output_size)
{
    const char *error = SDL_GetError();
    snprintf(output, output_size, "%s", error && *error ? error : "unknown");
}

static int reinitialize_video_with_portable_providers(void)
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) == 0)
        return 1;
    fprintf(stderr,
            "[hgo/video] portable EGL/GLES provider retry failed: %s\n",
            SDL_GetError());
...
static void capture_frame_if_requested(void)
{
    ...
    if (!path || !*path)
        return;
    int wanted = positive_env("HGO_SCREENSHOT_FRAME");
    if (!wanted)
        wanted = 300;
...
{
    int width = positive_env("HGO_SCREEN_W");
    int height = positive_env("HGO_SCREEN_H");
    ...
    fprintf(stderr, "[hgo/video] resolution %dx%d (%s)\n",
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        if (!video_window) {
            fprintf(stderr,
                    "[hgo/video] SDL window depth%d/stencil%d failed: %s\n",
        ...
        if (share_root && !context_is_gles()) {
            fprintf(stderr,
                    "[hgo/video] SDL returned desktop GL; rejecting it\n");
        ...
        fprintf(stderr,
                "[hgo/video] GLES2 depth%d/stencil%d failed: %s\n",
                formats[i].depth, formats[i].stencil, SDL_GetError());
    ...
    const GLubyte *(*get_string)(GLenum) =
        (const GLubyte *(*)(GLenum))SDL_GL_GetProcAddress("glGetString");
    fprintf(stderr,
            "[hgo/video] SDL GLES2 depth%d/stencil%d drawable=%dx%d\n",
            depth_size, stencil_size, screen_width, screen_height);
    if (get_string) {
        fprintf(stderr, "[hgo/video] GL_VENDOR=%s\n",
                get_string(GL_VENDOR) ?
                    (const char *)get_string(GL_VENDOR) : "?");
        fprintf(stderr, "[hgo/video] GL_RENDERER=%s\n",
                get_string(GL_RENDERER) ?
                    (const char *)get_string(GL_RENDERER) : "?");
        fprintf(stderr, "[hgo/video] GL_VERSION=%s\n",
...
int hgo_sdl_video_init(void)
{
    ...
    if (env_on("HGO_RAW_EGL_CONTEXTS")) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr, "[hgo/video] raw EGL selected by override\n");
        return 0;
    }
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        ...
        copy_sdl_error(initial_error, sizeof initial_error);
        if (was_initialized ||
            !reinitialize_video_with_portable_providers()) {
            video_mode = VIDEO_RAW_EGL;
            fprintf(stderr,
                    "[hgo/video] SDL video unavailable (%s); using raw EGL\n",
                    initial_error);
            return 0;
        }
        fprintf(stderr,
                "[hgo/video] SDL video recovered with portable providers\n");
    }

    const char *driver = SDL_GetCurrentVideoDriver();
    int force_sdl = env_on("HGO_PURE_SDL_CONTEXTS");
    if (!force_sdl && driver && strcasecmp(driver, "mali") == 0) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr,
                "[hgo/video] backend '%s' -> raw EGL/fbdev\n", driver);
        if (!was_initialized)
            SDL_QuitSubSystem(SDL_INIT_VIDEO);
        drop_portable_provider_retry();
        return 0;
    ...
    fprintf(stderr, "[hgo/video] backend '%s' -> SDL-owned contexts%s\n",
            driver ? driver : "?", force_sdl ? " (override)" : "");
    if (!create_window()) {
        char initial_driver[64];
        char initial_error[256];
        snprintf(initial_driver, sizeof initial_driver, "%s",
                 driver ? driver : "?");
        copy_sdl_error(initial_error, sizeof initial_error);

        if (!was_initialized && !portable_provider_retry_active &&
            reinitialize_video_with_portable_providers()) {
            driver = SDL_GetCurrentVideoDriver();
            if (!force_sdl && driver && strcasecmp(driver, "mali") == 0) {
                video_mode = VIDEO_RAW_EGL;
                fprintf(stderr,
                        "[hgo/video] portable retry selected backend '%s' -> "
                        "raw EGL/fbdev\n", driver);
                SDL_QuitSubSystem(SDL_INIT_VIDEO);
                drop_portable_provider_retry();
                return 0;
            ...
            fprintf(stderr,
                    "[hgo/video] portable provider retry backend '%s'\n",
                    driver ? driver : "?");
            if (create_window()) {
                fprintf(stderr,
                        "[hgo/video] recovered with portable EGL/GLES "
                        "provider names\n");
                return 1;
            }
        }
        nx_die("SDL owns backend '%s' but could not create a GLES2 window "
               "(initial error: %s; final error: %s)",
               initial_driver, initial_error, SDL_GetError());
    }
    return 1;
}
...
{
    hgo_sdl_surface *surface = calloc(1, sizeof *surface);
    if (!surface)
...
{
    (void)display;
    hgo_sdl_surface *surface = (hgo_sdl_surface *)handle;
                                       ...
                                       const EGLint *attributes)
{
    ...
    (void)share;
    (void)attributes;
    hgo_sdl_context *context = calloc(1, sizeof *context);
    if (!context)
...
{
    (void)display;
    hgo_sdl_context *context = (hgo_sdl_context *)handle;
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    ...
    hgo_sdl_context *context = (hgo_sdl_context *)handle;
    hgo_sdl_surface *surface = (hgo_sdl_surface *)draw;
    if (!context || context->magic != CONTEXT_MAGIC ||
...
EGLBoolean hgo_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    (void)display;
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Hitman GO SDL EGL";
