...
int hgo_log_level = 0;   /* HGO_LOGCAT=1 mirrors the game's own log */
...
{
    fprintf(stderr, "[hgo] abort message: %s\n", m ? m : "(null)");
}
...
static int my_fclose(void *f)               { return f == (void *)hgo_sF ? 0 : fclose(xf(f)); }
...
static void my___stack_chk_fail(void)
{
    ...
    void *base = NULL;
    extern const char *hgo_mod_at(const void *addr, void **base_out);
    const char *module = hgo_mod_at(caller, &base);
    if (module && base)
...
static const unsigned char hgo_ctype_tab[1 + 256] = { 0 };
static const unsigned char *hgo_ctype_ptr = hgo_ctype_tab + 1;
...
extern void *hgo_android_sym(const char *name);
extern void *hgo_egl_sym(const char *name);
extern void *hgo_jni_sym(const char *name);
...
/* reallocarray is part of bionic's allocator surface, but forwarding it to
 * glibc would make the public binary require GLIBC_2.26.  Keep the Android
 * semantics locally: reject a multiplication overflow before realloc sees a
 * wrapped byte count. */
...
static nx_import tab[] = {
    ...
    A("__sF", hgo_sF), A("_ctype_", &hgo_ctype_ptr),
...
static void *my_dlsym(void *h, const char *sym)
{
    ...
    if (!a) a = hgo_android_sym(sym);
    if (!a) a = hgo_egl_sym(sym);
    if (!a) a = hgo_jni_sym(sym);
...
static int my_dladdr(const void *addr, void *info)
{
    ...
    extern const char *hgo_mod_at(const void *addr, void **base_out);
    void *base = NULL;
    const char *name = hgo_mod_at(addr, &base);
...
static int my_dl_iterate_phdr(int (*cb)(void *, size_t, void *), void *data)
{
    ...
    extern int hgo_iterate_mods(int (*cb)(void *, size_t, void *), void *data);
    return hgo_iterate_mods(cb, data);
}
...
size_t hgo_bionic_count(void) { return sizeof tab / sizeof *tab; }

nx_import *hgo_bionic_entries(size_t *n)
{
