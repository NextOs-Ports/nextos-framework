...
static hgo_window the_window = { 1, 1280, 720, 1 /* RGBA_8888 */ };
...
void hgo_window_set_size(int w, int h)
{
    the_window.width = w;
    the_window.height = h;
...
void *hgo_window_handle(void) { return &the_window; }
...
{
    if (w) __atomic_fetch_add(&((hgo_window *)w)->refs, 1, __ATOMIC_RELAXED);
}
static void a_window_release(void *w)
{
    if (w) __atomic_fetch_sub(&((hgo_window *)w)->refs, 1, __ATOMIC_RELAXED);
}
static int32_t a_window_getWidth(void *w)  { return w ? ((hgo_window *)w)->width : 0; }
static int32_t a_window_getHeight(void *w) { return w ? ((hgo_window *)w)->height : 0; }
static int32_t a_window_getFormat(void *w) { return w ? ((hgo_window *)w)->format : 1; }
...
typedef struct { int32_t refs; } hgo_looper;
static __thread hgo_looper *tls_looper;
static hgo_looper main_looper = { 1 };
...
static void a_looper_acquire(void *l) { if (l) ((hgo_looper *)l)->refs++; }
static void a_looper_release(void *l) { if (l) ((hgo_looper *)l)->refs--; }
...
const nx_import *hgo_android_table(size_t *n)
{
