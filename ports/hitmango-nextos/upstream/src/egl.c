...
static void *gl_raw(const char *name)
{
    if (!name || name[0] != 'g' || name[1] != 'l')
        return NULL;
    if (hgo_sdl_video_active())
        return hgo_sdl_gl_proc(name);
    if (!libgl) {
...
static hgo_uniform_name uniform_names[128];
...
static void remember_uniform(GLuint program, GLint location, const char *name)
{
    if (location < 0 || !name || uniform_name_count >=
        ...
        return;
    hgo_uniform_name *entry = &uniform_names[uniform_name_count++];
...
static int trace_texture_call(unsigned long call, GLsizei width,
                              GLsizei height)
{
    return hgo_trace_gl && (call <= 160 || width >= 512 || height >= 512);
}

static void capture_current_fbo_if_requested(void)
{
    static int finished;
    const char *path = getenv("HGO_FBO_CAPTURE");
...
{
    const char *prefix = getenv("HGO_GL_DUMP_PREFIX");
...
static void my_glBindFramebuffer(GLenum target, GLuint framebuffer)
{
    ...
    if (getenv("HGO_FBO_FINISH") && previous != 0 && framebuffer == 0 &&
        (target == GL_FRAMEBUFFER || target == 0x8CA9)) {
        if (!finish)
        ...
        if (finish) {
            ...
            if (hgo_trace_gl)
                fprintf(stderr,
                        "[hgo/gl] forced finish before fbo %u -> 0\n",
                        previous);
        }
    }
    if (real)
    ...
    framebuffer_calls++;
    if (hgo_trace_gl && framebuffer_calls <= 160)
        fprintf(stderr, "[hgo/gl] fbo-bind #%lu target=%#x framebuffer=%u\n",
...
{
    if (getenv("HGO_NO_OPAQUE_BACKBUFFER"))
        return;
    ...
    if (!gl_state.initialized) {
        ...
        if (getenv("HGO_GLSTATE_TRACE")) {
            fprintf(stderr,
                    "[hgo/gl] state fbo=%u mask=%u%u%u%u "
...
static void draw_gamepad_keyboard(void)
{
    ...
    int uppercase = 0;
    int selected = 0;
    const hgo_keyboard_key *keys = NULL;
    size_t key_count = 0;
    if (!hgo_input_keyboard_snapshot(typed, sizeof typed, &uppercase,
...
static void draw_gamepad_cursor(void)
{
    ...
    if (!hgo_input_cursor(&cursor_x, &cursor_y))
        return;
    ...
    hgo_input_set_screen_size(width, height);
...
static GLuint my_glCreateShader(GLenum type)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] create %s shader=%u\n",
    ...
    if (hgo_trace_gl) {
        ...
        for (size_t i = 0; i < used; i++)
            if (preview[i] == '\n' || preview[i] == '\r')
                preview[i] = ' ';
        fprintf(stderr,
                "[hgo/gl] source #%lu shader=%u stage=%s bytes=%zu: %.160s\n",
                shader_sources, shader, shader_stage(shader), total, preview);
        if (getenv("HGO_GLSOURCE")) {
            fprintf(stderr, "[hgo/gl/source-begin] shader=%u stage=%s\n",
...
static void my_glCompileShader(GLuint shader)
{
    ...
    if (hgo_trace_gl || ok != GL_TRUE) {
        ...
        fprintf(stderr,
                "[hgo/gl] compile #%lu shader=%u stage=%s ok=%d log=%s\n",
...
static void my_glLinkProgram(GLuint program)
{
    ...
    if (hgo_trace_gl || ok != GL_TRUE) {
        ...
        fprintf(stderr, "[hgo/gl] link #%lu program=%u ok=%d log=%s\n",
...
static void my_glUseProgram(GLuint program)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] use-program %u\n", program);
    if (!real)
...
static GLint my_glGetAttribLocation(GLuint program, const GLchar *name)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] attrib program=%u location=%d name=%s\n",
...
static void my_glBindBuffer(GLenum target, GLuint buffer)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] buffer-bind target=%#x buffer=%u\n",
                target, buffer);
    if (!real)
                            ...
                            GLenum usage)
{
    ...
    if (!real)
        return;
    if (getenv("HGO_BUFFER_SPLIT_UPLOAD") && data && size > 0 &&
        ...
        if (sub) {
            ...
            if (hgo_trace_gl)
                fprintf(stderr,
                        "[hgo/gl] split buffer upload target=%#x "
                        "buffer=%u bytes=%ld\n",
                               ...
                               GLsizeiptr size, const void *data)
{
    ...
    if (hgo_trace_gl && buffer_uploads <= 160) {
        GLuint buffer = target == GL_ARRAY_BUFFER ? bound_array_buffer
                        ...
                        ? bound_element_array_buffer : 0;
        fprintf(stderr,
                "[hgo/gl] buffer-sub #%lu target=%#x buffer=%u offset=%ld "
                "bytes=%ld",
                                     ...
                                     const void *pointer)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr,
                "[hgo/gl] attrib-pointer index=%u size=%d type=%#x norm=%u "
...
static void my_glEnableVertexAttribArray(GLuint index)
{
    static void (*real)(GLuint);
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] attrib-enable index=%u\n", index);
    if (!real)
...
static void my_glDisableVertexAttribArray(GLuint index)
{
    static void (*real)(GLuint);
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] attrib-disable index=%u\n", index);
    if (!real)
                                      ...
                                      GLint level)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr,
                "[hgo/gl] fbo-texture fbo=%u target=%#x attachment=%#x "
                                         ...
                                         GLuint renderbuffer)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr,
                "[hgo/gl] fbo-renderbuffer fbo=%u target=%#x attachment=%#x "
                                     ...
                                     GLsizei width, GLsizei height)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr,
                "[hgo/gl] renderbuffer-storage target=%#x internal=%#x "
                "size=%dx%d\n",
...
static GLenum my_glCheckFramebufferStatus(GLenum target)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] fbo-status fbo=%u target=%#x status=%#x\n",
...
{
    if (!hgo_trace_gl || gl_state.framebuffer == 0 || draw_calls > 20)
        return;
    static GLenum (*check)(GLenum);
    if (!check)
        check = gl_raw("glCheckFramebufferStatus");
    if (check)
        fprintf(stderr, "[hgo/gl] pre-draw fbo=%u status=%#x\n",
...
static GLint my_glGetUniformLocation(GLuint program, const GLchar *name)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] uniform program=%u location=%d name=%s\n",
                program, location, name ? name : "(null)");
    return location;
}
...
static void my_glUniform1i(GLint location, GLint value)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] uniform1i location=%d name=%s value=%d\n",
                           ...
                           GLfloat w)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr,
                "[hgo/gl] uniform4f location=%d name=%s "
                "value=%.9g,%.9g,%.9g,%.9g\n",
                            ...
                            const GLfloat *value)
{
    ...
    if (hgo_trace_gl && value && count > 0) {
        fprintf(stderr,
                "[hgo/gl] uniform4fv location=%d name=%s count=%d "
                "first=%.9g,%.9g,%.9g,%.9g\n",
                location, uniform_label(location), count,
                value[0], value[1], value[2], value[3]);
        if (count == 4)
            fprintf(stderr,
                    "[hgo/gl] uniform4fv-matrix location=%d name=%s "
...
static void my_glBindTexture(GLenum target, GLuint texture)
{
    ...
    if (hgo_trace_gl && texture_binds <= 160)
        fprintf(stderr,
                "[hgo/gl] tex-bind #%lu unit=%#x target=%#x texture=%u\n",
...
static void my_glPixelStorei(GLenum pname, GLint value)
{
    ...
    if (hgo_trace_gl)
        fprintf(stderr, "[hgo/gl] pixel-store pname=%#x value=%d\n",
                pname, value);
    if (!real)
...
static void my_glTexParameteri(GLenum target, GLenum pname, GLint value)
{
    ...
    texture_parameters++;
    if (hgo_trace_gl && texture_parameters <= 240)
        fprintf(stderr,
                "[hgo/gl] tex-param #%lu unit=%#x texture=%u target=%#x "
                "pname=%#x value=%#x\n",
                            ...
                            GLenum format, GLenum type, const void *data)
{
    ...
    if (trace_texture_call(texture_images, width, height))
        fprintf(stderr,
                "[hgo/gl] tex-image #%lu unit=%#x texture=%u target=%#x "
                               ...
                               GLenum type, const void *data)
{
    ...
    if (trace_texture_call(texture_sub_images, width, height))
        fprintf(stderr,
                "[hgo/gl] tex-sub #%lu unit=%#x texture=%u target=%#x "
...
static void my_glDrawArrays(GLenum mode, GLint first, GLsizei count)
{
    ...
    draw_calls++;
    if (hgo_trace_gl && (draw_calls <= 20 || draw_calls % 1000 == 0))
        fprintf(stderr,
                "[hgo/gl] draw #%lu arrays mode=%#x first=%d count=%d "
                              ...
                              const void *indices)
{
    ...
    if (hgo_trace_gl && (draw_calls <= 20 || draw_calls % 1000 == 0))
        fprintf(stderr,
                "[hgo/gl] draw #%lu elements mode=%#x count=%d type=%#x "
                                      ...
                                      GLsizei image_size, const void *data)
{
    ...
    if (trace_texture_call(texture_images, width, height))
        fprintf(stderr,
                "[hgo/gl] tex-compressed #%lu unit=%#x texture=%u target=%#x "
                                         ...
                                         GLsizei image_size, const void *data)
{
    ...
    if (trace_texture_call(texture_sub_images, width, height))
        fprintf(stderr,
                "[hgo/gl] tex-compressed-sub #%lu unit=%#x texture=%u "
...
static EGLSurface my_eglCreateWindowSurface(EGLDisplay dpy, EGLConfig cfg,
                                            ...
                                            const EGLint *attrib)
{
    extern void *hgo_native_window(void);
    if (!p_eglCreateWindowSurface)
        p_eglCreateWindowSurface = sys("eglCreateWindowSurface");
    EGLNativeWindowType real = (EGLNativeWindowType)hgo_native_window();
...
static void capture_raw_frame_if_requested(void)
{
    ...
    const char *path = getenv("HGO_SCREENSHOT");
    if (!path || !*path)
        return;

    int live_mode = getenv("HGO_SCREENSHOT_LIVE") &&
                    strcmp(getenv("HGO_SCREENSHOT_LIVE"), "0") != 0;
    int live_request = live_mode && access("/tmp/hgoshot", F_OK) == 0;
    if (live_mode) {
        if (!live_request)
            return;
        unlink("/tmp/hgoshot");
    ...
    if (!live_mode) {
        long wanted = 300;
        const char *value = getenv("HGO_SCREENSHOT_FRAME");
        if (value && *value) {
            char *end = NULL;
...
/* Both presentation paths funnel through my_eglSwapBuffers, so counting and
 * sampling here covers the SDL-owned backend and the raw Mali one with one
 * hook.  Sampling more than one frame is required: a title card can legitimately
 * be black at the first reading. */
static void frame_proof_tick(void)
{
    static unsigned long frame;
    /* The real gl* entry points live only in this port's shim, so hand the
     * adapter the shim's resolver instead of letting it fall back to dlsym. */
    static int resolver_installed;
    if (!resolver_installed) {
        resolver_installed = 1;
        nxgl_frame_proof_set_resolver(hgo_gl_sym);
    }
    frame++;
...
static EGLBoolean my_eglSwapBuffers(EGLDisplay display, EGLSurface surface)
{
    ...
    frame_proof_tick();
    if (hgo_sdl_video_active())
        return hgo_sdl_swap_buffers(display, surface);
...
static void *my_eglGetProcAddress(const char *name)
{
    ...
    if (name[0] == 'g' && name[1] == 'l')
        return hgo_gl_sym(name);
    if (hgo_sdl_video_active())
        return hgo_sdl_egl_proc(name);
...
void hgo_egl_init(void)
{
    ...
    int sdl_owned = hgo_capture_mode ? 0 : hgo_sdl_video_init();
    tab_n = 0;
    tab[tab_n++] = (nx_import){ "eglChooseConfig",
        sdl_owned ? hgo_sdl_egl_proc("eglChooseConfig")
                  : (void *)my_eglChooseConfig };
    tab[tab_n++] = (nx_import){ "eglCreateContext",
        sdl_owned ? hgo_sdl_egl_proc("eglCreateContext")
                  : (void *)my_eglCreateContext };
    tab[tab_n++] = (nx_import){ "eglCreateWindowSurface",
        sdl_owned ? hgo_sdl_egl_proc("eglCreateWindowSurface")
                  : (void *)my_eglCreateWindowSurface };
    tab[tab_n++] = (nx_import){ "eglGetDisplay",
        sdl_owned ? hgo_sdl_egl_proc("eglGetDisplay")
    ...
    for (size_t i = 0; i < sizeof passthrough / sizeof *passthrough; i++) {
        void *f = sdl_owned ? hgo_sdl_egl_proc(passthrough[i])
...
const nx_import *hgo_egl_table(size_t *n)
{
    *n = tab_n;
    return tab;
}
