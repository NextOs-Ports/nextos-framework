...
int sf_log_level = 0;   /* SF_LOGCAT=1 mirrors the game's own log */
...
{
    fprintf(stderr, "[sf] abort message: %s\n", m ? m : "(null)");
}

/* -------------------------------------------------------- system properties */

/* Unity reads these before choosing its graphics backend.  Report the actual
 * GLES generation offered by the Mali-450 bridge: advertising GLES 3 here
 * while eglCreateContext supplies ES2 leaves Unity's GLES3 renderer running on
 * an ES2 context, which works for the simple title and loses the full scene. */
...
static int my_fclose(void *f)               { return f == (void *)sf_sF ? 0 : fclose(xf(f)); }
...
static void my___stack_chk_fail(void)
{
    ...
    void *base = NULL;
    extern const char *sf_mod_at(const void *addr, void **base_out);
    const char *module = sf_mod_at(caller, &base);
    if (module && base)
...
static const unsigned char sf_ctype_tab[1 + 256] = { 0 };
static const unsigned char *sf_ctype_ptr = sf_ctype_tab + 1;
...
/* Rastro de arquivo: sem ele nao da' para saber QUAL caminho a engine tentou
 * abrir, e a diferenca entre "nao procurou" e "procurou no lugar errado" e'
 * exatamente o que decide um bug de dados. */
static int unity_cmdline_path(const char *path)
{
    if (!path)
        return 0;
    char process_cmdline[64];
    snprintf(process_cmdline, sizeof process_cmdline, "/proc/%ld/cmdline",
             (long)getpid());
    return strcmp(path, "/proc/self/cmdline") == 0 ||
           strcmp(path, process_cmdline) == 0;
}

static FILE *unity_cmdline_stream(const char *origin)
{
    static const char command_line[] =
        "sallyface\0-force-gles20\0";
    FILE *temporary = tmpfile();
    if (!temporary)
        return NULL;
    if (fwrite(command_line, 1, sizeof command_line - 1, temporary) !=
            sizeof command_line - 1 ||
        fflush(temporary) != 0 || fseek(temporary, 0, SEEK_SET) != 0) {
        fclose(temporary);
        return NULL;
    }
    static int cmdline_logged;
    if (!__atomic_exchange_n(&cmdline_logged, 1, __ATOMIC_RELAXED))
        fprintf(stderr,
                "[sf/gfx] Unity cmdline: -force-gles20 via %s\n",
                origin ? origin : "arquivo");
    return temporary;
}

static int unity_cmdline_fd(const char *origin)
{
    FILE *temporary = unity_cmdline_stream(origin);
    if (!temporary)
        return -1;
    int fd = dup(fileno(temporary));
    fclose(temporary);
    if (fd >= 0)
        lseek(fd, 0, SEEK_SET);
    return fd;
...
static int my_open(const char *path, int flags, ...)
{
    ...
    /* Unity reads /proc/<pid>/cmdline during initJni.  The original player data
     * lists GLES3 only, so ask the engine to enter its native GLES2 backend
     * before any EGL or renderer state exists.  This is the same public Unity
     * switch used by the approved Cuphead runtime; it does not skip lifecycle
     * calls or replace an engine entry point. */
    if (!getenv("SF_NO_FORCE_GLES20") &&
        (flags & O_ACCMODE) == O_RDONLY && unity_cmdline_path(path)) {
        int fd = unity_cmdline_fd("open");
        if (fd >= 0)
            return fd;
    }
    int fd = open(path, flags, mode);
    BIONIC_TRACE("open(\"%s\", %#x) -> %d", path ? path : "(null)", flags, fd);
    return fd;
}
...
static int my_openat(int dirfd, const char *path, int flags, ...)
{
    ...
    if (!getenv("SF_NO_FORCE_GLES20") &&
        (flags & O_ACCMODE) == O_RDONLY && unity_cmdline_path(path)) {
        int fd = unity_cmdline_fd("openat");
        if (fd >= 0)
            return fd;
    }
    int fd = openat(dirfd, path, flags, mode);
    BIONIC_TRACE("openat(\"%s\", %#x) -> %d", path ? path : "(null)", flags, fd);
    return fd;
}

static FILE *my_fopen(const char *path, const char *mode)
{
    if (!getenv("SF_NO_FORCE_GLES20") && mode && mode[0] == 'r' &&
        unity_cmdline_path(path)) {
        FILE *file = unity_cmdline_stream("fopen");
        if (file)
            return file;
    }
    FILE *f = fopen(path, mode);
    BIONIC_TRACE("fopen(\"%s\", \"%s\") -> %p", path ? path : "(null)",
                 mode ? mode : "?", (void *)f);
    return f;
}
...
extern void *sf_android_sym(const char *name);
extern void *sf_egl_sym(const char *name);
extern void *sf_jni_sym(const char *name);

/* glibc so ganhou o wrapper reallocarray em 2.26; o piso universal e 2.17. */
static void *my_reallocarray(void *p, size_t nmemb, size_t size)
{
...
static nx_import tab[] = {
    ...
    A("__sF", sf_sF), A("_ctype_", &sf_ctype_ptr),
    ...
    /* files */
    M(open), { "open64", (void *)(uintptr_t)my_open }, M(openat), E(close), E(read), E(write), E(pread),
...
static void *my_dlopen(const char *name, int flags)
{
    ...
    /* O P/Invoke do IL2CPP pede o nome do DllImport cru ("fmod"), sem
     * prefixo nem sufixo: normalizar para o soname antes de comparar. */
    char normalized[128];
    if (!strchr(b, '.') && strncmp(b, "lib", 3) != 0 &&
        (size_t)snprintf(normalized, sizeof normalized, "lib%s.so", b) <
            sizeof normalized)
        b = normalized;
...
static void *my_dlsym(void *h, const char *sym)
{
    ...
    if (!a) a = sf_android_sym(sym);
    if (!a) a = sf_egl_sym(sym);
    if (!a) a = sf_jni_sym(sym);
...
static int my_dladdr(const void *addr, void *info)
{
    ...
    extern const char *sf_mod_at(const void *addr, void **base_out);
    void *base = NULL;
    const char *name = sf_mod_at(addr, &base);
...
static int my_dl_iterate_phdr(int (*cb)(void *, size_t, void *), void *data)
{
    ...
    extern int sf_iterate_mods(int (*cb)(void *, size_t, void *), void *data);
    return sf_iterate_mods(cb, data);
}
...
size_t sf_bionic_count(void) { return sizeof tab / sizeof *tab; }

nx_import *sf_bionic_entries(size_t *n)
{
