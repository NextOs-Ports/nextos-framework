...
static _Thread_local sf_sdl_context *current_context;
...
static void capture_frame_if_requested(void)
{
    ...
    if (finished)
        return;
    const char *path = getenv("SF_SCREENSHOT");
    if (!path || !*path)
        return;
    int wanted = positive_env("SF_SCREENSHOT_FRAME");
    if (!wanted)
        wanted = 300;
...
{
    int width = positive_env("SF_SCREEN_W");
    int height = positive_env("SF_SCREEN_H");
    ...
    fprintf(stderr, "[bc/video] resolution %dx%d (%s)\n",
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Sally Face", SDL_WINDOWPOS_CENTERED,
            SDL_WINDOWPOS_CENTERED, screen_width, screen_height,
            SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN_DESKTOP);
        if (!video_window) {
            fprintf(stderr,
                    "[bc/video] SDL window depth%d/stencil%d failed: %s\n",
        ...
        if (share_root && !context_is_gles()) {
            fprintf(stderr,
                    "[bc/video] SDL returned desktop GL; rejecting it\n");
        ...
        if (share_root) {
            ...
            alpha_size = a;
            fprintf(stderr,
                    "[bc/video] SDL config: R%dG%dB%dA%d depth%d stencil%d\n",
                    r, g, b, a, depth_size, stencil_size);
            break;
        }

        fprintf(stderr,
                "[bc/video] GLES2 depth%d/stencil%d failed: %s\n",
                formats[i].depth, formats[i].stencil, SDL_GetError());
    ...
    /* Via A: publish the drawable SDL really owns before Unity's initJni.
     * EGL continues to expose the same physical surface; only Android's fake
     * surface/metrics contract was stale at the hard-coded 1280x720. */
    const char *aspect_request = getenv("SF_ASPECT");
    sf_display_contract display = sf_display_contract_make(
        screen_width, screen_height, aspect_request);
    screen_width = display.drawable_width;
    screen_height = display.drawable_height;
    sf_window_configure(display.android_width, display.android_height,
                        display.lock_android_size);
    if (!display.requested_policy_valid)
        fprintf(stderr,
                "[sf/display] invalid SF_ASPECT='%s'; using fill\n",
                aspect_request ? aspect_request : "");
    fprintf(stderr,
            "DISPLAY-RECEIPT {\"schema\":\"sf-display-v1\","
            "\"via\":\"android-drawable-contract\","
            "\"policy\":\"%s\","
            "\"drawable\":{\"width\":%d,\"height\":%d},"
            "\"egl_surface\":{\"width\":%d,\"height\":%d},"
            "\"android_window\":{\"width\":%d,\"height\":%d},"
            "\"identity\":%s,\"locked\":%s}\n",
            sf_display_policy_name(display.policy),
            display.drawable_width, display.drawable_height,
            display.drawable_width, display.drawable_height,
            display.android_width, display.android_height,
            display.identity ? "true" : "false",
            display.lock_android_size ? "true" : "false");
    SDL_GL_SetSwapInterval(1);

    const GLubyte *(*get_string)(GLenum) =
        (const GLubyte *(*)(GLenum))SDL_GL_GetProcAddress("glGetString");
    fprintf(stderr,
            "[bc/video] SDL GLES2 depth%d/stencil%d drawable=%dx%d\n",
            depth_size, stencil_size, screen_width, screen_height);
    if (get_string) {
        fprintf(stderr, "[bc/video] GL_VENDOR=%s\n",
                get_string(GL_VENDOR) ?
                    (const char *)get_string(GL_VENDOR) : "?");
        fprintf(stderr, "[bc/video] GL_RENDERER=%s\n",
                get_string(GL_RENDERER) ?
                    (const char *)get_string(GL_RENDERER) : "?");
        fprintf(stderr, "[bc/video] GL_VERSION=%s\n",
...
int sf_sdl_video_init(void)
{
    ...
    if (env_on("SF_RAW_EGL_CONTEXTS")) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr, "[bc/video] raw EGL selected by override\n");
        return 0;
    }
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr,
                "[bc/video] SDL video unavailable (%s); using raw EGL\n",
                SDL_GetError());
        return 0;
    }

    const char *driver = SDL_GetCurrentVideoDriver();
    int force_sdl = env_on("SF_PURE_SDL_CONTEXTS");
    if (!force_sdl && driver && strcasecmp(driver, "mali") == 0) {
        video_mode = VIDEO_RAW_EGL;
        fprintf(stderr,
                "[bc/video] backend '%s' -> raw EGL/fbdev\n", driver);
        if (!was_initialized)
    ...
    fprintf(stderr, "[bc/video] backend '%s' -> SDL-owned contexts%s\n",
...
/* Saida no caminho SDL-owned (KMSDRM/Wayland): o SDL_QuitSubSystem(VIDEO)
 * restaura o CRTC salvo e solta o DRM master, sem o que o framebuffer do jogo
 * fica congelado na tela e o frontend nao reaparece.  No caminho raw/fbdev
 * (Mali-450 Utgard) NAO se toca em GL/SDL no desmonte -- o teardown trava o
 * Utgard -- entao esta funcao e' no-op la'. */
void sf_sdl_video_shutdown(void)
{
    if (video_mode != VIDEO_SDL)
        return;
    if (video_window) {
        SDL_DestroyWindow(video_window);
        video_window = NULL;
    ...
    if (SDL_WasInit(SDL_INIT_VIDEO))
        SDL_QuitSubSystem(SDL_INIT_VIDEO);
    fprintf(stderr, "[bc/video] SDL video encerrado (CRTC restaurado, DRM solto)\n");
}
...
{
    sf_sdl_surface *surface = calloc(1, sizeof *surface);
    if (!surface)
...
{
    (void)display;
    sf_sdl_surface *surface = (sf_sdl_surface *)handle;
                                       ...
                                       const EGLint *attributes)
{
    ...
    (void)share;
    (void)attributes;
    sf_sdl_context *context = calloc(1, sizeof *context);
    if (!context)
...
{
    (void)display;
    sf_sdl_context *context = (sf_sdl_context *)handle;
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    ...
    sf_sdl_context *context = (sf_sdl_context *)handle;
    sf_sdl_surface *surface = (sf_sdl_surface *)draw;
    if (!context || context->magic != CONTEXT_MAGIC ||
...
EGLBoolean sf_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    (void)display;
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Sally Face SDL EGL";
