/* Encoder ETC1 clean-room reaproveitado do padrão aprovado do Cuphead/Summertime Saga.
 * Produz GL_ETC1_RGB8_OES. Para alpha split, codifica A como RGB cinza.
 */
...
static void encode_block(const uint8_t rgb[16][3], uint8_t out[8]) {
  ...
  for (int x = 0; x < 4; x++) {
    for (int y = 0; y < 4; y++) {
      ...
      for (int c = 0; c < 3; c++) avg[half][c] += p[c];
      (*n)++;
    }
  }
  (void)n0;
...
/* Decoder independente, direto da disposição ETC1 do Khronos. O encoder acima
 * só produz os modos individual/differential normais, portanto não precisamos
 * dos modos adicionais T/H/planar do ETC2. */
static void decode_block(const uint8_t in[8], uint8_t out[16][4]) {
  int diff = (in[3] >> 1) & 1;
  int flip = in[3] & 1;
  int table0 = (in[3] >> 5) & 7;
  int table1 = (in[3] >> 2) & 7;
  ...
  if (!diff) {
    base0[0] = expand4((in[0] >> 4) & 15);
    base1[0] = expand4(in[0] & 15);
    base0[1] = expand4((in[1] >> 4) & 15);
    base1[1] = expand4(in[1] & 15);
    base0[2] = expand4((in[2] >> 4) & 15);
    base1[2] = expand4(in[2] & 15);
  } else {
    for (int c = 0; c < 3; c++) {
      int q0 = (in[c] >> 3) & 31;
      int delta = in[c] & 7;
      if (delta & 4) delta -= 8;
      base0[c] = expand5(q0);
      base1[c] = expand5(q0 + delta);
    }
  }

  uint32_t msb = ((uint32_t)in[4] << 8) | in[5];
  uint32_t lsb = ((uint32_t)in[6] << 8) | in[7];
  for (int x = 0; x < 4; x++) {
    for (int y = 0; y < 4; y++) {
      int bit = x * 4 + y;
      int sel = (int)((((msb >> bit) & 1u) << 1) | ((lsb >> bit) & 1u));
      int second = flip ? (y >= 2) : (x >= 2);
      int table = second ? table1 : table0;
      int modifier = etc1_mod[table][sel & 1];
      if (sel & 2) modifier = -modifier;
      const int *base = second ? base1 : base0;
      uint8_t *dst = out[y * 4 + x];
      dst[0] = (uint8_t)clampi(base[0] + modifier, 0, 255);
      dst[1] = (uint8_t)clampi(base[1] + modifier, 0, 255);
      dst[2] = (uint8_t)clampi(base[2] + modifier, 0, 255);
      dst[3] = 255;
    }
  }
}
...
uint8_t *sf_etc1_decode_pair(const uint8_t *rgb, const uint8_t *alpha,
  ...
  for (int by = 0; by < bh; by++) {
    for (int bx = 0; bx < bw; bx++) {
      size_t block = ((size_t)by * (size_t)bw + (size_t)bx) * 8u;
      uint8_t color_px[16][4], alpha_px[16][4];
      decode_block(rgb + block, color_px);
      if (alpha) decode_block(alpha + block, alpha_px);
      for (int y = 0; y < 4; y++) {
        int dy = by * 4 + y;
        if (dy >= h) break;
        for (int x = 0; x < 4; x++) {
          int dx = bx * 4 + x;
          if (dx >= w) break;
          const uint8_t *c = color_px[y * 4 + x];
          uint8_t *d = rgba + ((size_t)dy * (size_t)w + (size_t)dx) * 4u;
          d[0] = c[0];
          d[1] = c[1];
          d[2] = c[2];
          d[3] = alpha ? alpha_px[y * 4 + x][0] : 255;
        }
      }
    }
  }
...
uint8_t *sf_etc1_decode_rgb_alpha8(const uint8_t *rgb, const uint8_t *alpha8,
                                    int w, int h) {
  if (!rgb || !alpha8 || w <= 0 || h <= 0) return NULL;
  uint8_t *rgba = malloc((size_t)w * (size_t)h * 4u);
  if (!rgba) return NULL;
    ...
    for (int bx = 0; bx < bw; bx++) {
      ...
      for (int y = 0; y < 4; y++) {
        ...
        for (int x = 0; x < 4; x++) {
          ...
          if (dx >= w) break;
          const uint8_t *c = color_px[y * 4 + x];
          size_t pixel = (size_t)dy * (size_t)w + (size_t)dx;
          uint8_t *d = rgba + pixel * 4u;
          d[0] = c[0];
          d[1] = c[1];
          d[2] = c[2];
          d[3] = alpha8[pixel];
