...
static double g_last_non_black;    /* rgb non-black share of the last probe */
static int g_have_last_non_black;  /* the probe above actually completed */
...
void nxgl_frame_proof_sample(int width, int height) {
  ...
  {
    /* Blindagem 22/08/2026 (campo dArkOSRE, ES-launch): num contexto criado
     * sobre a Mesa cruzada SEM driver, GL_RENDERER e' NULO e o glReadPixels
     * devolve LIXO -- o probe chegou a declarar OK 100%% com o painel PRETO.
     * Um contexto que nao sabe dizer o proprio renderer nao pode provar
     * imagem nenhuma: a amostra vira DEAD-CONTEXT e o veredito nunca sera'
     * OK. E' o mesmo criterio vivo do reparo de provedor. */
    ...
    const unsigned char *renderer_name =
        get_string ? get_string(0x1F01u /* GL_RENDERER */) : NULL;
    if (get_string != NULL &&
        (renderer_name == NULL || renderer_name[0] == '\0')) {
      g_dead_context++;
  ...
  g_samples++;
  if (g_pending_point == NXGL_PROOF_BEFORE_PRESENT)
    g_point_before++;
  ...
  g_last_non_black = non_black;
  g_have_last_non_black = 1;
...
void nxgl_frame_proof_sample_at(int width, int height,
  ...
  nxgl_frame_proof_sample(width, height);
  g_pending_point = -1;
}
...
void nxgl_frame_proof_before_present(int width, int height) {
  ...
  g_have_last_non_black = 0;
  nxgl_frame_proof_sample_at(width, height, NXGL_PROOF_BEFORE_PRESENT);
  if (g_have_last_non_black) {
    if (g_last_non_black < FRAME_PROOF_MIN_NON_BLACK)
      g_black_streak++;
    else
      g_black_streak = 0;
  } else if (g_dead_context > 0) {
    /* Amostra recusada por contexto morto conta como preta: e' o mesmo
     * painel apagado, so' que com a causa ja' nomeada. */
    g_black_streak++;
  }
  if (frame == 600ul || (frame % 1800ul) == 0ul)
    nxgl_frame_proof_publish();
  if (g_black_streak >= 3 && !g_streak_screamed) {
    int conclusive = 0;
    const char *context = launch_context(&conclusive);
