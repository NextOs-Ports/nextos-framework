 ...
 * nome em <gamedir>/obb/. */
#define COI_OBB_BASENAME "main.154.obb"
...
static const char *g_package_name = "com.sega.ssa.COI";
...
/* Publico: metodos NATIVOS do jogo recebem jstring vinda do Java (ex.
 * SetHaveController(count, name, deviceId) do Action Squad).  O valor NAO e'
 * copiado — o chamador mantem a string viva. */
void *jni_shim_make_string(const char *value) { return make_jstring(value); }
...
static void *jni_GetMethodID(void *env, void *clazz, const char *name,
  ...
  /* Action Squad consulta MyNativeActivity.GetDpi() antes de inicializar a
   * interface mobile. O fallback generico devolvia 0.0f; a engine dividia a
   * diagonal em pixels por zero, gravava mobile_screen_size=+inf e montava a
   * camera da HUD com dimensoes na casa dos bilhoes. 240 dpi e' a densidade
   * anunciada pela AConfiguration deste runtime e mantem o fluxo Java/JNI
   * original, apenas fornecendo a metrica que o Android forneceria. */
  if (strcmp(name, "GetDpi") == 0)
    return &g_method_tags[MID_GET_DPI];
...
static const char *coi_obb_path(void) {
  ...
  const char *gd = getenv("COI_GAMEDIR");
  if (!gd || !*gd) {
    static char cwd[768];
    gd = getcwd(cwd, sizeof(cwd)) ? cwd : ".";
  }
  snprintf(cached, sizeof(cached), "%s/obb/" COI_OBB_BASENAME, gd);
...
static float jni_CallFloatMethod(void *env, void *obj, void *methodID, ...) {
  ...
  if (methodID == &g_method_tags[MID_GET_DPI]) {
    logPrintf("jni_shim: MyNativeActivity.GetDpi -> 240.0\n");
    return 240.0f; /* igual a AConfiguration_getDensity() deste runtime */
  }
  return 0.0f;
}
