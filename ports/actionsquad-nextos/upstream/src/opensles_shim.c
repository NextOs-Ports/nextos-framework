...
static pthread_mutex_t g_pump_thread_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_t g_pump_thread;
static int g_pump_thread_started = 0;
static int g_pump_thread_stop = 0;

static void start_audio_pump(void);
static void stop_audio_pump(void);

static int audio_trace_enabled(void) {
  static int enabled = -1;
  if (enabled < 0) enabled = getenv("AS_AUDIO_TRACE") ? 1 : 0;
  return enabled;
}
...
static void queue_consume(AudioPlayer *p, uint32_t bytes) {
  while (bytes > 0 && p->queued_count > 0) {
    ...
    p->queued_count--;
    p->queued_front_offset = 0;
    __sync_fetch_and_add(&p->completed_buffers, 1);
  }
}
...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    uint32_t raw_got = ring_read(p, tmp, src_bytes_want);
    /* Queue completion is byte-based.  OpenAL primes OpenSL with a one-byte
     * buffer specifically to receive the first completion callback. */
    queue_consume(p, raw_got);
    uint32_t got = (raw_got / frame_size) * frame_size;
  ...
  if (audio_trace_enabled() && (callback_count <= 8 || callback_count % 25 == 0)) {
    logPrintf("[AS-AUDIO] mix cb=%u active=%d peak=%.0f p0_state=%u "
              "p0_ring=%u p0_enq=%u p0_done=%d\n",
              callback_count, num_active, mix_peak, g_players[0].play_state,
              ring_readable(&g_players[0]), g_players[0].enqueue_counter,
              g_players[0].decoder_done);
  }
}
...
static void ensure_audio_initialized(void) {
  ...
  want.channels = 2;
  /* OpenAL requests 1024-frame OpenSL buffers.  Matching that period avoids
   * consuming the whole two-buffer queue in one oversized SDL callback. */
  want.samples = 1024;
  ...
  logPrintf("[AS-AUDIO] SDL opened %d Hz, %d ch, %d frames\n",
            have.freq, have.channels, have.samples);
...
/* Android OpenSL ES dispatches buffer-queue callbacks from an audio worker,
 * never from NativeActivity's render/event thread.  Keep that ordering here:
 * calling the embedded OpenAL callback from ALooper_pollAll can wait on the
 * OpenAL device mutex and stall every frame of the game. */
static void *audio_pump_thread(void *arg) {
  (void)arg;
  const struct timespec interval = { .tv_sec = 0, .tv_nsec = 4000000L };

  while (!__atomic_load_n(&g_pump_thread_stop, __ATOMIC_ACQUIRE)) {
...
static void start_audio_pump(void) {
  if (getenv("AS_NO_AUDIOPUMP")) return;

  pthread_mutex_lock(&g_pump_thread_lock);
  if (!g_pump_thread_started) {
    __atomic_store_n(&g_pump_thread_stop, 0, __ATOMIC_RELEASE);
    int rc = pthread_create(&g_pump_thread, NULL, audio_pump_thread, NULL);
    if (rc == 0) {
      g_pump_thread_started = 1;
      debugPrintf("opensles_shim: dedicated OpenSL callback thread started (4 ms)\n");
      logPrintf("[AS-AUDIO] dedicated OpenSL callback thread started (4 ms)\n");
    } else {
      debugPrintf("opensles_shim: failed to start callback thread: %s\n",
                  strerror(rc));
    }
  }
  pthread_mutex_unlock(&g_pump_thread_lock);
}
...
static void stop_audio_pump(void) {
  ...
  __atomic_store_n(&g_pump_thread_stop, 1, __ATOMIC_RELEASE);
  thread = g_pump_thread;
  pthread_mutex_unlock(&g_pump_thread_lock);

  if (!pthread_equal(pthread_self(), thread)) pthread_join(thread, NULL);

  pthread_mutex_lock(&g_pump_thread_lock);
  g_pump_thread_started = 0;
  pthread_mutex_unlock(&g_pump_thread_lock);
}
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].play_ptr == itf_ptr) {
      ...
      if (audio_trace_enabled())
        logPrintf("[AS-AUDIO] player %d state %u -> %u ring=%u enq=%u\n",
                  i, p->play_state, state, ring_readable(p), p->enqueue_counter);
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (written > 0) {
        ...
        p->ever_enqueued = 1;
        if (audio_trace_enabled() &&
            (p->enqueue_counter <= 12 || p->enqueue_counter % 128 == 0))
          logPrintf("[AS-AUDIO] player %d enqueue #%u size=%u ring=%u state=%u\n",
                    i, p->enqueue_counter, written, ring_readable(p),
                    p->play_state);
...
static SLresult bq_RegisterCallback(void *self, slBufferQueueCallback callback, void *pContext) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (audio_trace_enabled())
        logPrintf("[AS-AUDIO] player %d queue callback=%p context=%p\n",
                  i, (void *)callback, pContext);
      /* if (text_base && ra >= (uintptr_t)text_base &&
          ra < (uintptr_t)text_base + text_size) {
...
static SLresult engine_CreateAudioPlayer(void *self, void **pPlayer,
  ...
  if (audio_trace_enabled())
    logPrintf("[AS-AUDIO] player %ld created capacity=%u format=%u ch/%u Hz/%u bit\n",
              (long)(p - g_players), p->queue_capacity, p->num_channels,
...
/* Public API.  Serializing the pump also keeps a diagnostic/manual caller from
 * invoking the same guest callback concurrently with the audio worker. */
...
static void opensles_pump_locked(void) {
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    /* OpenSL queue callbacks report completed buffers; they are not arbitrary
     * refill requests.  The embedded OpenAL callback wakes its producer thread,
     * which enqueues the replacement asynchronously. */
    int max_calls = 4;
    while (p->callback && max_calls > 0) {
      uint32_t pending = __atomic_load_n(&p->completed_buffers, __ATOMIC_ACQUIRE);
      if (pending == 0) break;
      if (!__atomic_compare_exchange_n(&p->completed_buffers, &pending,
                                       pending - 1, 0, __ATOMIC_ACQ_REL,
                                       __ATOMIC_ACQUIRE))
        continue;
      if (audio_trace_enabled() && p->debug_callback_logs < 12) {
        logPrintf("[AS-AUDIO] pump player %d completion #%u ring=%u pending=%u\n",
                  i, p->debug_callback_logs + 1, ring_readable(p), pending);
