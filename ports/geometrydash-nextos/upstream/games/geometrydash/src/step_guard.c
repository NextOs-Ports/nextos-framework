 ...
 *
 * OFF WITH NO ENV SET, AND THAT IS THE POINT.  Both hooks here reimplement a
 * shipped method from field offsets read out of ONE binary -- they came from
 * Geometry Dash SubZero, and 2.2.144 lays those objects out differently.
 * Installed blind, they are not a guard but a corruption: getModifiedDelta read
 * a carry that was not there and returned zero ticks every frame, and the
 * startGameDelayed replacement set a "playing" byte at an address that means
 * something else, so the level froze on "Attempt 1" with the music silent.
 *
 * The engine is left to run its own code.  The stall that motivated the guard
 * is already answered one level down, by the clock in bionic_shims.c giving
 * back the time a long frame spent loading, so no frame is ever asked to
 * simulate a second of physics in one go.  Set GD_STEP_GUARD=1 only after
 * re-reading the three offsets below out of THIS binary. */
void gd_install_step_guard(void) {
  const char *want = getenv("GD_STEP_GUARD");
  if (!want || *want != '1') {
    debugPrintf("[passo] trava de passo desligada -- a engine roda o proprio "
                "codigo (GD_STEP_GUARD=1 liga, so com offsets deste binario)\n");
    return;
  }
