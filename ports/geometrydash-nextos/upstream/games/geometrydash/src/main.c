/*
 * main.c -- Geometry Dash 2.2.144 (Cocos2d-x 2.2.3, arm64) so-loader for NextOS
 * on Mali-450.
 *
...
extern void gd_save_redirect_init(const char *package, const char *real_dir);
...
int main(int argc, char *argv[]) {
  ...
  if (gd_single_instance_lock() < 0)
    return 74;
  fprintf(stderr,
          "=== Geometry Dash 2.2.144 -- so-loader NextOS aarch64 / Mali-450 ===\n");
  ...
  jni_shim_set_paths(writable, "com.robtopx.geometryjump");
  /* The game builds "/data/data/<package>/" itself for its saves instead of
   * asking for the writable path; send that whole prefix into userdata. */
  gd_save_redirect_init("com.robtopx.geometryjump", writable);
  {
