...
static void ensure_audio_initialized(void) {
   ...
   * mesmo se o ALSA pedir mais. GD_AUDIO_SAMPLES ajusta sem rebuild. */
  {
    const char *e = getenv("GD_AUDIO_SAMPLES");
    int s = e ? atoi(e) : 1024;
  ...
  /* `dummy` e `disk` ABREM e devolvem sucesso -- e o jogo fica mudo achando que
   * tem som. Nao contam como aberto: seguem para a escada de drivers. */
  if (g_audio_dev != 0) {
    const char *drv = SDL_GetCurrentAudioDriver();
    if (drv && (strcmp(drv, "dummy") == 0 || strcmp(drv, "disk") == 0)) {
      fprintf(stderr, "[opensles_shim] driver '%s' nao produz som; procurando "
                      "um de verdade\n", drv);
  ...
  if (g_audio_dev == 0) {
    ...
    for (unsigned i = 0; i < sizeof(cands) / sizeof(cands[0]) && g_audio_dev == 0; i++) {
      ...
      if (g_audio_dev != 0) {
        const char *now = SDL_GetCurrentAudioDriver();
        if (now && (strcmp(now, "dummy") == 0 || strcmp(now, "disk") == 0)) {
          SDL_CloseAudioDevice(g_audio_dev);
          g_audio_dev = 0;
        }
      }
   ...
   * dedicada a ~4ms mantém o ring cheio. COI_NO_AUDIOPUMP=1 desliga. */
  if (!getenv("COI_NO_AUDIOPUMP")) {
