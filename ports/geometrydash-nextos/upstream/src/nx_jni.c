...
/* Keyed on name AND signature: getDeviceRefreshRate exists as both ()I and
 * ()F, and folding them into one ID hands the engine an int through the float
 * slot -- it read the refresh rate as 0, set the frame interval to 1/0, and
 * every delta after that was infinity. */
static int same_sig(const char *a, const char *b) {
    if (!a || !b) return a == b;
    return strcmp(a, b) == 0;
}
...
static void *intern(const char *name, const char *sig, const void *entry, int is_field) {
    for (int i = 0; i < g_id_n; i++)
        if (g_ids[i].is_field == is_field && g_ids[i].name && name &&
            strcmp(g_ids[i].name, name) == 0 && same_sig(g_ids[i].sig, sig))
...
void *nx_method_id(const nx_jni_config *cfg, const char *name, const char *sig) {
    ...
    /* Always report an unresolved method the first time it is asked for.  The
     * table's default is a silent 0, which is right for an ad or an achievement
     * and catastrophic for anything else -- SystemClock.uptimeMillis() answering
     * 0 froze every level while the renderer kept running. */
    if (!entry) {
        ...
        if (!known) {
            ...
            fprintf(stderr, "[nx_jni] metodo NAO RESOLVIDO (devolve 0): %s %s\n",
                    name, sig ? sig : "");
        }
    }
