...
// ---------------- attr (bionic = 56B, glibc = 64B) ----------------
// pthread_attr_init da glibc escreve 8 bytes ALÉM do storage do guest (que
// reservou 56 na pilha) e apaga um registrador salvo do frame dele — smash
// silencioso. Implementamos o layout bionic inteiro aqui e só traduzimos na
// hora do pthread_create.
typedef struct {
  uint32_t flags;
  void *stack_base;
  size_t stack_size;
...
int b_attr_init(void *a) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (!at)
    return 22;
  memset(at, 0, sizeof(*at));
  at->stack_size = 1024 * 1024;
  at->guard_size = 4096;
  at->sched_policy = 0; // SCHED_NORMAL
  return 0;
}
int b_attr_destroy(void *a) {
  (void)a;
  return 0;
}
int b_attr_setdetachstate(void *a, int state) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (!at)
    return 22;
  if (state)
    at->flags |= BIONIC_ATTR_DETACHED;
  else
    at->flags &= ~BIONIC_ATTR_DETACHED;
  return 0;
}
int b_attr_getdetachstate(void *a, int *state) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (state)
    *state = at && (at->flags & BIONIC_ATTR_DETACHED) ? 1 : 0;
  return 0;
}
// bionic aceita pilhas menores que PTHREAD_STACK_MIN; devolver EINVAL aqui fez
// o FMOD::System::init responder ERR_INTERNAL em outro port. Aceitar sempre.
int b_attr_setstacksize(void *a, size_t sz) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (at)
    at->stack_size = sz;
  return 0;
}
int b_attr_getstacksize(void *a, size_t *sz) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (sz)
    *sz = at ? at->stack_size : 1024 * 1024;
  return 0;
}
int b_attr_setguardsize(void *a, size_t sz) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (at)
    at->guard_size = sz;
  return 0;
}
int b_attr_setschedparam(void *a, const int *p) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (at && p)
    at->sched_priority = *p;
  return 0;
}
int b_attr_setschedpolicy(void *a, int policy) {
  bionic_attr_t *at = (bionic_attr_t *)a;
  if (at)
    at->sched_policy = policy;
  return 0;
}
int b_attr_setscope(void *a, int scope) {
  (void)a;
  (void)scope;
  return 0;
}
...
int b_pthread_create(pthread_t *thread, const void *attr,
  ...
  if (attr) {
    const bionic_attr_t *at = (const bionic_attr_t *)attr;
    size_t sz = at->stack_size;
    if (sz < (size_t)PTHREAD_STACK_MIN)
      sz = (size_t)PTHREAD_STACK_MIN;
    if (sz > 64u * 1024 * 1024)
      sz = 64u * 1024 * 1024;
    pthread_attr_setstacksize(&ga, sz);
    if (at->flags & BIONIC_ATTR_DETACHED)
