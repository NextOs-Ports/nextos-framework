          ...
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (!found) {
              ...
              if (s) {
                *ptr = (uintptr_t)s;
                found = 1;
                /* SUMMERTIME_RESOLVE_LOG=1: audita de QUAL biblioteca o
                 * fallback tirou cada simbolo (pega shadowing por libs do
                 * firmware, ex.: blob Mali/PortMaster). */
                if (getenv("SUMMERTIME_RESOLVE_LOG")) {
                  Dl_info di;
                  if (dladdr(s, &di) && di.dli_fname)
                    debugPrintf("resolve: %s -> %s\n", name, di.dli_fname);
                  else
                    debugPrintf("resolve: %s -> %p\n", name, s);
                }
              }
            }
            if (!found)
