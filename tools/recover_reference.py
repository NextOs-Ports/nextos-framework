...
def main(repo_id,relative,destination):
    ...
    url='https://raw.githubusercontent.com/'+repository+'/'+port['source_commit']+'/'+relative
    ...
    (target.parent/(target.name+'.provenance.json')).write_text(json.dumps(report,indent=2)+'\n')
